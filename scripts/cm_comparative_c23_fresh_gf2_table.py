...
from cmbench.comparative.gf2_fresh_table_experiment import C23Config, run


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the C23 fresh exact GF(2) method table")
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--rounds", type=int, default=5)
    parser.add_argument("--max-seconds", type=float, default=1200.0)
    args = parser.parse_args()
    result = run(
        C23Config(args.output.name, rounds=args.rounds, max_seconds=args.max_seconds),
        args.output,
    ...
    print(
        f"C23 status={result['status']} best={summary['best_fixed_method']} "
