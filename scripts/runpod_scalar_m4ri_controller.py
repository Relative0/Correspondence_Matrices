...
def configure(attempt):
    ...
    transport.BUNDLE = AUDIT / 'm4ri-payload.zip'
...
def commands(profile='full'):
    out = '/workspace/cm-packed-evidence/m4ri'
    return [dict(name='scalar-correctness', args=['-m', 'pytest', 'tests/test_scalar_research.py', '-q',
                 '-p', 'no:cacheprovider', '--junitxml', '/workspace/cm-packed-evidence/TESTS.xml'], timeout=60),
            *[dict(name='m4ri-' + action, args=['-m', 'scripts.cm_scalar_m4ri_control', action,
                    '--output', out], timeout=900 if action == 'build' else 2400)
...
def prepare(profile='full'):
    initial = ['scripts/cm_scalar_m4ri_control.py', 'tests/conftest.py',
    ...
    if any(not p.is_file() or p.suffix not in ('.json', '.alist', '.gz', '.c', '') for p in data):
    ...
    payload = AUDIT / 'm4ri-payload.zip'
    ...
    shared.write(AUDIT / 'M4RI-SOURCE_FREEZE.json', source)
    shared.write(AUDIT / 'M4RI-AUTHORIZATION.json', dict(
...
def run(attempt, out):
    ...
    try:
        frozen = shared.load(AUDIT / 'M4RI-SOURCE_FREEZE.json')
