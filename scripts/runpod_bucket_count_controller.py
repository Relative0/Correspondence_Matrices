...
HORIZON = 5700
TESTS = [
    'test_bucket_counts', 'test_bucket_cudd_reference',
...
def commands(profile='full'):
    out = '/workspace/cm-packed-evidence/bucket'
    return [dict(name='correctness', args=['-m', 'pytest', *['tests/' + n + '.py' for n in TESTS],
            ...
            *[dict(name='bucket-' + action, args=['-m', 'scripts.cm_bucket_count_campaign', action,
...
def prepare(profile='full'):
    initial = ['scripts/cm_bucket_count_campaign.py', 'tests/conftest.py',
               *['tests/' + name + '.py' for name in TESTS]]
    files = source_closure(initial)
    data = [AUDIT / name for name in ('CORPUS.json', 'FIXTURES.json', 'SCHEDULE.json', 'PROTOCOL.json', 'ADMISSION.json')]
    data += sorted((AUDIT / 'corpus').iterdir())
    if any(not p.is_file() or p.suffix not in ('.json', '.dimacs', '') for p in data):
