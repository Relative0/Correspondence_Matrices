    ...
    with audit.arm_scope(args.arm):
        ...
        if args.timing_only:
            return dict(n=args.n, arm=args.arm, pid=os.getpid(), import_ns=import_ns,
                        ...
                        released=released, output_sha256=digest,
                        scope="one environment build, checksum delivery, import and process lifecycle")
        tracemalloc.start()
        result = audit.bs.build_bitset_env(names)
...
def run(args):
    ...
    filename = "fresh_process_timing.json" if args.timing_only else "fresh_process.json"
