"""Ephemeral token-gated receiver for authorized shards and two control files."""
import base64
import hashlib
import hmac
...
EXPECTED_CONTROL = json.loads(os.environ.pop("CM_CONTROLS_JSON"))
DEADLINE = float(os.environ.pop("CM_HARD_DEADLINE"))
UPLOAD = Path("/workspace/cm-benchmark-upload")
CONTROL = Path("/workspace/cm-benchmark-control")
RESULT = Path("/workspace/cm-benchmark-result.zip")
CAP = 256 << 20
LOCK = threading.Lock()
LOG = bytearray()
STATE = {"stage": "awaiting-inputs", "done": False, "error": None,
         "uploaded": [], "uploaded_controls": []}
UPLOAD.mkdir(parents=True, exist_ok=False)
CONTROL.mkdir(parents=True, exist_ok=False)
if (not isinstance(EXPECTED_CONTROL, list) or len(EXPECTED_CONTROL) != 2
        or any(not isinstance(row, dict) for row in EXPECTED_CONTROL)
        or {row.get("id") for row in EXPECTED_CONTROL} != {"runner", "plan"}
        or any(not re.fullmatch(r"[a-z0-9_.-]{1,80}", str(row.get("path", "")))
               for row in EXPECTED_CONTROL)):
    raise RuntimeError("invalid control-file declaration")
CONTROL_ROWS = {row["id"]: row for row in EXPECTED_CONTROL}
...
def run_worker():
    ...
    try:
        ...
        env.update({"CM_SHARD_DIR": str(UPLOAD), "CM_SHARDS": json.dumps(EXPECTED),
                    "CM_CORE_SCREEN_SOURCE": str(CONTROL / CONTROL_ROWS["runner"]["path"]),
                    "CM_CORE_SCREEN_PLAN_SOURCE": str(CONTROL / CONTROL_ROWS["plan"]["path"]),
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if time.time() >= DEADLINE:
            ...
            return
        control_match = re.fullmatch(r"/control/(runner|plan)", self.path)
        if control_match:
            control_id = control_match.group(1)
            row = CONTROL_ROWS[control_id]
            length = int(self.headers.get("Content-Length", "-1"))
            if self.headers.get("Transfer-Encoding") or length != row["bytes"] or length > 1 << 20:
                self.reply(400, {"error": "control length mismatch"})
                return
            data = self.rfile.read(length)
            if len(data) != length or hashlib.sha256(data).hexdigest() != row["sha256"]:
                self.reply(400, {"error": "control digest mismatch"})
                return
            target = CONTROL / row["path"]
            with LOCK:
                if target.exists() and hashlib.sha256(target.read_bytes()).hexdigest() != row["sha256"]:
                    self.reply(409, {"error": "existing control mismatch"})
                    return
                if not target.exists():
                    target.write_bytes(data)
                if control_id not in STATE["uploaded_controls"]:
                    STATE["uploaded_controls"].append(control_id)
                STATE["stage"] = "uploaded-control-%d-of-%d" % (
                    len(STATE["uploaded_controls"]), len(EXPECTED_CONTROL))
        ...
        if self.path == "/run":
            with LOCK:
                ...
                if (len(STATE["uploaded"]) != len(EXPECTED)
                        or len(STATE["uploaded_controls"]) != len(EXPECTED_CONTROL)):
                    self.reply(409, {"error": "inputs incomplete"})
                    return
                STATE["started"] = True
