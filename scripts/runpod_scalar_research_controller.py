...
def commands(profile='full'):
    out = '/workspace/cm-packed-evidence/scalar'
    return [dict(name='correctness', args=['-m', 'pytest', *['tests/' + n + '.py' for n in TESTS],
            ...
            *[dict(name='scalar-' + action, args=['-m', 'scripts.cm_scalar_research_campaign', action,
...
def prepare(profile='full'):
    initial = ['scripts/cm_scalar_research_campaign.py', 'tests/conftest.py',
               *['tests/' + name + '.py' for name in TESTS]]
    files = source_closure(initial)
    data = [AUDIT / name for name in ('CORPUS.json', 'FIXTURES.json', 'SCHEDULE.json')]
    data += sorted((AUDIT / 'corpus').iterdir())
    if any(not p.is_file() or p.suffix not in ('.json', '.alist', '') for p in data):
