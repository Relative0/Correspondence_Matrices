"""Frozen full-CNF bucket-count feasibility and native-control campaign."""
import argparse
import gc
import hashlib
import json
...
def methods(case):
    arms = ('bucket_natural', 'bucket_min_fill', 'bucket_cm', 'cudd_natural', 'cudd_dynamic')
    return ('cse', 'factorized')+arms if case['n'] <= 24 else arms


def freeze():
    cases, admissions = [], []
    for item in json.loads((AUDIT/'CORPUS.json').read_text(encoding='utf-8'))['files']:
        path = AUDIT/'corpus'/item['name']; assert common.sha(path) == item['sha256']
        text = path.read_text(encoding='utf-8-sig')
        try:
            n, clauses = parse_dimacs(text, max_vars=512, max_clauses=3000)
            if n == 0: raise ValueError('zero-variable converted model excluded by admission rule')
        except ValueError as exc:
            admissions.append(dict(name=item['name'], admitted=False, reason=str(exc))); continue
        case = dict(id=item['name'][:-7], task='cnf', cohort='public full CNF', n=n,
                    text=text, source_sha256=item['sha256'], source_path=item['path'])
        cases.append(case)
        admissions.append(dict(name=item['name'], admitted=True, n=n, clauses=len(clauses)))
    synthetic = []
    for cohort, widths in (('development', (12, 16)), ('confirmation', (22, 24, 80, 160))):
        for n in widths:
            rng = random.Random(SEED+n)
            for family in ('cycle', 'band', 'dense'):
                if family == 'cycle': clauses = [(-(i+1), (i+1) % n+1) for i in range(n)]
                elif family == 'band':
                    clauses = [(-(i+1), -(i+2)) for i in range(n-1)]
                    clauses += [(i+1, i+2, i+3) for i in range(n-2) if rng.randrange(3) == 0]
                else: clauses = [(-(i+1), -(j+1)) for i in range(n) for j in range(i+1, n)]
                synthetic.append(dict(id=f'{cohort}-{family}-n{n}', task='cnf', cohort=cohort,
                                      family=family, n=n, text=dimacs(n, clauses)))
    ...
    common.write(AUDIT/'ADMISSION.json', admissions)
    common.write(AUDIT/'FIXTURES.json', cases)
    common.write(AUDIT/'SCHEDULE.json', schedule)
    common.write(AUDIT/'PROTOCOL.json', dict(limits=LIMITS, repeats=9, case_timeout_s=180,
                 native_memory_estimate_bytes=256 << 20, child_address_space_limit_bytes=3 << 30,
                 query_rule='q1 unrestricted; q8 seeded assignments to up to three declared variables; retain contradictions',
                 baseline='CSE for n<=24; native CUDD dynamic otherwise',
                 large_packed_control='not attempted above configured width 24',
                 scope='Full DIMACS variable counts, not projected feature-product counts'))
    print(json.dumps(dict(cases=len(cases), cells=len(schedule), corpus_admissions=admissions)))
...
def case_worker(case, output):
    ...
    schedule = [r for r in json.loads((AUDIT/'SCHEDULE.json').read_text()) if r['case'] == case['id']]
...
def run(output):
    ...
    outcomes = []
    for case in json.loads((AUDIT/'FIXTURES.json').read_text()):
        target = output/case['id']; target.mkdir()
        start = time.monotonic()
        try:
            with (target/'WORKER.log').open('xb') as log:
                child = subprocess.run([sys.executable, '-B', '-m', 'scripts.cm_bucket_count_campaign', 'case',
...
def memory(output):
    ...
    largest = max((c for c in cases if c['cohort'] == 'public full CNF'), key=lambda c: c['n'])
    selected = [c for c in cases if c['n'] == 24] + [largest]
    rows = []
    for c in selected:
        for r in range(3):
            for method in methods(c) if r % 2 == 0 else methods(c)[::-1]:
                start = time.perf_counter_ns()
                child = subprocess.run([sys.executable, '-B', '-m', 'scripts.cm_bucket_count_campaign', 'memory-worker',
...
def verify(output):
    ...
    for outcome in json.loads((output/'OUTCOMES.json').read_text()):
        ...
        rows += group
        for q in (1, 8):
            baseline = 'cse' if any(r['method'] == 'cse' for r in group) else 'cudd_dynamic'
    ...
    common.write(output/'RESULT.json', dict(scheduled_cells=len(schedule), accounted_cells=len(rows), complete_cells=sum(r['status']=='complete' for r in rows),
