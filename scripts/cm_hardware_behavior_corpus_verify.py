"""Independently replay a saved hardware behavior-change corpus audit."""
...
from scripts import cm_hardware_behavior_corpus as runner  # noqa: E402
...
def verify(directory: Path, repositories: Path) -> dict[str, object]:
    ...
    final_status = (
        "admissible" if summary["status_without_replay"] == "admissible_pending_replay"
        else summary["status_without_replay"]
    )
    return {
        "schema": "cm-hardware-behavior-corpus-independent-verification/v1",
        "status": "passed",
        "checksum_files_verified": checksum_count,
        "repositories_replayed": len(replay["repositories"]),
        "scanned_commits_replayed": sum(row.get("scanned_commit_count", 0)
                                         for row in replay["repositories"]),
        "selected_transitions_replayed": sum(row.get("selected_transition_count", 0)
                                               for row in replay["repositories"]),
        "selection_hash_license_metric_mismatches": 0,
