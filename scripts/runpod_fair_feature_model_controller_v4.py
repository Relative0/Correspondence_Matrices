...
PROFILE = 'fair-feature-model-v3-jp'
...
def configure(attempt, profile):
    ...
    transport.REMOTE_CODE = (ROOT / 'scripts/cm_fair_feature_model_remote_v3.py').read_text()
...
def reserve(out):
    ...
    try:
        ...
        if prior != 13.0 or prior + total > 15.0:
...
def run(attempt, out):
    ...
    try:
        ...
        assert digest(ROOT / 'scripts/cm_fair_feature_model_remote_v3.py') == frozen['remote_sha256']
        ...
        if machine_id == '52izyrj03ple': raise RuntimeError('previously unready host reassigned')
...
def main():
    ...
    parser.add_argument('--profile', default='fair-feature-model-v3-jp')
