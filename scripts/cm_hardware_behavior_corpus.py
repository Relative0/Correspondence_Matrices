"""Run the frozen, non-timed hardware behavior-change corpus audit."""
...
from cmbench.comparative import hardware_behavior_corpus as audit  # noqa: E402


PROTOCOL = ROOT / "docs/research/CM_HARDWARE_BEHAVIOR_CHANGE_CORPUS_PROTOCOL_2026_09_04.md"
PROGRAM_PATHS = (
    "cmbench/comparative/hardware_behavior_corpus.py",
    "scripts/cm_hardware_behavior_corpus.py",
    "scripts/cm_hardware_behavior_corpus_verify.py",
    "docs/research/CM_HARDWARE_BEHAVIOR_CHANGE_CORPUS_PROTOCOL_2026_09_04.md",
)
...
def process_affinity() -> dict[str, object]:
    if hasattr(os, "sched_getaffinity"):
        ...
        return {"kind": "cpu_set", "logical_count": len(cpus), "cpus": cpus}
    if os.name == "nt":
        process_mask = ctypes.c_size_t()
        system_mask = ctypes.c_size_t()
        kernel = ctypes.WinDLL("kernel32", use_last_error=True)
        ok = kernel.GetProcessAffinityMask(
            kernel.GetCurrentProcess(), ctypes.byref(process_mask), ctypes.byref(system_mask)
        )
        if ok:
            return {"kind": "windows_mask", "logical_count": process_mask.value.bit_count(),
                    "mask_hex": hex(process_mask.value)}
    return {"kind": "unavailable", "logical_count": None}


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--repositories", type=Path,
                        default=ROOT / "tmp/cm-hardware-behavior-corpus/repositories")
    ...
    manifest = {
        "schema": "cm-hardware-behavior-corpus-manifest/v1",
        ...
        "cutoff": audit.CUTOFF,
        "max_scanned_commits": audit.MAX_SCANNED_COMMITS,
        "max_selected_transitions": audit.MAX_SELECTED_TRANSITIONS,
        "clone_command": "git clone --filter=blob:none --no-checkout --branch <branch> <public-url> <ignored-project-tmp-directory>",
        "environment": {
        ...
        "network_during_audit": (
            "partial-clone object acquisition permitted; independent replay disables lazy fetch"
        ),
    }
    write_json(output / "AUDIT.json", evidence)
    write_json(output / "SUMMARY.json", summary)
    write_json(output / "MANIFEST.json", manifest)
    inventory = {"schema": "cm-hardware-behavior-corpus-inventory/v1",
                 "files": checksum_rows(output)}
    write_json(output / "INVENTORY.json", inventory)
    write_checksums(output)
    print(json.dumps({
        "output": str(output),
        "status_without_replay": summary["status_without_replay"],
