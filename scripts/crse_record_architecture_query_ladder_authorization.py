"""Record the user's exact authorization for the frozen query-ladder request."""
...
OUTPUT = HERE / "RUNPOD_ARCHITECTURE_QUERY_LADDER_EXACT_PAYLOAD_AUTHORIZED_2026_09_03.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise SystemExit("refusing to overwrite query-ladder authorization record")
    request = _load(REQUEST)
    if (
        request.get("schema") != "cm-runpod-architecture-query-ladder-authorization-request/v1"
        ...
        or request.get("exact_approval_text") != (
        ...
        raise ValueError("query-ladder request is not the exact non-authorizing artifact")
    scope = request["scope"]
    boundary = request["resource_and_cost_boundary"]
    package = request["package"]
    spec = importlib.util.spec_from_file_location("query_ladder_controller_authorization", CONTROLLER)
    ...
    if (
        ...
        raise ValueError("query-ladder controller or transport source changed after request")
    authorization = {
        "schema": "cm-runpod-architecture-query-ladder-exact-payload-authorization/v1",
        "authorized": True,
        "recorded_utc": datetime.now(timezone.utc).isoformat(),
        "user_authorization_text": request["exact_approval_text"],
    ...
    print(json.dumps({
        ...
        "maximum_total_charge_usd": authorization["user_total_ceiling_usd"],
    }, sort_keys=True))
    return 0
