"""Run the bounded environment/probe stage inside the authorized RunPod Pod."""
import hashlib
...
def run(name, command, timeout):
    ...
    if result.returncode:
        ...
        raise RuntimeError(name + " failed: " + tail)
...
def main():
    ...
    run("balanced-functional-pilot", [sys.executable, "scripts/cm_comparative_task_pilot.py", "run",
                                      "--output", str(OUT / "functional-pilot")], 900)
    run("balanced-functional-pilot-verify", [sys.executable, "scripts/cm_comparative_task_pilot.py", "verify",
                                             "--output", str(OUT / "functional-pilot")], 300)
    summary = {"schema": "cm-benchmark-runpod-environment-probe/v1", "status": "passed",
               "shards": 11, "resource_writes": 1, "performance_claims": 0,
