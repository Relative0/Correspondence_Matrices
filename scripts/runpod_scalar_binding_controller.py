...
def configure(attempt):
    ...
    transport.BUNDLE = AUDIT / 'binding-payload.zip'
...
def commands(profile='full'):
    out = '/workspace/cm-packed-evidence/binding'
    return [dict(name='correctness', args=['-m', 'pytest', *['tests/' + n + '.py' for n in TESTS],
            ...
            dict(name='m4ri-build', args=['-m', 'scripts.cm_scalar_m4ri_control', 'build', '--output', out], timeout=900),
            dict(name='binding-run', args=['-m', 'scripts.cm_affine_binding_diagnostic', 'run', '--output', out], timeout=1200)]
...
def prepare(profile='full'):
    initial = ['scripts/cm_affine_binding_diagnostic.py', 'tests/conftest.py',
               *['tests/' + name + '.py' for name in TESTS]]
    files = source_closure(initial)
    data = [AUDIT / name for name in ('CORPUS.json', 'FIXTURES.json', 'SCHEDULE.json', 'M4RI-UPSTREAM.json', 'm4ri-20240729.tar.gz')]
    data += sorted((AUDIT / 'corpus').iterdir())
    data += [AUDIT / n for n in ('BINDING-FIXTURES.json', 'BINDING-SCHEDULE.json', 'revisions/pre-binding/affine_constraints.py')]
    data.append(ROOT / 'native/cm_scalar_m4ri/control.c')
    if any(not p.is_file() or p.suffix not in ('.json', '.alist', '.gz', '.c', '.py', '') for p in data):
    ...
    payload = AUDIT / 'binding-payload.zip'
    ...
    shared.write(AUDIT / 'BINDING-SOURCE_FREEZE.json', source)
    shared.write(AUDIT / 'BINDING-AUTHORIZATION.json', dict(
...
def run(attempt, out):
    ...
    try:
        frozen = shared.load(AUDIT / 'BINDING-SOURCE_FREEZE.json')
