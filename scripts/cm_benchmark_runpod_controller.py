"""Run the authorized CM benchmark environment/probe stage on one owned CPU Pod."""
...
AUTHORIZATION = BASE / "authorization-001/RUNPOD_AUTHORIZATION.json"
REQUEST = BASE / "approval-request-001/RUNPOD_PAID_LAUNCH_APPROVAL_REQUEST.json"
MANIFEST = PRELAUNCH / "UPLOAD_MANIFEST.json"
BOOTSTRAP = ROOT / "scripts/cm_benchmark_runpod_bootstrap.py"
REMOTE = ROOT / "scripts/cm_benchmark_runpod_remote.py"
...
def require_authorization():
    ...
    if (authorization.get("authorized") is not True or authorization.get("campaign_id") != CAMPAIGN
            or authorization.get("approval_request_sha256") != digest(REQUEST)
            or authorization.get("exact_authorized_text") != request.get("approval_text_to_repeat")
            or authorization.get("upload_manifest_file_sha256") != digest(MANIFEST)
            ...
            or authorization.get("maximum_total_pods") != 2
...
def watchdog(out):
    ...
    if not re.fullmatch(re.escape(CAMPAIGN) + r"-primary-[a-f0-9]{12}", state.get("name", "")):
...
def create_payload(name, manifest, token, created):
    ...
    source = source.replace(marker, repr(base64.b64encode(REMOTE.read_bytes()).decode()))
...
def extract_artifact(out, data):
    ...
    if summary.get("status") != "passed" or summary.get("shards") != 11:
        raise RuntimeError("retrieved probe summary failed")
...
def run(out):
    ...
    try:
        ...
        created = time.time()
        state = {"name": CAMPAIGN + "-primary-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        print(json.dumps({"stage": "create-primary", "rate_usd_per_hour": RATE_CAP}), flush=True)
        ...
        with requests.Session() as transport:
            ...
            while time.time() < created + LIFETIME - 120:
                ...
                if progress.get("done"):
                    ...
                    break
                time.sleep(5)
            else: raise RuntimeError("environment/probe stage deadline exceeded")
