...
CHECKOUT = ROOT / 'build/cm-frontiers'
PROFILE = 'regression'
...
def configure(attempt, profile):
    ...
    transport.REMOTE_CODE = (ROOT / 'scripts/cm_frontier_research_remote_v2.py').read_text()
...
def run(attempt, out):
    ...
    try:
        ...
        assert digest(ROOT / 'scripts/cm_frontier_research_remote_v2.py') == frozen['remote_sha256']
