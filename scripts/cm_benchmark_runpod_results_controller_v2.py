...
AUTHORIZATION = BASE / "results-retry-authorization-001/RUNPOD_RESULTS_RETRY_AUTHORIZATION.json"
...
REMOTE = ROOT / "scripts/cm_benchmark_runpod_remote_v4.py"
...
POST_FAILURE_READINESS = BASE / "account-readiness-004/ACCOUNT_READINESS.json"
EXPECTED_OUT = BASE / "runpod-results-002"
...
def require_authorization():
    ...
    failed_create = load(FAILED_CREATE)
    post_failure = load(POST_FAILURE_READINESS)
    bound = {
        "upload_manifest": digest(MANIFEST),
        "final_reconciliation": digest(RECONCILIATION),
        "first_results_authorization": digest(BASE / "results-authorization-001/RUNPOD_RESULTS_AUTHORIZATION.json"),
        "failed_create_record": digest(FAILED_CREATE),
        "post_failure_readiness": digest(POST_FAILURE_READINESS),
    ...
    if (authorization.get("authorized") is not True or authorization.get("campaign_id") != CAMPAIGN
            ...
            or authorization.get("maximum_creation_retries_after_http_500") != 1
    ...
    if (failed_create.get("status") != "failed" or failed_create.get("creation_http_status") != 500
            or failed_create.get("pod_created") is not False or failed_create.get("uploaded_shards") != 0
            or failed_create.get("cleanup", {}).get("owned_pod_absent") is not True
            or post_failure.get("inventory", {}).get("v1", {}).get("pod_count") != 0
            or post_failure.get("inventory", {}).get("v2", {}).get("pod_count") != 0
            or post_failure.get("transport", {}).get("resource_writes") != 0):
        raise RuntimeError("failed-create or delayed zero-inventory evidence mismatch")
...
def create_payload(name, manifest, token, created):
    ...
    controls = [
        {"id": "runner", "path": "core-screen.py", "bytes": CORE_SCREEN.stat().st_size,
         "sha256": digest(CORE_SCREEN)},
        {"id": "plan", "path": "core-screen-plan.json", "bytes": CORE_PLAN.stat().st_size,
         "sha256": digest(CORE_PLAN)},
    ]
    return {"name": name, "computeType": "CPU", "cloudType": "SECURE", "imageName": IMAGE,
            ...
            "env": {"CM_BOOTSTRAP_TOKEN": token, "CM_SHARDS_JSON": json.dumps(manifest["bundles"]),
                    "CM_CONTROLS_JSON": json.dumps(controls),
...
def upload_control(transport, endpoint, control_id, path, *, attempts=4):
    raw = path.read_bytes()
    ...
    for attempt in range(1, attempts + 1):
        try:
            accepted = json.loads(proxy(
                transport, "POST", endpoint + "/control/" + control_id, data=raw,
                cap=16384, timeout=60,
            ))
            if accepted.get("accepted_sha256") != expected:
                raise RuntimeError("control acknowledgement mismatch")
            history.append({"attempt": attempt, "status": "accepted"})
            return history
        except RuntimeError as exc:
            history.append({"attempt": attempt, "status": str(exc)})
            if str(exc) != "proxy HTTP 404" or attempt == attempts:
                raise
            time.sleep(2 * attempt)
    raise RuntimeError("control upload attempts exhausted")
...
def run(out):
    ...
    record = {"status": "preflight", "started_utc": now(), "creation_attempted": False,
              "creation_uncertain": False, "pod_created": False, "uploaded_controls": 0,
              "uploaded_shards": 0}
    client = state = None
    try:
        ...
        with requests.Session() as transport:
            ...
            record["control_upload_attempts"] = []
            for index, (control_id, path) in enumerate(
                    (("runner", CORE_SCREEN), ("plan", CORE_PLAN)), 1):
                history = upload_control(transport, endpoint, control_id, path)
                record["control_upload_attempts"].append({"control": control_id, "attempts": history})
                record["uploaded_controls"] = index
                print(json.dumps({"stage": "control-upload", "control": index, "of": 2}), flush=True)
    ...
    print(json.dumps({key: record.get(key) for key in
                      ("status", "pod_created", "uploaded_controls", "uploaded_shards",
                       "estimated_compute_cost_usd", "cleanup", "error")},
                     indent=2), flush=True)
