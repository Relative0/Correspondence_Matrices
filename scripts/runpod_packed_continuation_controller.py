...
AUDIT = ROOT / 'docs/audits/2026-09-11-cm-runpod-continuation'
...
def configure(attempt):
    ...
    transport.BUNDLE = AUDIT / 'payload.zip'
...
def commands(profile='full'):
    out = '/workspace/cm-packed-evidence'
    if profile == 'memory':
        return [dict(name='memory-diagnostic', args=['-m', 'scripts.cm_packed_process_memory',
                     'run', '--output', out + '/memory'], timeout=300)]
    panels = out + '/panels'
    io_out = out + '/io'
    return [dict(name='correctness', args=['-m', 'pytest', *['tests/' + n + '.py' for n in TESTS],
                 '-q', '-p', 'no:cacheprovider', '--basetemp', '/workspace/cm-pytest',
                 '--junitxml', out + '/TESTS.xml'], timeout=300),
            dict(name='panels-freeze', args=['-m', 'scripts.cm_packed_cloud_freeze', '--output', panels], timeout=60),
            *[dict(name='panels-' + action, args=['-m', 'scripts.cm_packed_queries_campaign',
                   action, '--output', panels], timeout=900)
              for action in ('development', 'confirmation', 'lifecycle')],
            dict(name='panels-verify', args=['-m', 'scripts.cm_packed_queries_verify', '--output', panels], timeout=600),
            *[dict(name='io-' + action, args=['-m', 'scripts.cm_packed_io_campaign', action,
                   '--output', io_out], timeout=1200) for action in ('freeze', 'run', 'verify')]]
...
def prepare(profile='full'):
    initial = ['scripts/cm_packed_queries_campaign.py', 'scripts/cm_packed_queries_verify.py',
               'scripts/cm_packed_cloud_freeze.py',
               'scripts/cm_packed_process_memory.py',
               'scripts/cm_packed_io_campaign.py', 'tests/conftest.py',
               *['tests/' + name + '.py' for name in TESTS]]
    files = source_closure(initial)
    # This is a Python-source-only allowlist. Optional remote adapter *code*
    # enters through the existing CLI tests; no configuration data is bundled.
    if any(Path(name).suffix != '.py' for name in files):
        raise ValueError('non-source upload candidate')
    ...
    shared.write(AUDIT / 'SOURCE_FREEZE.json', source)
    shared.write(AUDIT / 'AUTHORIZATION.json', dict(
        user_instruction='Please use Runpod for heavier workloads (things that might take more than 10 seconds on this machine). I give you authorization of up to $5 USD, no need to ask for authorization, or authorization for reruns etc.',
        budget_usd=BUDGET, attempt_reservation_usd=RESERVATION,
...
def run(attempt, out):
    ...
    total = math.fsum(shared.load(p)['reserved_usd'] for p in previous) + RESERVATION
    ...
    try:
        frozen = shared.load(AUDIT / 'SOURCE_FREEZE.json')
...
def main():
    ...
    parser.add_argument('--profile', choices=('full', 'memory'), default='full')
