#!/usr/bin/env python3
"""Frozen k=13..15 selector study with tuning/held-out separation.

The tuning side is a deterministic, balanced synthetic corpus.  The held-out
side is the exact-support k=13..15 slice of the frozen EPFL corpus.  Both raw
AST and CM-node flat/words kernels are measured by the deep-audit protocol.
"""
...
def main() -> int:
    ...
    for index, record in enumerate(held_out, 1):
        row = audit._measure_record(
        ...
        row["role"] = "held_out"
        rows.append(row)
        print(f"held_out {index}/{len(held_out)} {row['id']}", flush=True)
    ...
    environment = {
        ...
        "protocol": "paired alternating medians; synthetic tuning and EPFL held-out; thresholds frozen before timing",
        "corpus_sha256": {"synthetic_gap": _sha(corpus), "epfl": _sha(EPFL)},
        "counts": {"tuning": len(tuning), "held_out": len(held_out)},
    }
