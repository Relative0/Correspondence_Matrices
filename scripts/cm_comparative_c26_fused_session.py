...
from cmbench.comparative.gf2_fused_session_experiment import C26Config, run


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the C26 fused resident C22 evaluation")
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--rounds", type=int, default=5)
    parser.add_argument("--max-seconds", type=float, default=1200.0)
    args = parser.parse_args()
    result = run(
        C26Config(args.output.name, rounds=args.rounds, max_seconds=args.max_seconds),
        args.output,
    ...
    print(
        f"C26 status={result['status']} gate={summary['fused_promotion_gate']} "
        f"break_even={summary['fused_advice_on_break_even_query_count']} "
