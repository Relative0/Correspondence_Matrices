...
def configure(attempt, profile):
    ...
    transport.REMOTE_CODE = (ROOT / 'scripts/cm_fair_feature_model_remote.py').read_text()
...
def run(attempt, out):
    ...
    try:
        ...
        assert digest(ROOT / 'scripts/cm_fair_feature_model_remote.py') == frozen['remote_sha256']
...
def main():
    ...
    parser.add_argument('--profile', default='fair-feature-model-v1')
