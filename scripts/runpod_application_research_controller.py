...
CHECKOUT = ROOT / 'build/cm-applications'
PROFILE = 'regression'
...
def configure(attempt, profile):
    ...
    transport.REMOTE_CODE = (ROOT / 'scripts/cm_application_research_remote.py').read_text()
...
def reserve(out):
    ...
    try:
        ...
        prior = math.fsum(shared.load(p)['reserved_usd']
            for name in ('2026-09-11-cm-next-research', '2026-09-12-cm-evidence-frontiers')
            for p in (ROOT/'docs/audits'/name).glob('attempt-*/RESERVATION.json'))
        if prior + total > 10.0: raise RuntimeError('cumulative authorized budget exhausted')
        out.mkdir(exist_ok=False)
        shared.write(out/'RESERVATION.json', dict(reserved_usd=RESERVATION, total_reserved_usd=total,
            budget_usd=BUDGET, profile=PROFILE, recorded_utc=preflight.utc_now()))
        return total
    finally:
        handle.close()
        lock.unlink()
...
def run(attempt, out):
    ...
    try:
        ...
        assert digest(ROOT / 'scripts/cm_application_research_remote.py') == frozen['remote_sha256']
