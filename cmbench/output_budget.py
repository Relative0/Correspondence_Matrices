...
def estimate_explicit_output(
    ...
    *,
    ...
    """Return a conservative, deterministic size estimate.

    The temporary estimate covers the final construction plus evaluator-sized
    packed operands. It is an admission estimate, not a Python heap census.
    """

    k = int(variable_count)
    if k < 0:
        raise ValueError("variable_count must be >= 0")
    slots = max(1, int(operation_slots))
    elements = 1 << k
