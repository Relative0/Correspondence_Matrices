"""C26 fused resident session using hash-bound verified request contexts."""
...
from .gf2_verified_context import (
    VerifiedGF2RequestContext,
    build_verified_gf2_context,
    verify_verified_gf2_context,
)

SESSION_SCHEMA = "crse-c26-fused-gf2-source-portfolio-session/v1"
QUERY_SCHEMA = "crse-c26-fused-gf2-source-portfolio-query/v1"
QUERY_TIMING_FIELDS = (
    "verified_context_ns",
    "plan_cache_ns",
    "exact_completion_ns",
...
@dataclass(frozen=True)
class FusedResidentQueryResult:
    session_id: str
    query_index: int
    case_id: str | None
    n_vars: int | None
...
class FusedResidentSourcePortfolioSession:
    def __init__(self, session_id: str, policy_path: Path, *, advice_enabled: bool = True,
                 max_queries: int = 32, clock: Callable[[], int] = time.perf_counter_ns):
        if (
            ...
            raise ValueError("invalid C26 fused-session configuration")
        setup_started = clock()
        policy = load_source_portfolio_policy(policy_path)
        policy_loaded = max(1, clock() - setup_started)
        self.session_id = session_id
        self.policy = json.loads(json.dumps(policy, allow_nan=False))
        self.advice_enabled = advice_enabled
        self.max_queries = max_queries
        self._clock = clock
        self._plans: dict[int, FusedWidthPlan] = {}
        self._successful_queries = 0
        self._refused_queries = 0
        self._closed = False
        self.setup_timings_ns = {
            "policy_load_validate_ns": policy_loaded,
            "session_initialize_ns": max(1, clock() - setup_started - policy_loaded),
        }
    ...
    def _refused(self, *, case_id: str | None, n_vars: int | None, reason: str,
                 ...
                 total_started: int) -> FusedResidentQueryResult:
        self._refused_queries += 1
        return FusedResidentQueryResult(
            session_id=self.session_id,
    ...
    def execute(self, case: Mapping[str, Any], *,
                force_source_refusal: bool = False) -> FusedResidentQueryResult:
        if type(force_source_refusal) is not bool or force_source_refusal and not self.advice_enabled:
            raise ValueError("invalid C26 query switches")
        ...
        try:
            ...
            context = build_verified_gf2_context(
                case, require_source_packed=self.advice_enabled and not force_source_refusal)
        ...
        if cache_hit:
            ...
            plan = FusedWidthPlan(
                ...
                max_partitions=self.policy["max_partitions"],
                materialize_budget=self.policy["materialize_budget"],
                requested_arm=requested,
            )
            self._plans[context.n_vars] = plan
        if plan.requested_arm != requested:
            raise RuntimeError("C26 cached plan arm mismatch")
        timings["plan_cache_ns"] = max(1, self._clock() - started)

        started = self._clock()
        fallback_used = False
        if self.advice_enabled and not force_source_refusal:
            if not context.source_packed_verified or context.packed_polynomial is None:
                raise RuntimeError("C26 selected source context is incomplete")
            ...
            reason = "verified_context_source_packed"
        else:
            ...
            selected = EXHAUSTIVE
            fallback_used = force_source_refusal
            reason = ("verified_context_source_refused_exact_fallback"
                      if force_source_refusal else "advice_globally_disabled")
        if analysis.source_sha256 != context.truth_sha256:
            raise RuntimeError("C26 exact completion source identity mismatch")
        ...
        if best is not None and ExactGF2Artifact.from_dict(best).reconstruct() != context.truth_bits:
            raise RuntimeError("C26 final artifact reconstruction failed")
        ...
        json.dumps(result_identity, sort_keys=True, separators=(",", ":"), allow_nan=False)
...
def verify_fused_resident_query_result(
    document: dict[str, Any], context: VerifiedGF2RequestContext | None, *,
    policy_sha256: str, required_best: dict[str, Any] | None | object = ...,
    reconstruct_artifact: bool = True,
) -> None:
    expected = {field.name for field in FusedResidentQueryResult.__dataclass_fields__.values()}
    ...
    if (
        ...
        raise ValueError("invalid C26 fused query record")
    if document["status"] == "refused":
        if context is not None or any(document.get(field) is not None for field in (
            ...
            raise ValueError("invalid C26 fused refusal")
        return
    if context is None:
        raise ValueError("C26 successful query requires verified context")
