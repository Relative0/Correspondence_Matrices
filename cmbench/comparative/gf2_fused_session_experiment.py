"""C26 repeated-query evaluation of hash-bound fused C22 sessions."""
...
SCHEMA = "crse-c26-fused-resident-c22-experiment/v1"
METHODS = (
...
class C26Config:
    ...
    def validate(self) -> None:
        if (
            ...
            raise ValueError("invalid C26 experiment bounds")
...
def _fused_query(session: FusedResidentSourcePortfolioSession,
                 case: dict[str, Any], required_best: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("C26 fused query failed exact delivery")
    return {
...
def execute_fused_batch(*, session_id: str, method: str, cases: list[dict[str, Any]],
                        oracles: dict[str, Any], c22_policy_path: Path) -> dict[str, Any]:
    if method not in {"fused_c22_advice_on", "fused_c22_advice_off"} or not cases:
        raise ValueError("invalid C26 fused batch")
    total_started = time.perf_counter_ns()
    started = time.perf_counter_ns()
    session = FusedResidentSourcePortfolioSession(
        session_id,
        c22_policy_path,
        advice_enabled=method == "fused_c22_advice_on",
    ...
    query_records = []
    for case in cases:
        query = _fused_query(session, case, oracles[case["case_id"]]["best_artifact"])
    ...
    if not closed["closed"] or closed["successful_queries"] != len(cases):
        raise RuntimeError("C26 fused session close invariant failed")
...
def summarize(rows: list[dict[str, Any]], memory_rows: list[dict[str, Any]],
    ...
    break_even = next((
        ...
        if by_query_count[str(count)]["methods"]["fused_c22_advice_on"]
        ["aggregate_speedup_over_direct_screened"] >= 1.0
        and by_query_count[str(count)]["methods"]["fused_c22_advice_on"]
    ...
    return {
        ...
        "fused_advice_on_break_even_query_count": break_even,
        "fused_promotion_gate": controls.get("all_passed") is True and break_even is not None,
        "fused_promotion_gate_contract": {
...
def _run_controls(cases: list[dict[str, Any]], oracles: dict[str, Any],
    ...
    fallback_session = FusedResidentSourcePortfolioSession(
        "c26-fallback", c22_policy_path, max_queries=len(cases))
    fallback = []
    for case in cases:
        ...
        if result["best_artifact"] != oracles[case["case_id"]]["best_artifact"]:
            raise RuntimeError("C26 forced fallback changed exhaustive best")
        fallback.append({
    ...
    session = FusedResidentSourcePortfolioSession("c26-refusal", c22_policy_path, max_queries=1)
    ...
    limit = FusedResidentSourcePortfolioSession("c26-limit", c22_policy_path, max_queries=1)
    if limit.execute(seed).status != "ok":
        raise RuntimeError("C26 query-limit setup failed")
    raw_refusals.append({"control_id": "query_limit", "status": limit.execute(seed).status})

    tampered_policy = dict(policy)
    tampered_policy["selected_arm"] = EXHAUSTIVE
    tampered_path = output / "control_policy_tampered.json"
    _write(tampered_path, tampered_policy)
    try:
        FusedResidentSourcePortfolioSession("c26-bad-policy", tampered_path)
    except ValueError:
        status = "refused"
    else:
        status = "accepted"
    raw_refusals.append({"control_id": "tampered_policy_at_setup", "status": status})

    valid_context = build_verified_gf2_context(seed, require_source_packed=True).to_dict()
    context_controls = []
    ...
    return {
        "schema": "crse-c26-fused-session-controls/v1",
...
def render_report(result: dict[str, Any]) -> str:
    ...
    lines = [
        "# C26 fused verified-context resident evaluation", "",
        f"Status: **{result['status']}**  ",
        f"Fused promotion gate: **{'pass' if summary['fused_promotion_gate'] else 'fail'}**  ",
        f"Break-even query count: **{summary['fused_advice_on_break_even_query_count']}**", "",
        "C26 evaluates each expression once into a hash-bound truth/source context. The fused C22",
        "path consumes that context without reparsing or reevaluating the expression and retains",
        "exact CM completion plus final artifact reconstruction.", "",
        "| Queries | Fused advice on vs screened | Minimum width vs screened | Best fixed method |",
        "|---:|---:|---:|---|",
    ]
    for count in QUERY_COUNTS:
        row = summary["by_query_count"][str(count)]
        advice = row["methods"]["fused_c22_advice_on"]
        lines.append(
...
def run(config: C26Config, output: Path, dataset_path: Path,
    ...
    if (
        ...
        raise ValueError("C26 requires the sealed verified C23 corpus")
    ...
    if (
        ...
        raise ValueError("C26 frozen policy contract changed")
    _write(output / "run_spec.json", {
        ...
        "methods": list(METHODS), "lifecycle": "fused_verified_context_resident",
    ...
    if not controls["all_passed"]:
        raise RuntimeError("C26 functional controls failed")
    ...
    if any(not group for group in cases_by_width.values()):
        raise ValueError("C26 support-width coverage mismatch")
    rng = random.Random(f"{config.seed}:c26-balanced/v1")
    rows = []
    for round_index in range(config.rounds):
        ...
        for n_vars, query_count, method in order:
            if time.perf_counter() - wall_started > config.max_seconds:
                raise TimeoutError("C26 experiment exceeded wall bound")
            ...
            if method in DIRECT_METHODS:
                execution = execute_c25_direct_batch(
                    session_id=f"c26-r{round_index}-n{n_vars}-q{query_count}-{method}",
    ...
    for n_vars in N_VARS:
        ...
        for method in METHODS:
            ...
            try:
                if method in DIRECT_METHODS:
                    execution = execute_c25_direct_batch(
                        session_id=f"c26-memory-n{n_vars}-{method}", method=method,
                    ...
                    execution = execute_fused_batch(
                        session_id=f"c26-memory-n{n_vars}-{method}", method=method,
                        cases=sequence, oracles=oracles, c22_policy_path=c22_policy_path)
    ...
    result = {
        ...
        "dataset": {"cases": len(cases), "families": dataset["counts"]["families"],
                    "n_vars": list(N_VARS), "retrospective_fused_evaluation": True,
                    "policy_refit": False},
    ...
    sources = (
        "cmbench/comparative/gf2_fused_session_experiment.py",
        "cmbench/comparative/gf2_resident_session_experiment.py",
        "cmbench/recognition/gf2_fused_source_portfolio_session.py",
        "cmbench/recognition/gf2_verified_context.py",
        "scripts/cm_comparative_c26_fused_session.py",
    )
    artifacts = (
    ...
    _write(output / "manifest.json", {
        "schema": "crse-c26-run-manifest/v1",
