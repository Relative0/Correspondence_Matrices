...
def historical_scenarios(root=ROOT):
    ...
    require(len(selected) == 7, "historical selection unexpectedly changed")
...
def pilot(output, synthetic_only=False):
    ...
    scenarios, selection, admissions = synthetic_scenarios(), [], []
    if not synthetic_only:
        real, selection = historical_scenarios(frozen)
        scenarios += real
...
def main():
    ...
    if args.worker:
        ...
        result = pilot(args.pilot_output, args.synthetic_only)
