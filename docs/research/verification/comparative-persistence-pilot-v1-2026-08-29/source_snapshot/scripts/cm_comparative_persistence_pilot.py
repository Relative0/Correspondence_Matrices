...
bounded functional verification.  CM and structural CSE build, serialize,
decode, reconstruct and query every version under a balanced four-block plan.
The associated task traces are saved as generated controls and explicitly do
not satisfy the natural-session evidence gate.  Timings are diagnostic only.
"""
...
SCHEMA = "cm-comparative-persistence-pilot/v1"
CHECKSUM_SCHEMA = "cm-comparative-persistence-checksums/v1"
SEED = 2026082904
SOURCES = (
...
def build_bundle(root: Path = ROOT) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    schedule = build_plan(
        campaign_id="real-persistence-v1",
        cases=cases,
...
def run(output: Path) -> dict[str, Any]:
    ...
    summary = {
        ...
        "limitations": [
            ...
            "CM and CSE only; native CUDD/ZDD/d4 persistence remains gated",
