...
class PersistenceTests(unittest.TestCase):
    ...
    def test_cm_and_cse_reload_exact_complete_relations_without_answer_cache(self):
        outputs = []
        for backend in persistence.BACKENDS:
            with self.subTest(backend=backend):
                ...
                self.assertEqual(identity["counters"]["programs_built"], 3)
    ...
    def test_bundle_round_trip_is_canonical_and_reconstructs_new_objects(self):
        bundle, _counts = persistence.build_bundle(scenario(), "cm")
        payload = persistence.canonical_bytes(bundle)
        decoded = persistence.decode_bundle(payload, scenario=scenario(), expected_backend="cm")
        loaded = persistence.validate_bundle(decoded, scenario=scenario(), expected_backend="cm")
        self.assertEqual(len(loaded), 3)
        self.assertIsNot(decoded, bundle)
        with self.assertRaises(ValueError):
            persistence.decode_bundle(payload + b"\n", scenario=scenario(), expected_backend="cm")
    ...
    def test_contract_refuses_cross_backend_or_wrong_lifecycle(self):
        ...
        with self.assertRaises(ValueError):
            persistence.execute_persistence(
                scenario=scenario(), backend="cnf", contract=contract, case_id="persistence-k6"
            )
