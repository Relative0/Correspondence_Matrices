...
def run(output: Path) -> dict[str, Any]:
    ...
    for plan in plans:
        for cell in plan["cells"]:
            ...
            tasks.validate_task_result(result, plan["contracts"][cell["arm"]], expected[plan["task"]]["rows"])
...
def verify(output: Path) -> dict[str, Any]:
    ...
    for cell_id, terminal in state["states"].items():
        ...
        tasks.validate_task_result(terminal["result"], plan["contracts"][cell["arm"]], expected)
