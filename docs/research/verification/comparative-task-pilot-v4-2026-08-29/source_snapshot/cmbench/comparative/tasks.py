...
def execute_task(
    *,
    ...
    validate_task_result(result, contract, expected_backend=backend, expected_case_id=case_id)
    return result
...
def validate_task_result(
    ...
    *,
    ...
    require(isinstance(document, Mapping) and set(document) == {"schema", "task", "rows"} and
            ...
            "semantic document")
    require(len(document["rows"]) == validate_contract(contract)["queries"], "semantic query cardinality")
    payload = canonical_bytes(document)
    ...
    require(resources == {"measurement": "in_process_functional_smoke", "rss_measured": False},
            "functional-smoke resource boundary")
    if expected_backend == "sat":
        require(isinstance(identity["native_identity"], Mapping), "native SAT identity missing")
    elif expected_backend is not None:
        require(identity["native_identity"] is None, "native identity on nonnative backend")
