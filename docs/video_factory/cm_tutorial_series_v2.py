...
import cm_tutorial_curriculum_v2 as curriculum
import cm_tutorial_visuals_v2 as visuals
...
def require_unfrozen():
    if (OUT/'DELIVERY_MANIFEST_V2.json').exists():
...
def generate():
    ...
    for index,lesson in enumerate(curriculum.LESSONS):
        folder=OUT/lesson['id']; target=folder/'VOICE_RESPONSE_V2.json'; full='\n\n'.join(s['text'] for s in lesson['scenes'])
        if lesson.get('reuse_audio_from') and not target.exists():
            ...
            shutil.copyfile(old/'brian_source_v1.mp3',folder/'brian_source_v2.mp3')
        ...
        mp3=base64.b64decode(response.pop('audio_base64')); (folder/'brian_source_v2.mp3').write_bytes(mp3)
        response.update(headers);response.update(identity=identity,voice_id=VOICE,voice_name='Brian',payload=payload,generated_unix=time.time(),source_audio_sha256=sha(folder/'brian_source_v2.mp3'))
...
def assemble():
    ...
    for lesson in curriculum.LESSONS:
        folder=OUT/lesson['id']; video=folder/(lesson['id']+'_v2.mp4')
        if video.exists():
            print('Preserving existing master: '+lesson['id'],flush=True);global_frame+=len(lesson['scenes']);continue
        response=json.loads((folder/'VOICE_RESPONSE_V2.json').read_text(encoding='utf-8'))
        wav=folder/'brian_decoded_v2.wav'
        run([ff,'-v','error','-y','-i',str(folder/'brian_source_v2.mp3'),'-ar','48000','-ac','1',str(wav)])
        ...
        chunks.append(data[start:]); audio=folder/'brian_narration_v2.wav'
        ...
        (folder/'captions_v2.vtt').write_text('\n'.join(vtt),encoding='utf-8')
        ...
        write(folder/'QA_REPORT_V2.json',report);global_frame+=len(timeline)
        print(f"Encoded and decoded {lesson['id']}: {total:.2f} seconds",flush=True)

def combine():
    require_unfrozen()
    ff=ffmpeg();target=OUT/'cm_foundations_complete_course_v2.mp4'
    ...
    for lesson in curriculum.LESSONS:
        folder=OUT/lesson['id'];report=json.loads((folder/'QA_REPORT_V2.json').read_text(encoding='utf-8'))
        videos.append("file '"+(folder/(lesson['id']+'_v2.mp4')).as_posix()+"'")
    ...
    (OUT/'course_captions_v2.vtt').write_text('\n'.join(vtt),encoding='utf-8')
    run([ff,'-v','error','-n','-f','concat','-safe','0','-i',str(OUT/'course.concat.txt'),'-i',str(OUT/'course_captions_v2.vtt'),'-i',str(OUT/'course.ffmeta'),'-map','0:v:0','-map','0:a:0','-map','1:s:0','-map_metadata','2','-map_chapters','2','-c:v','copy','-c:a','aac','-b:a','192k','-c:s','mov_text','-metadata:s:s:0','language=eng','-movflags','+faststart',str(target)])
    run([ff,'-v','error','-i',str(target),'-map','0:v','-map','0:a','-f','null','-'])
    write(OUT/'COURSE_QA_V2.json',dict(expected_duration_s=cursor,chapters=len(curriculum.LESSONS),video_sha256=sha(target),full_decode='passed'))
