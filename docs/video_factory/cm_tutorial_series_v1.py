...
import cm_tutorial_curriculum_v1 as curriculum
import cm_tutorial_visuals_v1 as visuals
...
def captions(words):
    ...
    for word in words:
        ...
        if group and (len(text)>76 or word['end']-group[0]['start']>5.5 or word['start']-group[-1]['end']>1):
...
def combine():
    require_unfrozen()
    ff=ffmpeg();target=OUT/'cm_foundations_complete_course_v1.mp4'
