"""One explicitly authorized Linux one-pass confirmation via frozen HTTPS."""
import argparse
import base64
...
OUT = HERE / "runpod-linux-one-pass-execute-002"
PRIOR_LOCAL_REFUSAL = HERE / "runpod-linux-one-pass-execute-001" / "RUN.json"
...
base.REMOTE_CODE = replace_remote_once(
    ...
    "    try:\n"
    "        summary = json.loads((OUT / 'linux-one-pass-confirmation/summary.json').read_text())\n"
    "        validation['confirmation_summary'] = {key: summary.get(key) for key in\n"
    "            ('status', 'semantic_mismatches', 'confirmation_passed', 'sequence_speedup_median')}\n"
    "    except Exception as exc:\n"
    "        validation['validation_error'] = type(exc).__name__ + ': ' + str(exc)",
)
base.REMOTE_CODE = replace_remote_once(
...
base.REMOTE_CODE = replace_remote_once(
    ...
    "emit('stage', name='linux-one-pass-confirmation')\n"
    "    run('linux-one-pass-confirmation', [sys.executable, '-B',\n"
    "         'scripts/crse_linux_one_pass_confirmation.py', '--cases',\n"
    "         'study/natural_normalization_cases.json', '--pack',\n"
    "         'study/proved_rule_pack.json', '--output',\n"
    "         str(OUT/'linux-one-pass-confirmation')], 420)",
)
base.OUT = OUT
BOOTSTRAP_PATH = LEGACY_HERE / "http_transport_bootstrap.py"
MANIFEST_PATH = HERE / "linux_one_pass_upload_manifest.json"
PROPOSAL_PATH = HERE / "LINUX_ONE_PASS_CONFIRMATION_PROTOCOL_2026_08_29.md"
AUTHORIZATION_PATH = HERE / "RUNPOD_LINUX_ONE_PASS_AUTHORIZED_2026_08_29.json"
...
def require_authorization():
    if not AUTHORIZATION_PATH.is_file():
        raise RuntimeError("exact Linux-confirmation authorization record is absent")
    authorization = load(AUTHORIZATION_PATH)
    expected = {
        "schema": "crse-runpod-linux-confirmation-authorization/v1",
        "authorized": True,
        "one_create": True,
        "no_replacement": True,
        "source_files": 16,
    ...
    if any(authorization.get(key) != value for key, value in expected.items()):
        raise RuntimeError("Linux-confirmation authorization scope mismatch")
...
def watchdog():
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(r"cm-linux-confirm-[a-f0-9]{12}", state["name"])
...
def execute_remote(pod_id, token, raw, created, record):
    ...
    with requests.Session() as proxy:
        ...
        while time.time() < created + CLEANUP_AT - 30:
            ...
            if time.time() >= created + 300 and progress.get("stage") != "linux-one-pass-confirmation":
...
def save_evidence(log):
    ...
    study = evidence_root / "linux-one-pass-confirmation"
    ...
    pairs = {}
    for row in rows:
        pairs.setdefault(row.get("round"), {})[row.get("arm")] = row
    semantic_pairs_match = len(pairs) == 5
    for arms in pairs.values():
        if set(arms) != {"no_rewrite", "one_pass"}:
            semantic_pairs_match = False
            continue
        left = {case["case_id"]: case["value_sha256"] for case in arms["no_rewrite"].get("cases", [])}
        right = {case["case_id"]: case["value_sha256"] for case in arms["one_pass"].get("cases", [])}
        semantic_pairs_match = semantic_pairs_match and left == right and len(left) == 32
    ...
    if (validation.get("status") != "complete" or validation.get("error") is not None
        ...
        or summary.get("schema") != "crse-linux-one-pass-confirmation/v1"
        or summary.get("status") != "complete" or summary.get("semantic_mismatches") != 0
        or summary.get("scientific_scope") != "cross-machine confirmation of the frozen one-pass 128-use policy on the unchanged D5 cases"
        or summary.get("config") != {"cases": 32, "cpu_threads": 1, "kernel_repeats": 128, "rounds": 5}
        or summary.get("input", {}).get("cases_sha256") != "ebe582d2b0e3b006dbde48e4314f7aba469e731478fdf8dbe0a5dc1aa95e9a98"
        or summary.get("input", {}).get("pack_file_sha256") != "63393b9a7691710d4730b404bac85a9f377e5f9cd2c09047217353b9a8629915"
        or summary.get("input", {}).get("training_use") is not False
        or summaries.get("applications_by_rule") != {"aig-demorgan-or/v1": 433, "aig-xor-dnf/v1": 18,
                                                        "boolean-common-factor/v1": 0}
        or artifacts.get("schema") != "crse-linux-one-pass-artifacts/v1"
        or artifacts.get("files_sha256") != measured_hashes
        or len(rows) != 10 or {row.get("round") for row in rows} != set(range(5))
        or {row.get("arm") for row in rows} != {"no_rewrite", "one_pass"}
        or any(row.get("schema") != "crse-linux-one-pass-measurement/v1" or row.get("status") != "ok"
               or row.get("mismatches") != 0 or row.get("case_count") != 32
               or row.get("kernel_repeats") != 128 or len(row.get("cases", [])) != 32 for row in rows)
        or not semantic_pairs_match or dependencies.get("numpy") != "2.3.2"
        or runtime.get("source_files") != 16 or runtime.get("runpod_pod_id") != load(IDENTITY)["pod_id"]
        or runtime.get("image_tag") != base.IMAGE_TAG or runtime.get("image_amd64_digest") != base.IMAGE_AMD64_DIGEST):
        raise RuntimeError("retrieved evidence did not satisfy frozen Linux-confirmation gates")
    result["validation"] = validation
    result["runtime_pod_id"] = runtime["runpod_pod_id"]
    result["linux_confirmation"] = {
        "cases": 32,
        "rounds": 5,
        "measurement_rows": len(rows),
        "semantic_mismatches": 0,
        "confirmation_passed": summaries.get("confirmation_passed"),
        "one_pass_speedup_over_no_rewrite": summaries.get("one_pass_speedup_over_no_rewrite"),
        "case_speedup_geomean": summaries.get("case_speedup_geomean"),
        "applications_by_rule": summaries.get("applications_by_rule"),
    }
    return result
...
def run():
    ...
    client = None
    try:
        refused = load(PRIOR_LOCAL_REFUSAL)
        if (refused.get("status") != "refused_pre_create"
                or refused.get("creation_attempted") is not False
                or refused.get("pod_created") is not False):
            raise RuntimeError("prior local refusal is not a proved no-create outcome")
        ...
        prior = load(LEGACY_HERE / "HTTP-CORPUS-FINAL-VERIFICATION-20260829-063212-489230.json")
        if (prior.get("complete") is not True or prior.get("pod_id") != "4q816o02xw5lxn"
                or prior.get("owned_pods_absent_verified") is not True
                or prior.get("create_requests_this_authorization") != 1
                or prior.get("automatic_replacement_queued") is not False):
            raise RuntimeError("prior corpus request is not reconciled")
        ...
        expected_command = ["python", "-B", "scripts/crse_linux_one_pass_confirmation.py", "--cases",
            "study/natural_normalization_cases.json", "--pack", "study/proved_rule_pack.json",
            "--output", "run-output/linux-one-pass-confirmation"]
        lock_lines = REQUIREMENTS_LOCK_PATH.read_text(encoding="utf-8").splitlines()
        if (len(manifest["files"]) != 16 or manifest.get("file_count") != 16
                or manifest.get("bytes") != 573061 or manifest.get("authorization_status") != "pending"
                ...
                or manifest.get("runtime") != {"architecture": "amd64", "image": base.IMAGE,
                                                "numpy": "2.3.2", "python": "3.13.15"}
                or NUMPY_REQUIREMENT not in lock_lines):
            raise RuntimeError("frozen Linux-confirmation package mismatch")
        ...
        created = time.time()
        state = {"name": "cm-linux-confirm-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        write(OUT / "TRANSPORT-FREEZE.json", {
            ...
            "source_files": 16, "source_bytes": sum(row["bytes"] for row in manifest["files"]),
