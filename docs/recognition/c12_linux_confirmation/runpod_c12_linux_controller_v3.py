...
OUT = HERE / "runpod-c12-linux-execute-003"
...
PRIOR_VERIFICATION = HERE / "RUNPOD_C12_LINUX_ATTEMPT_FINAL_VERIFICATION_20260830.json"
PRIOR_NO_CREATE = HERE / "runpod-c12-linux-execute-002/RUN.json"
...
def require_authorization() -> dict:
    ...
    expected = {"schema": "crse-runpod-c12-linux-retry-authorization/v1", "authorized": True,
...
def run() -> int:
    ...
    try:
        ...
        if (no_create.get("status") != "failed"
                or no_create.get("error") != "current account/resource/budget preflight failed"
            ...
            raise RuntimeError("prior retry preflight is not a verified no-create outcome")
        prior = load(PRIOR_VERIFICATION)
        if (prior.get("status") != "safe_failure_reconciled" or prior.get("complete") is not True
                ...
                or prior.get("uploaded_source_files") != 0):
            raise RuntimeError("prior C12 authorization is not safely reconciled")
