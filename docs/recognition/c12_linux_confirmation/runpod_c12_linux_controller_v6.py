"""Execute the corrected, explicitly authorized C12 package-v2 confirmation."""
...
OUT = HERE / "runpod-c12-linux-execute-006"
...
PRIOR_VERIFICATION = HERE / "RUNPOD_C12_LINUX_RETRY_FINAL_VERIFICATION_20260830.json"
PRIOR_NO_CREATE = HERE / "runpod-c12-linux-execute-005/RUN.json"
...
def require_authorization() -> dict:
    ...
    expected = {"schema": "crse-runpod-c12-linux-package-v2-authorization/v1", "authorized": True,
        "user_total_ceiling_usd": 5.0, "controller_total_ceiling_usd": 0.05,
        "one_create": True, "no_replacement": True, "source_files": 16,
        ...
        "prior_attempts_reconciled": True, "local_isolated_validation": "pass"}
...
def save_evidence(log: str) -> dict:
    ...
    if (validation.get("status") != "complete" or validation.get("error") is not None
        ...
        or dependencies.get("numpy") != "2.3.2" or runtime.get("source_files") != 16
...
def run() -> int:
    ...
    state = client = None
    try:
        no_create = load(PRIOR_NO_CREATE)
        if (no_create.get("status") != "failed"
                or no_create.get("error") != "watchdog exited before create"
                or no_create.get("creation_attempted") is not False
                or no_create.get("creation_uncertain") is not False
                or no_create.get("pod_created") is not False
                or no_create.get("uploaded_source_files") != 0
                or no_create.get("estimated_compute_cost_usd") is not None
                or no_create.get("cleanup", {}).get("owned_pod_absent") is not True
                or any(no_create.get("cleanup", {}).get("inventories", {}).values())):
            raise RuntimeError("prior package-v2 launch is not a verified no-create outcome")
        prior = load(PRIOR_VERIFICATION)
        if (prior.get("status") != "safe_workload_failure_reconciled" or prior.get("complete") is not True
                or prior.get("scientific_confirmation_complete") is not False
                or prior.get("automatic_replacement_queued") is not False
                or prior.get("owned_pod_absent_verified") is not True
                or prior.get("transport_upload_complete") is not True
                or prior.get("uploaded_source_files") != 14):
            raise RuntimeError("prior C12 attempts are not safely reconciled")
        local_validation = load(LOCAL_VALIDATION)
        if (local_validation.get("status") != "pass"
                or local_validation.get("initial_file_count") != 16
                or local_validation.get("manifest_sha256") != sha(MANIFEST)
                or local_validation.get("semantic_mismatches") != 0
                or local_validation.get("measurement_rows") != 2560):
            raise RuntimeError("corrected package has not passed isolated local validation")
        ...
        if (manifest.get("schema") != "crse-c12-linux-confirmation-upload-manifest/v2"
                or manifest.get("file_count") != 16 or len(manifest.get("files", [])) != 16
                ...
                or manifest.get("dependency_correction") != [
                    "cmbench/output_budget.py", "cmbench/recognition/features.py"]
                or manifest.get("isolated_local_workload_required_before_authorization") is not True
        ...
        transport.write(OUT / "TRANSPORT-FREEZE.json", {
            ...
            "source_files": 16, "source_bytes": manifest["bytes"],
