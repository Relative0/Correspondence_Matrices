...
from cmbench.comparative.gf2_support_aware_experiment import C27Config, run


def main() -> int:
    parser = argparse.ArgumentParser(description="Run C27 support-aware fresh confirmation")
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--rounds", type=int, default=5)
    parser.add_argument("--max-seconds", type=float, default=1200.0)
    args = parser.parse_args()
    result = run(
        C27Config(args.output.name, rounds=args.rounds, max_seconds=args.max_seconds),
        args.output,
    ...
    print(
        f"C27 status={result['status']} "
        f"gate={summary['support_aware_confirmation_gate']} "
        f"break_even={summary['support_aware_break_even_query_count']} "
