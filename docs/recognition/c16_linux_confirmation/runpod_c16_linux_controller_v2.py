"""Execute the frozen, explicitly authorized C16 v2 Linux confirmation."""
...
OUT = HERE / "runpod-c16-linux-v2-execute-001"
...
PROTOCOL = HERE / "C16_SECOND_MACHINE_TIMING_PACKAGE_V2_PROTOCOL_2026_08_31.md"
LOCAL_VALIDATION = HERE / "C16_PACKAGE_V2_LOCAL_VALIDATION_20260831.json"
...
def require_authorization() -> dict:
    ...
    expected = {"schema": "crse-runpod-c16-package-v2-exact-payload-authorization/v1", "authorized": True,
        ...
        "local_isolated_validation": "pass", "local_validation_pythonpath_injected": False}
...
def run() -> int:
    ...
    try:
        ...
        if (manifest.get("schema") != "crse-c16-linux-confirmation-upload-manifest/v2"
                or manifest.get("file_count") != 18 or len(manifest.get("files", [])) != 18
                or manifest.get("bytes") != authorization["source_bytes"]
                or manifest.get("authorization_status") != "exact_payload_approval_required"
...
def main():
    ...
    with base.host_awake_guard("c16-v2-http-watchdog" if args.watchdog else "c16-v2-http-controller"):
