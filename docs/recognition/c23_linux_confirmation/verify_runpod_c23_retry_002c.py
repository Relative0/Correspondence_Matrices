"""Independently verify successful retry 002c of the C23 Linux replication."""
...
RUN_DIR = HERE / "runpod-c23-linux-execute-002c"
...
OUTPUT = HERE / "RUNPOD_C23_RETRY_002C_FINAL_VERIFICATION_20260831.json"
METHODS = {
    "cm_exhaustive",
    "cm_screened",
...
def main() -> None:
    ...
    invariants = {
        "one_create_remote_complete_no_replacement": (
            ...
            and run.get("error") == "retrieved C23 evidence failed frozen gates"
        ...
        "remote_runtime_and_vendored_dependencies": (
            ...
            and any(row.get("target") == "dd/autoref.py"
                    for row in manifest.get("files", []))
        ),
        "remote_commands": (
    ...
    output = {
        "schema": "crse-runpod-c23-retry-002c-final-verification/v1",
        "status": "pass",
        "complete": True,
