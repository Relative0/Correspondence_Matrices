"""Retry the unchanged C38 payload after a reconciled local pre-create failure."""
...
OUT = HERE / "runpod-c38-linux-execute-002"
...
def build_watchdog_state(created: float, nonce: str) -> dict[str, float | str]:
    """Build the exact state envelope accepted by the inherited watchdog."""
    if not math.isfinite(created) or not re.fullmatch(r"[a-f0-9]{12}", nonce):
        raise ValueError("invalid C38 retry watchdog state input")
    return {
        # The inherited, independently exercised watchdog owns this historical
        # namespace. The C38 scientific payload and run name remain unchanged.
        "name": "cm-c7-linux-" + nonce,
...
def require_authorization() -> dict:
    ...
    if (
        ...
        or authorization.get("initial_no_create_reconciliation_sha256")
...
def run() -> int:
    ...
    try:
        ...
        created = time.time()
        state = build_watchdog_state(created, uuid.uuid4().hex[:12])
