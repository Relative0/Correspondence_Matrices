"""One explicitly authorized C7 second-machine confirmation via frozen HTTPS."""
import argparse
import base64
...
OUT = HERE / "runpod-c7-linux-confirmation-execute-002"
PRIOR_LOCAL_REFUSAL = HERE / "runpod-c7-linux-confirmation-execute-001" / "RUN.json"
...
PROPOSAL_PATH = HERE / "C7_SECOND_MACHINE_TIMING_PROTOCOL_2026_08_30.md"
AUTHORIZATION_PATH = HERE / "RUNPOD_C7_LINUX_AUTHORIZED_2026_08_30.json"
...
def require_authorization():
    ...
    expected = {
        "schema": "crse-runpod-c7-linux-confirmation-authorization/v1",
        "authorized": True,
        "one_create": True,
        "no_replacement": True,
        "source_files": 14,
...
def run():
    ...
    try:
        ...
        if (refused.get("status") != "failed" or refused.get("creation_attempted") is not False
                or refused.get("pod_created") is not False or refused.get("uploaded_source_files") != 0
                or refused.get("error") != "prior Runpod request is not reconciled"):
