...
def main():
    ...
    try:
        ...
        run('public-fetch', ['git', '-C', str(ROOT), 'fetch', '--depth', '1',
            ...
            '6f38fa521619aa0cbf91ee3c294ffca304e313ba'], 300)
        ...
        for item in hydration:
            ...
            with urllib.request.urlopen(item['url'], timeout=60) as response:
                data = response.read(item['upstream_bytes'] + 1)
            assert len(data) == item['upstream_bytes'] and hashlib.sha256(data).hexdigest() == item['upstream_sha256'], item['path']+' upstream identity'
            if item['restore_crlf']:
                assert b'\r' not in data
                data = data.replace(b'\n', b'\r\n')
            assert len(data) == item['bytes'] and hashlib.sha256(data).hexdigest() == item['sha256'], item['path']+' historical identity'
            target = ROOT/path
            target.parent.mkdir(parents=True, exist_ok=True)
