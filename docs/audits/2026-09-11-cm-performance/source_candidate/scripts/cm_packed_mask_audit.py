...
def session(case, backend, queries, stages=None, warm=False):
    ...
    elif backend in ("cse", "cse_flat"):
        step("compile", lambda: bs.get_expr_cse_program(expr, flatten=backend == "cse_flat"))
    ...
    def execute():
        ...
        for fixed in queries:
            ...
            if backend == "direct":
                ...
                value = step("execute_bind", lambda: bs.eval_expr_flat_cse(
                    expr, live, fixed=fixed, flatten=backend == "cse_flat"))
...
def campaign(output, cohort, strong_only=False):
    fixtures = json.loads((output / "fixtures.json").read_text())[cohort]
    rows = []
    backends = ("cse_flat",) if strong_only else (
        "direct", "cse", "cse_flat", "cm_flat", "cm_words", "cm_public", "dense")
    label = cohort + ("_strong" if strong_only else "")
    ...
    raw_path = output / f"{label}_raw.jsonl"
    ...
    write_json(output / f"{label}_summary.json", summaries)
...
def main():
    ...
    parser.add_argument("mode", choices=("freeze", "profile", "environment", "development", "development-strong", "confirmation"))
    ...
    elif args.mode == "development-strong": campaign(args.output, "development", strong_only=True)
