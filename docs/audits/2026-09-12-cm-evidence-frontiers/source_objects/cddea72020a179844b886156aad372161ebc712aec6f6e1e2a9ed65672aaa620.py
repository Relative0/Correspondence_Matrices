...
def main():
    ...
    parser.add_argument('--kind',choices=('sxfm','concrete'),default='sxfm')
    parser.add_argument('--output',type=Path,required=True);args=parser.parse_args()
    cases=json.loads((AUDIT/('SXFM-MITERS.json' if args.kind=='sxfm' else 'CONCRETE-MITERS.json')).read_text())
    if args.case:
            ...
            with destination.with_suffix('.log').open('xb') as stream:
                try:
                    result=subprocess.run([sys.executable,'-B','-m','scripts.cm_frontier_mapping_verify',
                        '--case',case['id'],'--solver',solver,'--kind',args.kind,'--output',str(destination)],
