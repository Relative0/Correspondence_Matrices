...
# The activation API validates the exact supplied binary hash. Use the native
# source build on Unix; the retained Windows DLL cannot execute there.
TEST_LIBRARY = FROZEN_LIBRARY if sys.platform == "win32" else (
    ROOT / "build/cm_fused_slots" /
    ("libcm_fused_slots.dylib" if sys.platform == "darwin" else "libcm_fused_slots.so")
)
...
@pytest.mark.skipif(not TEST_LIBRARY.is_file(), reason="platform-native test library unavailable; run scripts/build_cm_fused_slots.py")
def test_confirmed_library_activates_only_with_matching_sha256() -> None:
    expected = hashlib.sha256(TEST_LIBRARY.read_bytes()).hexdigest()
    config = NativeRestrictionConfig.from_environment({
        FEATURE_ENV: "true",
        LIBRARY_ENV: str(TEST_LIBRARY),
        SHA256_ENV: expected,
    })
    engine = ExactRestrictionEngine(expr_to_json_dag(expression()), 4, config)
...
def test_confirmed_multi_root_union_activates_and_preserves_order() -> None:
    ...
    config = NativeRestrictionConfig(
        True,
        TEST_LIBRARY,
        hashlib.sha256(TEST_LIBRARY.read_bytes()).hexdigest(),
    )
    engine = ExactMultiRootRestrictionEngine(
        expressions_to_multi_root_dag(roots),
        tuple(expr_to_json_dag(root) for root in roots),
