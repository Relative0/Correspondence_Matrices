"""Read-only preflight for one proposed zero-volume corpus/RSS CPU study."""
...
import http_structural_preflight_v3 as previous_preflight

RATE_CAP = 0.25
STORAGE_RESERVE_PER_HOUR = 0.01
PRIOR_HTTP_RESERVE = 0.03
PHASE_CAP = 0.10
CAMPAIGN_CAP = 0.20
PRIOR_POD_IDS = {"eidn8uu97y3b6q", "s2dpiij1msutml", "8voqzr4b1a4qti"}
SUCCESSFUL_RUN = HERE / "http-structural-execute-001"
SUCCESSFUL_FINAL = HERE / "HTTP-STRUCTURAL-FINAL-VERIFICATION-20260829-051855-153785.json"
...
def prior_attempts():
    previous = previous_preflight.prior_attempts()
    run = load(SUCCESSFUL_RUN / "RUN.json")
    final = load(SUCCESSFUL_FINAL)
    frozen = load(SUCCESSFUL_RUN / "TRANSPORT-FREEZE.json")
    if (previous.get("pod_ids") != ["eidn8uu97y3b6q", "s2dpiij1msutml"]
        or previous.get("cleanup_verified") is not True
        or run.get("status") != "complete" or run.get("pod_id") != "8voqzr4b1a4qti"
        ...
        or final.get("complete") is not True or final.get("pod_id") != "8voqzr4b1a4qti"
        ...
        or final.get("evidence", {}).get("raw_rows") != 1560
        or final.get("evidence", {}).get("calls") != 360
    ...
    for version in ("v1", "v2"):
        ...
        if check.get("inventory") or check.get("details_http_status") != {
                "8voqzr4b1a4qti": 404, "eidn8uu97y3b6q": 404, "s2dpiij1msutml": 404}:
    ...
    for name, field in (("runpod_structural_controller_v4.py", "controller_sha256"),
                        ("http_transport_bootstrap.py", "bootstrap_sha256"),
                        ("http_structural_preflight_v3.py", "preflight_sha256")):
...
if __name__ == "__main__":
    ...
    path = HERE / ("HTTP-CORPUS-PREFLIGHT-" + datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S-%f") + ".json")
