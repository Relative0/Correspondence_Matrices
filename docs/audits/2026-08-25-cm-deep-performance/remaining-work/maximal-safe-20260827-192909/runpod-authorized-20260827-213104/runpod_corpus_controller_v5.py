"""One explicitly authorized corpus/oracle/RSS study via frozen HTTPS."""
import argparse
import base64
...
OUT = HERE / "http-corpus-execute-001"
spec = importlib.util.spec_from_file_location("preserved_cpu_smoke", HERE / "runpod_retry_cpu8_v1_controller.py")
...
# Derive the proposed corpus command from the preserved remote program. Each
...
base.REMOTE_CODE = replace_remote_once(
    ...
    "summary = json.loads((OUT / 'corpus-memory/summary.json').read_text())",
)
base.REMOTE_CODE = replace_remote_once(
    base.REMOTE_CODE,
    "validation['memory_summary']",
    "validation['corpus_summary']",
)
base.REMOTE_CODE = replace_remote_once(
    base.REMOTE_CODE,
    ...
    "run('focused-tests', [sys.executable, '-m', 'pytest', '-q', 'tests/test_output_budget.py',\n"
    "                          'tests/test_cm_corpus_memory_validation.py', '-p', 'no:cacheprovider',\n"
    "                          '--basetemp', str(OUT/'pytest-temp'),\n"
    "                          '--junitxml', str(OUT/'focused.xml')], 180)",
)
base.REMOTE_CODE = replace_remote_once(
    base.REMOTE_CODE,
    ...
    "emit('stage', name='corpus-study')\n"
    "    run('corpus-study', [sys.executable, 'scripts/cm_corpus_memory_validation.py',\n"
    "                         '--execution', 'runpod',\n"
    "                         '--selection-manifest', 'study/CORPUS-MEMORY-SELECTION.json',\n"
    "                         '--output-dir', str(OUT/'corpus-memory')], 600)",
)
base.OUT = OUT
BOOTSTRAP_PATH = HERE / "http_transport_bootstrap.py"
MANIFEST_PATH = HERE.parent / "continuation-20260829-125214" / "RUNPOD-CORPUS-UPLOAD-MANIFEST-V2.json"
PROPOSAL_PATH = HERE.parent / "continuation-20260829-125214" / "RUNPOD-CORPUS-MEMORY-VALIDATION-PROPOSAL-20260829.md"
AUTHORIZATION_PATH = HERE / "HTTP-CORPUS-AUTHORIZED-20260829.json"
...
def require_authorization():
    if not AUTHORIZATION_PATH.is_file():
        raise RuntimeError("exact corpus-study authorization record is absent")
    authorization = load(AUTHORIZATION_PATH)
    expected = {
        "schema": "cm-runpod-corpus-authorization/v1",
        "authorized": True,
        "one_create": True,
        "no_replacement": True,
        "source_files": 71,
    ...
    if any(authorization.get(key) != value for key, value in expected.items()):
        raise RuntimeError("corpus-study authorization scope mismatch")
...
def watchdog():
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(r"cm-corpus-study-[a-f0-9]{12}", state["name"])
...
def execute_remote(pod_id, token, raw, created, record):
    ...
    with requests.Session() as proxy:
        ...
        while time.time() < created + CLEANUP_AT - 30:
            ...
            if time.time() >= created + 300 and progress.get("stage") not in ("focused-tests", "corpus-study"):
...
def save_evidence(log):
    ...
    corpus = evidence_root / "corpus-memory"
    summary = load(corpus / "summary.json")
    selection = load(corpus / "selection-manifest.json")
    oracles = load(corpus / "oracles.json")
    rows = [json.loads(line) for line in (corpus / "raw.jsonl").read_text(encoding="utf-8").splitlines()]
    rss_jobs = [json.loads(line) for line in (corpus / "rss-jobs.jsonl").read_text(encoding="utf-8").splitlines()]
    calls = {(row["case_id"], row["representation"], row["schedule"], row["repetition"])
             for row in rows}
    if (validation.get("status") != "complete" or validation.get("junit") != {"tests": 79, "failures": 0, "errors": 0, "skipped": 0}
        or validation.get("corpus_summary", {}).get("rows") != 630
        or validation.get("corpus_summary", {}).get("source_unchanged") is not True
        or summary.get("rows") != 630 or summary.get("statuses") != {"ok": 630}
        or summary.get("source_unchanged") is not True or summary.get("production_estimator_accepted") is not False
        or summary.get("calibration_performed") is not False
        or summary.get("real_workload_compatibility") != "not measured; BX1/B2/EPFL are frozen benchmark corpora"
        or summary.get("rss", {}).get("jobs_with_samples") != 420
        or summary.get("rss", {}).get("jobs_with_kernel_hwm") != 420
        or selection.get("schema") != "cm-corpus-memory-selection/v1"
        or len(selection.get("cases", [])) != 35 or len(oracles) != 35
        or not any(case.get("syntactic_k") > case.get("k") for case in selection.get("cases", []))
        or len(rss_jobs) != 420 or len(rows) != 630 or len(calls) != 630
        or any(row.get("status") != "ok" or row.get("exact") is not True for row in rows)
        or {row.get("corpus") for row in rows} != {"bx1", "b2", "epfl"}
        or {row.get("role") for row in rows} != {"calibration-corpus", "heldout-corpus"}
        or {row.get("schedule") for row in rows} != {"cold", "warm"}
        or {row.get("representation") for row in rows} != {"dense", "bigint", "words"}
        or any(row.get("output_sha256") != row.get("independent_oracle_sha256") for row in rows)
        or any(item.get("timed_out") or item.get("returncode") != 0 for item in rss_jobs)
        or runtime.get("source_files") != 71 or runtime.get("runpod_pod_id") != load(IDENTITY)["pod_id"]):
        raise RuntimeError("retrieved evidence did not satisfy frozen corpus-study gates")
    result["validation"] = validation
    result["runtime_pod_id"] = runtime["runpod_pod_id"]
    result["corpus_grid"] = {"cases": len(oracles), "jobs": len(rss_jobs), "calls": len(calls),
                              "rows": len(rows), "all_exact": True,
                              "rss_jobs_sampled": summary["rss"]["jobs_with_samples"],
...
def run():
    ...
    try:
        ...
        prior = load(HERE / "GPU-FINAL-OUTCOME.json")
        ...
        if (len(manifest["files"]) != 71 or manifest.get("bytes") != 1680864
                or manifest.get("authorization_status") != "pending"
                or len(lock["packages"]) != 13 or lock["source_builds_allowed"]):
            raise RuntimeError("frozen corpus-study package mismatch")
        ...
        created = time.time()
        state = {"name": "cm-corpus-study-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        write(OUT / "TRANSPORT-FREEZE.json", {
            ...
            "source_files": 71, "source_bytes": sum(row["bytes"] for row in manifest["files"]),
