...
import runpod_http_smoke_controller as controller


def main():
    c = controller
    original_out = c.OUT
    fixture = c.HERE / ("http-fake-checks-" + uuid.uuid4().hex[:8])
    fixture.mkdir()
    results = []
    ...
    for filename in ("http_transport_bootstrap.py", "http_transport_preflight.py", "runpod_http_smoke_controller.py"):
    ...
    for case in (*success_cases, "http400", "http500", "wrong-image", "wrong-ports", "wrong-volume", "overprice", "nan-price", "wrong-cpu", "low-ram", "upload-timeout", "delete-failure", "preflight-failed", "conflicting-image", "conflicting-cloud", "unknown-cloud", "gpu-present", "wrong-compute-type"):
        ...
        class Client:
            ...
            def post(self, url, **kwargs):
                ...
                assert body["ports"] == c.EXPECTED_PORTS and body["volumeInGb"] == 10
            ...
            def get(self, url, **kwargs):
                ...
                pod = {"id": "fakepod0123", "name": captures["name"], "computeType": "CPU", "cloudType": "SECURE",
                       ...
                       "imageName": c.base.IMAGE, "containerDiskInGb": 12, "volumeInGb": 10,
    ...
    c.write(c.HERE / ("HTTP-TRANSPORT-CHECKS-" + uuid.uuid4().hex[:8] + ".json"), result)
