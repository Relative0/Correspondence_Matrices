...
RUN_LABEL = os.environ.get("CM_SMOKE_RUN_LABEL", "gpu-execute-001")
if not RUN_LABEL or any(char not in "abcdefghijklmnopqrstuvwxyz0123456789-" for char in RUN_LABEL):
    raise RuntimeError("unsafe controller run label")
OUT = Path(__file__).resolve().parent / RUN_LABEL
...
@contextmanager
def host_awake_guard(role):
    """Prevent idle sleep only during this controller process; never override lid sleep."""
    if os.name != "nt":
        raise RuntimeError("this host wake guard is validated for Windows only")
    import ctypes
    ...
    power = PowerStatus()
    if not kernel.GetSystemPowerStatus(ctypes.byref(power)) or power.ACLineStatus != 1:
        raise RuntimeError("plug the controller host into AC power before launching a pod")
    set_state = kernel.SetThreadExecutionState
    set_state.argtypes = [wintypes.DWORD]
    set_state.restype = wintypes.DWORD
    requested = 0x80000001  # ES_CONTINUOUS | ES_SYSTEM_REQUIRED; no away/display mode.
    if not set_state(requested):
        raise RuntimeError("failed to establish temporary idle-sleep prevention")
    try:
        write_exclusive(OUT / ("HOST-AWAKE-" + role + ".json"), {
            "started_utc": utc_now(), "pid": os.getpid(), "ac_power_connected": True,
            "requested_execution_state": requested, "persistent_power_settings_changed": False,
            "limitation": "does not prevent lid-close, explicit sleep, power loss, or network loss"})
        yield
    finally:
        released = bool(set_state(0x80000000))
        write_exclusive(OUT / ("HOST-AWAKE-RELEASED-" + role + ".json"), {
            "released_utc": utc_now(), "released": released, "pid": os.getpid()})
...
def inventory_both(client):
    result = {}
    for version, endpoint in (("v1", REST_V1), ("v2", REST_V2)):
        response = client.get(endpoint + "/pods", timeout=5, allow_redirects=False)
        response.raise_for_status()
        value = response.json()
        pods = value if isinstance(value, list) else value.get("pods")
        if not isinstance(pods, list):
            raise RuntimeError("unexpected pod inventory schema")
        result[version] = [{key: pod.get(key) for key in
                            ("id", "name", "desiredStatus", "status")} for pod in pods]
    return result


def account_preflight(client):
    prior = json.loads((Path(__file__).resolve().parent / "CPU8-V1-FINAL-RECONCILIATION.json").read_text())
    if (prior.get("status") != "owned_pod_absent_after_horizon"
            or not prior["snapshot"]["after_original_horizon"]
            or not prior["snapshot"]["all_inventory_reads_succeeded"]
            or not prior["snapshot"]["zero_pods_observed"]):
        raise RuntimeError("prior uncertain CPU request is not reconciled")
    inventories = inventory_both(client)
    if any(inventories.values()):
        raise RuntimeError("both-API zero-pod preflight failed")
    response = client.get(REST_V2 + "/billing/pods",
                          params={"startTime": "2026-08-27T14:30:00Z", "endTime": utc_now()},
                          timeout=20, allow_redirects=False)
    response.raise_for_status()
    metadata = response.json()["metadata"]
    previous_cost = float(metadata["totals"]["totalAmount"])
    # No allocation was observed in earlier smoke attempts. Any charge needs attribution first.
    billing_clear = previous_cost == 0 and metadata["recordCount"] == 0
    ...
    if account.get("errors"):
        raise RuntimeError("credit preflight query failed")
    account = account["data"]["myself"]
    credit_ok = float(account["clientBalance"]) >= RATE_CAP + 0.002
    spend = float(account["currentSpendPerHr"])
    limit = account.get("spendLimit")
    spend_ok = limit is not None and float(limit) >= spend + RATE_CAP + 0.002
    result = {"checked_utc": utc_now(), "inventories": inventories,
              "prior_request_reconciled": True, "billing_metadata": metadata,
              "billing_may_lag": True, "previous_observed_smoke_cost_usd": previous_cost,
              "billing_clear": billing_clear, "credit_sufficient": credit_ok,
              "spending_limit_sufficient": spend_ok, "resource_writes": 0}
    write_exclusive(OUT / "ACCOUNT-PREFLIGHT.json", result)
    if not billing_clear or not credit_ok or not spend_ok:
        raise RuntimeError("billing/credit preflight did not clear")
    return previous_cost


def catalog_offer(client):
    endpoint = REST_V2 + "/catalog/gpus/" + quote(GPU_ID, safe="")
    response = client.get(endpoint, params={"include": "AVAILABILITY", "product": "POD",
                          "cloud": "SECURE", "count": GPU_COUNT}, timeout=20, allow_redirects=False)
    response.raise_for_status()
    offer = response.json()
    write_exclusive(OUT / "LIVE-OFFER.json", {"checked_utc": utc_now(), "endpoint": endpoint,
                                             "product": "POD", "gpu_count": GPU_COUNT, "gpu": offer})
    if (offer.get("id") != GPU_ID or offer.get("secure") is not True
            or offer.get("availability") not in ("LOW", "MEDIUM", "HIGH")):
        raise RuntimeError("approved Secure GPU type is unavailable")
    rate = float(offer["price"]["secure"]) * GPU_COUNT
    if not 0 < rate <= RATE_CAP:
        raise RuntimeError("live GPU offer exceeds approved rate")
    return offer, rate
...
def terminate(client, pod_id):
    ...
    for url in (REST_V2 + "/pods/" + pod_id, REST_V1 + "/pods/" + pod_id):
        ...
        for _ in range(2):
            try:
                inventories = inventory_both(client)
                if all(row.get("id") != pod_id for rows in inventories.values() for row in rows):
...
def collect_logs(client, pod_id, deadline, setup_deadline, lines=None):
    ...
    while time.time() < deadline:
        if time.time() >= setup_deadline and not any('"name": "focused-tests"' in line for line in lines):
            raise RuntimeError("boot/install exceeded the five-minute setup deadline")
        try:
...
def watchdog():
    try:
        client = session()
        startup_pods = safe_pods(client)
    except Exception as exc:
        write_exclusive(OUT / "WATCHDOG-STARTUP-FAILURE.json", {
            "checked_utc": utc_now(), "error_type": type(exc).__name__,
            "network_probe_passed": False, "creation_permitted": False})
        return 1
    ready = {"ready_utc": utc_now(), "pid": os.getpid(), "deadline_epoch": time.time() + HARD_LIFETIME_S,
             "network_probe_passed": True, "startup_pod_count": len(startup_pods)}
    ...
    result = {"activated_utc": utc_now(), "pod_id_known": bool(pod_id), "terminated": False,
              "owned_pod_absent": False, "recovery_errors": []}
    if not pod_id and STATE.exists():
        name = json.loads(STATE.read_text())["name"]
        for attempt in range(3):
            try:
                inventory = safe_pods(client)
                matching = [row for row in inventory if row.get("name") == name]
                if len(matching) == 1:
                    pod_id = matching[0]["id"]
                elif not matching:
                    result["owned_pod_absent"] = True
                    result["account_pod_count"] = len(inventory)
                else:
                    result["recovery_errors"].append("multiple matching pod names; manual review required")
                break
            except Exception as exc:
                result["recovery_errors"].append(type(exc).__name__)
                if attempt < 2:
                    time.sleep(2)
    if pod_id:
        result["terminated"], result["termination_attempts"] = terminate(client, pod_id)
    write_exclusive(OUT / "WATCHDOG-RESULT.json", result)
    return int(not (result["terminated"] or result["owned_pod_absent"]))
...
def run():
    ...
    for _ in range(200):
        if WATCHDOG_READY.exists():
            break
        if (OUT / "WATCHDOG-STARTUP-FAILURE.json").exists():
            raise RuntimeError("watchdog network probe failed; no pod creation permitted")
        time.sleep(0.2)
    ...
    try:
        ...
        prior_cost = account_preflight(client)
        offer, quoted_rate = catalog_offer(client)
        ...
        if quoted_rate > RATE_CAP or projected + prior_cost > TOTAL_CAP:
        ...
        payload = {"name": name, "cloud": "SECURE", "image": IMAGE,
                   ...
                   "gpu": {"id": GPU_ID, "count": GPU_COUNT},
                   "ports": [], "env": environment,
                   "args": "python -u -c " + shlex.quote(bootstrap)}
        record.update({"creation_request_utc": utc_now(), "creation_endpoint": REST_V2 + "/pods",
                       "approved_gpu": GPU_ID, "gpu_count": GPU_COUNT,
                       "minimum_vcpu": MIN_VCPU, "minimum_ram_gb": MIN_RAM_GB,
                       "previous_observed_smoke_cost_usd": prior_cost,
                       "quoted_rate_usd_per_hour": quoted_rate,
                       "source_bundle_bytes": len(bundle), "source_bundle_sha256": hashlib.sha256(bundle).hexdigest(),
                       "source_file_count": len(manifest["files"]), "container_args_bytes": len(payload["args"].encode()),
                       "environment_value_bytes": sum(len(value.encode()) for value in environment.values()),
                       "largest_environment_value_bytes": max(len(value.encode()) for value in environment.values()),
                       "estimated_json_payload_bytes": len(json.dumps(payload).encode())})
        ...
        record["creation_response_headers"] = {key: response.headers[key][:400] for key in
                                               ("Date", "X-Request-Id", "X-Correlation-Id", "CF-Ray")
        ...
        record.update({"pod_created": True, "pod_id": pod_id})
        created_pod = pod
        ...
        gpu = created_pod.get("gpu") or {}
        record.update({"status": "running", "cloud": created_pod.get("cloud"),
                       "gpu_id": gpu.get("id"), "gpu_count": gpu.get("count"),
                       "vcpu": pod.get("vcpuCount"), "ram_gb": pod.get("memoryInGb"),
                       "container_disk_gb": pod.get("containerDiskInGb"), "volume_gb": pod.get("volumeInGb"),
                       "mounts": created_mounts, "ports": created_pod.get("ports"),
                       "v1_ports": pod.get("ports"), "v1_gpu": pod.get("gpu"),
                       "machine_gpu_id": (pod.get("machine") or {}).get("gpuTypeId"),
                       "machine_secure": (pod.get("machine") or {}).get("secureCloud"),
                       "data_center": created_pod.get("dataCenterId"),
                       "global_networking": created_pod.get("globalNetworking"),
                       "actual_rate_usd_per_hour": actual_rate, "v2_cost": created_pod.get("cost"),
                       "projected_20_min_cost_usd": (actual_rate+0.002)/3,
                       "storage_rate_reserve_usd_per_hour": 0.002,
                       "offer_availability": offer.get("availability")})
        if (not 0 < actual_rate <= RATE_CAP or record["projected_20_min_cost_usd"] + prior_cost > TOTAL_CAP
                or record["gpu_id"] != GPU_ID or record["gpu_count"] != GPU_COUNT or record["cloud"] != "SECURE"
                or not 0 < float(record["container_disk_gb"]) <= CONTAINER_DISK_GB
                or record["volume_gb"] not in (None, 0) or record["mounts"] or pod.get("networkVolume")
                or record["ports"] != [] or record["v1_ports"] not in (None, [])
                or (record["global_networking"] or {}).get("enabled", False)
                or not MIN_VCPU <= float(record["vcpu"]) < float("inf")
                or not MIN_RAM_GB <= float(record["ram_gb"]) < float("inf")
                or record["machine_gpu_id"] not in (None, GPU_ID) or record["machine_secure"] is False
                or (record["v2_cost"] is not None and not 0 < float(record["v2_cost"]) <= RATE_CAP)
                or created_pod.get("image") != IMAGE):
        ...
        record["estimated_compute_cost_usd"] = (round(float(record["actual_rate_usd_per_hour"]) * record["lifetime_s"] / 3600, 6)
                                                if "actual_rate_usd_per_hour" in record else None)
    ...
    return int(record["status"] != "complete" or not record.get("terminated") or bool(record.get("postflight_pods")))
...
def main():
    ...
    with host_awake_guard("watchdog" if args.watchdog else "controller"):
