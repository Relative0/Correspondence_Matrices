...
def run():
    ...
    try:
        ...
        payload = {"name": name, "computeType": "CPU", "cloudType": "SECURE", "imageName": IMAGE,
                   ...
                   "ports": [], "env": environment, "dockerEntrypoint": ["python", "-u", "-c"],
                   "dockerStartCmd": [REMOTE_CODE], "interruptible": False, "locked": False,
        ...
        if response.status_code not in (200, 201):
            title = None
            try:
                title = response.json().get("title") or response.json().get("detail")
            except ValueError:
                pass
        ...
        record.update({"pod_created": True, "pod_id": pod_id, "status": "running",
                       ...
                       "volume_gb": pod.get("volumeInGb"), "quoted_rate_usd_per_hour": quoted_rate,
        ...
        if (actual_rate > RATE_CAP or record["projected_20_min_cost_usd"] > TOTAL_CAP
                or record["container_disk_gb"] is None or record["container_disk_gb"] > CONTAINER_DISK_GB or record["volume_gb"] != 0
