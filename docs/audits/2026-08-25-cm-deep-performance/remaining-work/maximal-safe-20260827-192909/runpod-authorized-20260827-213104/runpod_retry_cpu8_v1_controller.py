...
RUN_LABEL = os.environ.get("CM_SMOKE_RUN_LABEL", "cpu8-v1-execute-001")
...
def catalog_offer(client):
    endpoint = REST_V2 + "/catalog/cpus/" + CPU_ID
    response = client.get(endpoint, params={"include": "AVAILABILITY",
    ...
    write_exclusive(OUT / "LIVE-OFFER.json", {"checked_utc": utc_now(), "endpoint": endpoint,
                                             "product": "POD", "vcpu_count": VCPU, "cpu": payload})
    offer = payload
    if offer.get("id") != CPU_ID or offer.get("availability") not in ("LOW", "MEDIUM", "HIGH"):
    ...
    if (not 0 < rate <= RATE_CAP or not RAM_GB <= ram < float("inf")
            or not offer["vcpu"]["min"] <= VCPU <= offer["vcpu"]["max"]):
...
def terminate(client, pod_id):
    attempts = []
    for url in (REST_V1 + "/pods/" + pod_id, REST_V2 + "/pods/" + pod_id):
        try:
...
def run():
    ...
    try:
        ...
        # Runpod's own MCP client routes CPU creation to REST v1 (AE-2991).
        # Preserve the approved image, workload, resources, and zero-service settings.
        payload = {"name": name, "computeType": "CPU", "cloudType": "SECURE", "imageName": IMAGE,
                   "containerDiskInGb": CONTAINER_DISK_GB, "volumeInGb": 0,
                   "cpuFlavorIds": [CPU_ID], "cpuFlavorPriority": "custom", "vcpuCount": VCPU,
                   "ports": [], "env": environment,
                   "dockerEntrypoint": ["python", "-u", "-c"], "dockerStartCmd": [bootstrap],
                   "interruptible": False, "locked": False}
        record.update({"creation_request_utc": utc_now(), "creation_endpoint": REST_V1 + "/pods", "quoted_vcpu": VCPU,
                       "quoted_ram_gb": quoted_ram, "quoted_rate_usd_per_hour": quoted_rate,
                       "source_bundle_bytes": len(bundle), "source_bundle_sha256": hashlib.sha256(bundle).hexdigest(),
                       "source_file_count": len(manifest["files"]), "container_args_bytes": sum(len(arg.encode()) for arg in payload["dockerEntrypoint"] + payload["dockerStartCmd"]),
        ...
        response = client.post(REST_V1 + "/pods", json=payload, timeout=120, allow_redirects=False)
