"""One authorized CPU smoke via a frozen authenticated HTTPS payload."""
import argparse
import base64
...
def watchdog():
    ...
    write(READY, {"ready_utc": preflight.utc_now(), "ready_epoch": time.time(), "pid": os.getpid(),
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(r"cm-http-smoke-[a-f0-9]{12}", state["name"])
...
def confirm_watchdog(proc, state):
    for _ in range(50):
        ...
        if STATE_ACK.exists():
            ack = load(STATE_ACK)
            if ack.get("state") != state or ack.get("pid") != proc.pid:
...
def create_payload(name, offer, token, raw, created):
    ...
    return {"name": name, "computeType": "CPU", "cloudType": "SECURE", "imageName": base.IMAGE,
            ...
            "containerDiskInGb": 12, "volumeInGb": 10, "volumeMountPath": "/workspace",
...
def validate_pod(pod, state, offer, prior_cost):
    ...
    if (pod.get("id") != load(IDENTITY)["pod_id"] or pod.get("name") != state["name"]
        ...
        or pod.get("volumeInGb") != 10 or pod.get("volumeMountPath") != "/workspace"
        ...
        or not math.isfinite(prior_cost) or prior_cost < 0
        or projected > ADDITIONAL_CAP or projected + prior_cost > CAMPAIGN_CAP):
    ...
    return {"rate_usd_per_hour": rate, "ram_gb": ram, "projected_20_min_cost_usd": projected,
            ...
            "container_disk_gb": 12, "pod_volume_gb": 10, "ports": EXPECTED_PORTS,
...
def save_evidence(log):
    ...
    validation = load(OUT / "evidence/run-output/REMOTE-VALIDATION.json")
    runtime = load(OUT / "evidence/run-output/RUNTIME.json")
    if (validation.get("status") != "complete" or validation.get("junit") != {"tests": 70, "failures": 0, "errors": 0, "skipped": 0}
        or validation.get("memory_summary", {}).get("rows") != 312
        ...
        raise RuntimeError("retrieved evidence did not satisfy frozen smoke gates")
...
def run():
    ...
    try:
        ...
        if len(manifest["files"]) != 65 or len(lock["packages"]) != 13 or lock["source_builds_allowed"]:
            raise RuntimeError("frozen smoke package mismatch")
        ...
        created = time.time()
        state = {"name": "cm-http-smoke-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        record["actual_resources"] = validate_pod(pod, state, offer, ready["observed_previous_cost_usd"])
