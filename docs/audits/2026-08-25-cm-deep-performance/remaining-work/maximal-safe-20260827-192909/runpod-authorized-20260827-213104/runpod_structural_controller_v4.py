"""One authorized structural validation study via a frozen HTTPS payload."""
import argparse
import base64
...
OUT = HERE / "http-structural-execute-001"
...
# Derive the authorized structural command from the preserved, previously
# executed remote program. Each mutation is exact and must occur once.
base.REMOTE_CODE = replace_remote_once(
    base.REMOTE_CODE,
    "summary = json.loads((OUT / 'memory/summary.json').read_text())",
    "summary = json.loads((OUT / 'structural/summary.json').read_text())",
)
base.REMOTE_CODE = replace_remote_once(
    base.REMOTE_CODE,
    "'--execution', 'runpod', '--supports', '6', '8', '--families',\n"
    "                         'mixed-chain', 'alternating-tree', '--contexts', 'none',",
    "'--execution', 'runpod', '--supports', '6', '8', '12', '16', '--families',\n"
    "                         'mixed-chain', 'shared-diamond', 'wide-and', 'alternating-tree',\n"
    "                         'reconvergent-xor', '--contexts', 'none',",
)
base.REMOTE_CODE = replace_remote_once(
    base.REMOTE_CODE,
    "'--output-dir', str(OUT/'memory')], 300)",
    "'--output-dir', str(OUT/'structural')], 600)",
)
base.OUT = OUT
...
def watchdog():
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(r"cm-struct-study-[a-f0-9]{12}", state["name"])
...
def execute_remote(pod_id, token, raw, created, record):
    ...
    with requests.Session() as proxy:
        ...
        record["uploaded_source_files"] = 65
        ...
        while time.time() < created + CLEANUP_AT - 30:
            ...
            if time.time() >= created + 300 and progress.get("stage") not in ("focused-tests", "memory-study"):
...
def save_evidence(log):
    ...
    structural = evidence_root / "structural"
    summary = load(structural / "summary.json")
    jobs = load(structural / "jobs.json")
    rows = [json.loads(line) for line in (structural / "raw.jsonl").read_text(encoding="utf-8").splitlines()]
    calls = {(row["case_id"], row["representation"], row["schedule"], row["repetition"])
             for row in rows}
    if (validation.get("status") != "complete" or validation.get("junit") != {"tests": 70, "failures": 0, "errors": 0, "skipped": 0}
        or validation.get("memory_summary", {}).get("rows") != 1560
        or validation.get("memory_summary", {}).get("source_unchanged") is not True
        or summary.get("rows") != 1560 or summary.get("statuses") != {"ok": 1560}
        or summary.get("source_unchanged") is not True or summary.get("production_estimator_accepted") is not False
        or len(jobs) != 240 or len(rows) != 1560 or len(calls) != 360
        or any(row.get("status") != "ok" or row.get("exact") is not True for row in rows)
        or {row.get("family") for row in rows} != {"mixed-chain", "shared-diamond", "wide-and", "alternating-tree", "reconvergent-xor"}
        or {row.get("k") for row in rows} != {6, 8, 12, 16}
        or {row.get("context") for row in rows} != {"none"}
        or {row.get("schedule") for row in rows} != {"cold", "warm"}
        or runtime.get("source_files") != 65 or runtime.get("runpod_pod_id") != load(IDENTITY)["pod_id"]):
        raise RuntimeError("retrieved evidence did not satisfy frozen structural-study gates")
    result["validation"] = validation
    result["runtime_pod_id"] = runtime["runpod_pod_id"]
    result["structural_grid"] = {"jobs": len(jobs), "calls": len(calls), "rows": len(rows),
                                 "all_exact": True, "statuses": summary["statuses"]}
    return result
...
def run():
    ...
    try:
        ...
        if len(manifest["files"]) != 65 or len(lock["packages"]) != 13 or lock["source_builds_allowed"]:
            raise RuntimeError("frozen structural-study package mismatch")
        ...
        created = time.time()
        state = {"name": "cm-struct-study-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        write(OUT / "TRANSPORT-FREEZE.json", {
            ...
            "source_files": 65, "source_bytes": sum(row["bytes"] for row in manifest["files"]),
            "transport_payload_sha256": hashlib.sha256(raw).hexdigest(), "transport_payload_bytes": len(raw),
            "manifest_sha256": hashlib.sha256(base.MANIFEST_PATH.read_bytes()).hexdigest(),
