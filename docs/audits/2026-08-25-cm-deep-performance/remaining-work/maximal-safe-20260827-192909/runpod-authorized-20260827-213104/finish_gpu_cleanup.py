...
def main():
    controller = load("cleanup_v1_controller", "runpod_gpu_smoke_controller.py")
    reconcile = load("cleanup_inventory", "reconcile_gpu.py")
    ...
    guard = ROOT / "gpu-final-cleanup-guard"
    ...
    with controller.host_awake_guard("reconciliation"):
        while True:
            ...
            if time.time() >= horizon:
                ...
                controller.write_exclusive(ROOT / "GPU-FINAL-RECONCILIATION.json", result)
