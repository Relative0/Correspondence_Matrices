"""One-create W8 semantic retry after a transient zero-availability refusal."""
import argparse
import base64
...
REMOTE_CODE_PATH = HERE / "runpod_w8_logikbench_semantic_remote_v1.py"
base.REMOTE_CODE = REMOTE_CODE_PATH.read_text(encoding="utf-8")
base.OUT = OUT
BOOTSTRAP_PATH = HERE / "http_native_scout_bootstrap_v2.py"
MANIFEST_PATH = HERE / "RUNPOD-W8-LOGIKBENCH-SEMANTIC-UPLOAD-MANIFEST-V1-20260830.json"
BUNDLE_PATH = HERE / "RUNPOD-W8-LOGIKBENCH-SEMANTIC-UPLOAD-BUNDLE-V1-20260830.zip"
PROPOSAL_PATH = HERE / "RUNPOD-W8-LOGIKBENCH-SEMANTIC-SCOUT-PROPOSAL-20260830.md"
AUTHORIZATION_PATH = HERE / "HTTP-W8-LOGIKBENCH-SEMANTIC-V3-AUTHORIZED-20260831.json"
...
def require_authorization():
    if not AUTHORIZATION_PATH.is_file():
        raise RuntimeError("exact W8 semantic-scout authorization record is absent")
    authorization = load(AUTHORIZATION_PATH)
    expected = {
        "schema": "cm-runpod-w8-logikbench-semantic-authorization/v1",
        "authorized": True,
        "one_create": True,
        "no_replacement_within_this_controller": True,
        "external_source_upload_approved": True,
        "source_files": 82,
        "source_bytes": 15054954,
        "source_bundle_bytes": 1540008,
        ...
        "semantic_root_oracle_only": True,
        "upstream_commit": "891ced851ea4c2f9a46f6ab991eeee199e2fd516",
        "prior_w8_conversion_audit_verified": True,
        "prior_w8_conversion_cleanup_verified": True,
        "retry_after_transport_failure": True,
        "prior_failed_run": "w8-logikbench-semantic-v1-001",
        "prior_failed_pod_id": "41by0ln85ailn7",
        "prior_failed_uploaded_source_files": 0,
        "prior_failed_cleanup_verified": True,
        "prior_no_create_run": "w8-logikbench-semantic-v2-001",
        "prior_no_create_verified": True,
        "chunk_bytes": CHUNK_BYTES,
    }
    if any(authorization.get(key) != value for key, value in expected.items()):
        raise RuntimeError("W8 semantic-scout authorization scope mismatch")
    hashes = {
    ...
    if any(authorization.get(key) != value for key, value in hashes.items()):
        raise RuntimeError("W8 semantic-scout authorization hash mismatch")
    return authorization


def frozen_bundle(manifest):
    payload = BUNDLE_PATH.read_bytes()
    if len(payload) != 1540008 or hashlib.sha256(payload).hexdigest() != "142f6d5e6ad4fe68ef3f64e6a74a0236fa786ae9e990c27ea1ed8c533faa24aa":
...
def watchdog():
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(r"cm-w8-logikbench-semantic-v3-[a-f0-9]{12}", state["name"])
...
def execute_remote(pod_id, token, raw, created, record):
    ...
    with requests.Session() as proxy:
        ...
        record["uploaded_source_files"] = 82
        ...
        while time.time() < created + CLEANUP_AT - 30:
            ...
            if time.time() >= created + 390 and progress.get("stage") not in ("semantic-root-oracle-scout",):
...
def save_evidence(log):
    ...
    if validation.get("status") != "complete":
        ...
        return result
    try:
        semantic_root = evidence_root / "w8-semantic"
        scout = load(semantic_root / "semantic-scout.json")
        draft = load(semantic_root / "confirmation-draft.json")
        oracle = load(semantic_root / "oracle-package.json")
    ...
    oracle_ids = [row.get("case_id") for row in oracle_rows]
    ...
    if (
        ...
        or scout.get("schema") != "cm-comparative-w8-semantic-scout/v1"
        ...
        or scout.get("unique_eligible") < 30
        or scout.get("primary_selected") != 30
        or len(primary) != 30
        or any(row.get("status") != "eligible" for row in primary)
        or any(row.get("translation_compatible") is not True for row in primary)
        or any(row.get("translation_truth_sha256") != row.get("truth_sha256") for row in primary)
        or any(row.get("performance_measurement") is not False for row in primary)
        or any(row.get("performance_claim_permitted") is not False for row in primary)
        or draft.get("schema") != "cm-comparative-w8-logikbench-confirmation-draft/v1"
        ...
        or len(case_ids) != 30
        or any(not str(case.get("source", {}).get("path", "")).startswith("sources/logikbench-") for case in cases)
        or oracle.get("schema") != "cm-comparative-w8-oracle-package/v1"
        or len(oracle_rows) != 30
        or case_ids != oracle_ids
        or summary.get("primary_selected") != 30
        or summary.get("unique_eligible") != scout.get("unique_eligible")
        or summary.get("semantic_scout_sha256") != hashlib.sha256((semantic_root / "semantic-scout.json").read_bytes()).hexdigest()
        or summary.get("oracle_package_sha256") != hashlib.sha256((semantic_root / "oracle-package.json").read_bytes()).hexdigest()
        or summary.get("confirmation_draft_sha256") != hashlib.sha256((semantic_root / "confirmation-draft.json").read_bytes()).hexdigest()
    ):
        result["partial_evidence"] = {
            "remote_status": validation.get("status"),
            "eligible": scout.get("eligible"),
            "unique_eligible": scout.get("unique_eligible"),
            "validation_failure": "retrieved evidence did not satisfy frozen W8 semantic gates",
        }
        return result
...
def run():
    ...
    try:
        ...
        if (len(manifest["files"]) != 82 or manifest.get("bytes") != 15054954
                or manifest.get("schema") != "cm-runpod-w8-logikbench-semantic-upload-manifest/v1"
            ...
            raise RuntimeError("frozen W8 semantic-scout package mismatch")
        ...
        created = time.time()
        prior_failure = load(HERE / "w8-logikbench-semantic-v1-001/RUN.json")
        if (prior_failure.get("status") != "failed"
                or prior_failure.get("pod_id") != "41by0ln85ailn7"
                or prior_failure.get("error") != "chunk acceptance could not be reconciled"
                or prior_failure.get("uploaded_source_files") != 0
                or prior_failure.get("cleanup", {}).get("owned_pod_absent") is not True
                or prior_failure.get("cleanup", {}).get("inventories") != {"v1": [], "v2": []}):
            raise RuntimeError("prior semantic transport failure is not safely reconciled")
        prior_no_create = load(HERE / "w8-logikbench-semantic-v2-001/RUN.json")
        if (prior_no_create.get("status") != "failed"
                or prior_no_create.get("creation_attempted") is not False
                or prior_no_create.get("pod_created") is not False
                or prior_no_create.get("uploaded_source_files") != 0
                or prior_no_create.get("error") != "current account/resource/budget preflight failed"):
            raise RuntimeError("prior zero-availability refusal is not safely reconciled")
        state = {"name": "cm-w8-logikbench-semantic-v3-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        write(OUT / "TRANSPORT-FREEZE.json", {
            ...
            "source_files": 82, "source_bytes": sum(row["bytes"] for row in manifest["files"]),
        ...
        print(json.dumps({"action": "create_one_cpu_pod_for_w8_logikbench_semantic_v3_retry", "name": state["name"], "cpu": offer["id"], "rate": offer["rate_usd_per_hour"]}), flush=True)
