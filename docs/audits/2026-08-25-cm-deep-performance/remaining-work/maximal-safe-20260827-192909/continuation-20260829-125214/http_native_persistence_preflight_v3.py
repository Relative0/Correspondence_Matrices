"""Read-only native-persistence V3 preflight preserving and reconciling V2."""
...
import http_native_persistence_preflight_v2 as previous
...
V2_OUTPUT = HERE / "native-persistence-v2-001"
...
def check():
    ...
    if (
        ...
        or saved.get("pod_id") != "odcer7ilz59smo"
        or validation.get("error") != "RuntimeError: d4-build failed with exit code 2"
        ...
        raise RuntimeError("native persistence V2 result is not reconciled")
    ...
    try:
        ...
        client.close()
    if current != {"v1": [], "v2": []}:
        raise RuntimeError("nonempty RunPod inventory before native persistence V3")
    result["native_persistence_v2_reconciled"] = True
    result["native_persistence_v2_pod_id"] = saved["pod_id"]
    result["native_persistence_v2_estimated_compute_cost_usd"] = saved.get("estimated_compute_cost_usd")
