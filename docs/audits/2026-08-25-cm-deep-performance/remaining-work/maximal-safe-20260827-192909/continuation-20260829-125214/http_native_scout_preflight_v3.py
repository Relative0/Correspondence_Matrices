"""Read-only preflight for a proposed chunked retry after two reconciled failures."""
...
def prior_attempts():
    ...
    return {
        ...
        "pod_ids": sorted({*historical["pod_ids"], "4q816o02xw5lxn", SCOUT_POD_ID, RETRY_POD_ID}),
...
def billing_check(client):
    ...
    return {"metadata": {key: metadata.get(key) for key in ("query", "recordCount", "uniquePodCount", "totals")},
            ...
            "new_comparative_campaign_observed_cost_usd": sum(
                row["amount"] for row in sanitized if row["podId"] in {SCOUT_POD_ID, RETRY_POD_ID}
...
if __name__ == "__main__":
    ...
    path = HERE / ("HTTP-NATIVE-SCOUT-CHUNKED-RETRY-PREFLIGHT-" + datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S-%f") + ".json")
