"""CLI-audited closure-complete readiness scout; executed only by its controller."""
...
def publish_evidence(status, error, manifest, before):
    ...
    try:
        ...
        reconciliation = smoke.get("reconciliation") or {}
        validation["smoke_summary"] = {
            "status": smoke.get("status"),
            "planned_cells": smoke.get("planned_cells"),
            "observed_cells": reconciliation.get("observed_cells"),
            "statuses": reconciliation.get("statuses"),
        }
    except Exception as exc:
...
try:
    ...
    run_command(
        "p5-smoke",
        [sys.executable, "scripts/cm_comparative_smoke.py", "run", "--output", str(OUT / "p5-smoke")],
        60,
    )
    run_command(
        "p5-smoke-verify",
        [sys.executable, "scripts/cm_comparative_smoke.py", "verify", "--output", str(OUT / "p5-smoke")],
        60,
    )
