"""Frozen remote program for a readiness scout; executed only by its controller."""
...
def publish_evidence(status, error, manifest, before):
    ...
    try:
        ...
        suites = list(ET.parse(OUT / "focused.xml").getroot().iter("testsuite"))
        validation["junit"] = {
            key: sum(int(row.get(key, "0")) for row in suites)
        ...
        validation.update(
            smoke_summary={key: smoke.get(key) for key in ("status", "planned_cells", "observed_cells", "statuses")},
            native_summary={key: native.get(key) for key in (
