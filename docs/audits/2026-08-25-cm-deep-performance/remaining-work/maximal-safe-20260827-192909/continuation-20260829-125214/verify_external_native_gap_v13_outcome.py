"""Read-only safety/evidence reconciliation for the externally owned V13 pod."""
...
import verify_external_native_gap_v12_outcome as prior


HERE = Path(__file__).resolve().parent
RUN_DIR = HERE / "native-gap-v13-001"
POD_ID = "4ovdjr0todatgl"
PREVIOUS_FINAL = HERE / "EXTERNAL-NATIVE-GAP-V12-FINAL-VERIFICATION-20260829-161613-266111.json"
AUTHORIZATION = HERE / "HTTP-NATIVE-GAP-CAMPAIGN-V13-AUTHORIZED-20260829.json"
PROPOSAL = HERE / "RUNPOD-NATIVE-GAP-CAMPAIGN-V13-D4-FENCE-PROPOSAL-20260829.md"
CONTROLLER = HERE / "runpod_native_gap_controller_v13.py"
PREFLIGHT = HERE / "http_native_gap_preflight_v13.py"
MANIFEST = HERE / "RUNPOD-NATIVE-SCOUT-UPLOAD-MANIFEST-V9-20260829.json"
...
def evidence_check():
    ...
    if len(starts) != 1 or digest != starts[0]["sha256"] or archive.stat().st_size != starts[0]["bytes"]:
        raise ValueError("retrieved V13 evidence differs from remote markers")
    ...
    with zipfile.ZipFile(archive) as bundle:
        ...
        for info in infos:
            ...
            if hashlib.sha256(target.read_bytes()).digest() != hashlib.sha256(bundle.read(info)).digest():
                raise ValueError("extracted evidence differs from V13 archive")
    ...
    native_outputs = {
        name: (output / ("native-scout/" + name + ".json")).exists()
        for name in ("cadical", "cudd", "d4", "perf")
    }
    return {
        ...
        "native_output_files": native_outputs,
        ...
        "failure_confirmed": (
            ...
            and metadata == {"tests": 185, "failures": 0, "errors": 0, "skipped": 0}
            ...
            and control_cleanup
            and native_outputs == {"cadical": False, "cudd": False, "d4": False, "perf": False}
...
def main():
    ...
    with prior.api.session() as client:
        checks = {}
        for version, endpoint in (("v1", prior.api.V1), ("v2", prior.api.V2)):
            details = {}
            ...
            checks[version] = {
                ...
                "inventory": prior.api.inventory(client, endpoint),
            }
        ...
        billing = prior.api.billing_check(client)
    ...
    result["controller_prior_cost_omitted_recent_attempts"] = (
    ...
    result["actual_resource_identity_verified"] = (
        ...
        and pod.get("imageName") == prior.safe_controller.base.IMAGE
        and pod.get("containerDiskInGb") == 12
        and type(pod.get("volumeInGb")) is int
        and pod.get("volumeInGb") == 0
        and sorted(pod.get("ports")) == sorted(prior.safe_controller.EXPECTED_PORTS)
    ...
    result["chunked_transport_verified"] = (
        ...
        and run.get("upload_chunks") == math.ceil(freeze["transport_payload_bytes"] / prior.safe_controller.CHUNK_BYTES)
    ...
    for role in ("http-controller", "http-watchdog"):
        ...
        current_pid_running = prior.safe_controller.windows_pid_running(release["pid"])
        current_created = prior.windows_process_created_utc(release["pid"]) if current_pid_running else None
        released = datetime.fromisoformat(release["released_utc"])
        pid_reused = current_created is not None and current_created > released
        releases[role] = {
            **release,
            "current_pid_occupant_running": current_pid_running,
            "current_pid_occupant_created_utc": current_created.isoformat() if current_created is not None else None,
            "pid_reused_after_release": pid_reused,
            "original_process_still_running": current_pid_running and not pid_reused,
        }
    result["guard_releases"] = releases
    result["guards_exited"] = all(
        row.get("released") is True and row["original_process_still_running"] is False
    ...
    output = HERE / (
        "EXTERNAL-NATIVE-GAP-V13-FINAL-VERIFICATION-"
        + datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S-%f")
        + ".json"
    )
    prior.safe_controller.write(output, result)
