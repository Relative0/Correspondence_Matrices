"""Read-only preflight for W3 completion after the isolated sqrt exclusion."""
...
SHARDS = {
    "ir-development-square": {"output": "p7-w3-final-ir-development-square-v7-001", "cells": 4},
    "relation-development-a": {"output": "p7-w3-final-relation-development-a-v7-001", "cells": 85},
    "relation-development-b-light": {"output": "p7-w3-final-relation-development-b-light-v7-001", "cells": 75},
    "relation-development-square": {"output": "p7-w3-final-relation-development-square-v7-001", "cells": 5},
}
ORDER = tuple(SHARDS)
...
def check():
    ...
    ir_development_a = _load("p7-w3-split-ir-development-a-v4-001")
    collision = _load("p7-w3-split-ir-development-b-v4-001")
    ir_development_b_timeout = _load("p7-w3-split-ir-development-b-v5-001")
    ir_development_b_light = _load("p7-w3-tail-ir-development-b-light-v6-001")
    ir_development_sqrt_timeout = _load("p7-w3-tail-ir-development-sqrt-v6-001")
    ...
    _require_complete(ir_development_a, "ir-development-a", 68)
    if (
        collision.get("status") != "failed"
        or collision.get("creation_attempted") is not False
        or collision.get("pod_created") is not False
        or collision.get("uploaded_source_files") != 0
        or collision.get("error") != "nonempty RunPod inventory before P7 functional scout"
    ):
        raise RuntimeError("V4 unrelated-inventory no-create refusal is not reconciled")

    if (
        ir_development_b_timeout.get("status") != "failed"
        or ir_development_b_timeout.get("pod_id") != "dop3aggj7vsefp"
        or ir_development_b_timeout.get("uploaded_source_files") != 96
        or ir_development_b_timeout.get("evidence", {}).get("validation", {}).get("error")
        != "RuntimeError: p7-ir timed out"
        or ir_development_b_timeout.get("evidence", {}).get("validation", {}).get("source_unchanged") is not True
        or not _clean(ir_development_b_timeout)
    ):
        raise RuntimeError("V5 ir-development-b timeout is not reconciled")
    _require_complete(ir_development_b_light, "ir-development-b-light", 60)
    if (
        ir_development_sqrt_timeout.get("status") != "failed"
        or ir_development_sqrt_timeout.get("pod_id") != "wtvtfqt2kamwax"
        or ir_development_sqrt_timeout.get("uploaded_source_files") != 96
        or ir_development_sqrt_timeout.get("evidence", {}).get("validation", {}).get("error")
        != "RuntimeError: p7-ir timed out"
        or ir_development_sqrt_timeout.get("evidence", {}).get("validation", {}).get("source_unchanged") is not True
        or not _clean(ir_development_sqrt_timeout)
    ):
        raise RuntimeError("isolated ir-development sqrt timeout is not reconciled")
    ...
    fixed = (
        ...
        transport,
        ir_regression,
        ir_development_timeout,
        relation_regression,
        ir_development_a,
        ir_development_b_timeout,
        ir_development_b_light,
        ir_development_sqrt_timeout,
    )
    ...
    try:
        ...
        client.close()
    if current != {"v1": [], "v2": []}:
        raise RuntimeError("nonempty RunPod inventory before remaining W3 completion partition")

    offer = result["selected_offer"]
    projected = (offer["rate_usd_per_hour"] + STORAGE_RATE_RESERVE_USD_PER_HOUR) / 3
    result.update({
        "v4_inventory_collision_reconciled": True,
        "v5_ir_development_timeout_reconciled": True,
        "v6_ir_development_light_reconciled": True,
        "v6_isolated_sqrt_timeout_reconciled": True,
        "relation_sqrt_skipped_shared_oracle_infeasible": True,
