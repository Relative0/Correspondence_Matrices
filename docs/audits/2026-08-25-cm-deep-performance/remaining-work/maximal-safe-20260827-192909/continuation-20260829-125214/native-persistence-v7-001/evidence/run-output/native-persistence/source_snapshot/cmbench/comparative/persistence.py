...
The artifact contains only reconstructable structure: flat instructions for
CM/CSE or clauses for the direct-CNF control.  It never contains a cached truth
vector or query answer.  Artifact byte hashes are retained rather than compared
across unlike representations; exact reloaded semantics are compared against
scalar CNF enumeration and a separate flat-program or clause replay.
"""
...
FLAT_BUNDLE_SCHEMA = "cm-comparative-serialized-flat/v1"
CNF_BUNDLE_SCHEMA = "cm-comparative-serialized-cnf/v1"
...
def persistence_contract(*, contract_id: str, backend: str, k: int, queries: int) -> dict[str, Any]:
    ...
    contract = {
        ...
        "validation": {
            "oracle": (
                "scalar_cnf_plus_independent_clause_replay/v1"
                if backend == "cnf"
                else "scalar_cnf_plus_independent_flat_replay/v1"
            ),
    ...
    if backend == "cnf":
            ...
            versions.append(
                {
                    ...
                    "clauses": [list(clause) for clause in version["clauses"]],
                }
            )
    else:
        ...
        try:
            for index, version in enumerate(scenario["versions"]):
                ...
                # Round-trip validation here prevents a producer from emitting
                # an object the strict loader cannot later reconstruct.
            ...
            engine.close()
    bundle = {
        "schema": CNF_BUNDLE_SCHEMA if backend == "cnf" else FLAT_BUNDLE_SCHEMA,
        "backend": backend,
        "case_id": scenario["id"],
        "scenario_sha256": _digest(scenario),
        "k": k,
        "versions": versions,
        "producer": "canonical_direct_cnf/v1" if backend == "cnf" else "cm_session_flat_export/v1",
...
def _validate_clauses(value: Any, k: int) -> tuple[tuple[int, ...], ...]:
    require(isinstance(value, list), "serialized CNF clauses")
    clauses: list[tuple[int, ...]] = []
    for clause in value:
        require(isinstance(clause, list), "serialized CNF clause")
        literals: list[int] = []
        for literal in clause:
            require(type(literal) is int and literal != 0 and abs(literal) <= k, "serialized CNF literal")
...
def validate_bundle(
    ...
    """Validate all structure before returning reconstructed programs/clauses."""
    k = sessions.validate_scenario(scenario)
    ...
    require(
        bundle["schema"] == (CNF_BUNDLE_SCHEMA if expected_backend == "cnf" else FLAT_BUNDLE_SCHEMA)
    ...
    require(
        bundle["producer"] == (
            "canonical_direct_cnf/v1" if expected_backend == "cnf" else "cm_session_flat_export/v1"
        )
        and bundle["answer_cache_included"] is False,
    ...
    for index, (row, version) in enumerate(zip(rows, scenario["versions"])):
        fields = {"version", "version_id", "clauses" if expected_backend == "cnf" else "program"}
        require(
        ...
        if expected_backend == "cnf":
            clauses = _validate_clauses(row["clauses"], k)
            require(clauses == tuple(tuple(clause) for clause in version["clauses"]), "CNF reconstruction mismatch")
            loaded.append((index, clauses))
        else:
...
def _clause_relation(clauses: Sequence[Sequence[int]], k: int) -> int:
    relation = 0
    for assignment in range(1 << k):
        valid = True
        for clause in clauses:
            if not any(bool((assignment >> (abs(literal) - 1)) & 1) == (literal > 0) for literal in clause):
                valid = False
                break
        relation |= int(valid) << assignment
    return relation


def _semantic_rows_from_structures(
    structures: Sequence[tuple[int, Any]], scenario: Mapping[str, Any], backend: str
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    k = scenario["k"]
    for index, structure in structures:
        relation = _clause_relation(structure, k) if backend == "cnf" else scalar.execute_flat(structure, k)
        rows.append(
            {
                "version": index,
...
def execute_persistence(
    *,
    ...
    decoded_at = clock()
    structures = validate_bundle(decoded, scenario=scenario, expected_backend=backend)
    reconstructed = clock()
    rows = _semantic_rows_from_structures(structures, scenario, backend)
    queried = clock()
    timings = {
...
def validate_persistence_result(
    ...
    *,
    ...
    structures = validate_bundle(bundle, scenario=scenario, expected_backend=expected_backend)
    ...
    require(
        _semantic_rows_from_structures(structures, scenario, expected_backend) == expected_rows,
        "fresh reconstruction mismatch",
    )

    independent_rows = []
    auditor = None if expected_backend == "cnf" else (
        independent_auditor if independent_auditor is not None else scalar.independent_auditor()
    )
    for row, (_loaded_index, structure) in zip(bundle["versions"], structures):
        relation = _clause_relation(structure, scenario["k"]) if expected_backend == "cnf" else auditor.replay_flat(row["program"])
        index = row["version"]
    ...
    require(independent_rows == expected_rows, "independent structure replay mismatch")
    ...
    require(
        ...
        and counts["engine_instances"] == (0 if expected_backend == "cnf" else 1)
        and counts["programs_built"] == (0 if expected_backend == "cnf" else len(scenario["versions"]))
