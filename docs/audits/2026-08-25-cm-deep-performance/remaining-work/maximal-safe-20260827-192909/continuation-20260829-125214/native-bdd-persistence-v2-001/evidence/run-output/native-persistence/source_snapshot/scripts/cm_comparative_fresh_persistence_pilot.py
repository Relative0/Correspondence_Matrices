...
def build_bundle(
    ...
    *,
    ...
    if case_limit is not None:
        if type(case_limit) is not int or not 1 <= case_limit <= len(scenarios):
            raise ValueError("bounded positive case limit")
    ...
    selected_arms = fresh.ARMS if arm_filter is None else arm_filter
    if (
        not selected_arms
        or len(set(selected_arms)) != len(selected_arms)
        or any(arm not in fresh.ARMS for arm in selected_arms)
    ):
        raise ValueError("reviewed unique arm filter")
    execution_arms = tuple(arm for arm in selected_arms if capabilities[arm]["status"] == "available")
    refused_arms = tuple(arm for arm in selected_arms if capabilities[arm]["status"] == "refused")
    excluded_arms = tuple(arm for arm in fresh.ARMS if arm not in selected_arms)
    if not execution_arms or set(execution_arms) | set(refused_arms) != set(selected_arms):
    ...
    blocks = 2 * len(execution_arms) if blocks_override is None else blocks_override
    if type(blocks) is not int or not 1 <= blocks <= 2 * len(execution_arms):
        raise ValueError("bounded block override")
    contracts = {
    ...
    *,
    ...
    result = {
        ...
        "resources": {
            ...
            "memory_ranking_reason": "single functional pilot; no ranking contract or calibrated repetitions",
        },
        "identity": {
...
def run(
    ...
    *,
    ...
    plan, oracles = build_bundle(
        ROOT, arm_filter=arm_filter, case_limit=case_limit, blocks_override=blocks_override
    )
    plan["cloud_run"] = cloud_run
    ...
    summary = {
        ...
        "limitations": [
            ...
            "native CUDD ZDD and d4 execute only when their exact native dependencies are identified",
...
def verify(output: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or set(plan["execution_arms"]) & set(plan["refused_arms"])
        or set(plan["execution_arms"]) & set(plan["excluded_arms"])
        or set(plan["refused_arms"]) & set(plan["excluded_arms"])
        or set(plan["execution_arms"]) | set(plan["refused_arms"]) | set(plan["excluded_arms"])
        != set(fresh.ARMS)
    ):
        raise ValueError("capability partition")
    ...
    if (
        ...
        or summary.get("cloud_run") is not plan.get("cloud_run")
...
def main() -> int:
    ...
    run_parser.add_argument("--arms", type=str)
    run_parser.add_argument("--case-limit", type=int)
    run_parser.add_argument("--blocks", type=int)
    run_parser.add_argument("--cloud-run", action="store_true")
