...
class FreshPersistencePilotTests(unittest.TestCase):
    def test_plan_is_complete_balanced_and_refusal_preserving(self):
        ...
        available = {
            arm for arm, capability in plan["capabilities"].items()
            if capability["status"] == "available"
        }
        refused = set(fresh.ARMS) - available
        self.assertEqual(set(plan["execution_arms"]), available)
        self.assertEqual(set(plan["refused_arms"]), refused)
