...
def derive_w4_freeze(parent_path):
    import copy
    ...
    selected = set(SCOUT_CASE_IDS)
    if len(selected) != 12:
        raise RuntimeError("duplicate W4 selected case")
    derived = copy.deepcopy(parent)
    derived["cases"] = [case for case in parent["cases"] if case["case_id"] in selected]
    if len(derived["cases"]) != 12 or {case["case_id"] for case in derived["cases"]} != selected:
        raise RuntimeError("W4 selected case unavailable")
    policies = []
    ...
    provenance["w4_timing_scout"] = {
        ...
        "w3_audit_sha256": "d2d69c2e6830e4d6c14401e6dd96a07dee5c8011759e6c45e9c4d0899fb94382",
        "w8_confirmation_freeze_sha256": "427522568449d4d385ce642769b87b0703216535edb131653a0a75b2a8e39dcc",
        "case_count": 12,
        "selected_case_ids": SCOUT_CASE_IDS,
        "selection_ledger_sha256": "d5f8314ac1a75dcc3c5d34293230cd8fd158aedeb25c7aaa14b7cc649ebb7c3f",
        "selection_uses_comparative_timing": False,
        "typed_w3_exclusion": "development-epfl-sqrt-31cdaf5d0213",
    }
    ...
    if derived["freeze_sha256"] != DERIVED_FREEZE_SHA256:
        raise RuntimeError("W4 derived freeze identity mismatch")
...
def publish_evidence(status, error, manifest, before):
    ...
    validation = {
        ...
        "w4_freeze_sha256": DERIVED_FREEZE_SHA256,
    }
    try:
...
try:
    ...
    freeze_path = str(derive_w4_freeze(parent_freeze))
    ...
    for policy, blocks in (("p7-ir", 8), ("p7-relation", 10)):
        ...
        if remaining <= 60:
            raise RuntimeError("insufficient W4 execution horizon")
        run_command(
            name,
            [
                ...
                "--roles", "development", "--blocks", str(blocks),
                "--profile", "performance",
        ...
        remaining = execution_deadline - time.time()
        if remaining <= 20:
            raise RuntimeError("insufficient W4 verification horizon")
        run_command(
            name + "-verify",
            [
