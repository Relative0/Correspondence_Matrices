"""Read-only safety/evidence reconciliation for the externally owned V12 pod."""
...
RUN_DIR = HERE / "native-gap-v12-001"
POD_ID = "omsz7w8fmlhqn0"
PREVIOUS_FINAL = HERE / "CONCURRENT-NATIVE-SCOUT-V10-FINAL-VERIFICATION-20260829-160711-016880.json"
AUTHORIZATION = HERE / "HTTP-NATIVE-GAP-CAMPAIGN-V12-AUTHORIZED-20260829.json"
PROPOSAL = HERE / "RUNPOD-NATIVE-GAP-CAMPAIGN-V12-AUTHORITY-RECONCILIATION-20260829.md"
CONTROLLER = HERE / "runpod_native_gap_controller_v12.py"
PREFLIGHT = HERE / "http_native_scout_preflight_v12.py"
MANIFEST = HERE / "RUNPOD-NATIVE-SCOUT-UPLOAD-MANIFEST-V8-20260829.json"
...
def windows_process_created_utc(pid):
    """Return the current Windows PID occupant's creation time, if queryable."""
    import ctypes
    ...
    handle = kernel.OpenProcess(0x00001000, False, pid)  # Query only.
    if not handle:
        return None
    try:
...
def evidence_check():
    ...
    if len(starts) != 1 or digest != starts[0]["sha256"] or archive.stat().st_size != starts[0]["bytes"]:
        raise ValueError("retrieved V12 evidence differs from remote markers")
    ...
    with zipfile.ZipFile(archive) as bundle:
        ...
        for info in infos:
            ...
            if hashlib.sha256(target.read_bytes()).digest() != hashlib.sha256(bundle.read(info)).digest():
                raise ValueError("extracted evidence differs from V12 archive")
    ...
    return {
        ...
        "cudd_native_execution": cudd.get("worker", {}).get("native_execution"),
        "cudd_cases": cudd.get("worker", {}).get("cases"),
        "cudd_dump_reload_exact": cudd.get("worker", {}).get("dump_reload_exact"),
        "cudd_cleanup_verified": cudd_supervision.get("cleanup_verified"),
        "cudd_whole_tree_rss_measured": cudd_supervision.get("whole_tree_rss_measured"),
        ...
        "failure_confirmed": (
            ...
            and native_summary.get("error") == "d4 dynamic dependencies"
...
def main():
    ...
    for role in ("http-controller", "http-watchdog"):
        ...
        current_pid_running = safe_controller.windows_pid_running(release["pid"])
        current_created = (
            windows_process_created_utc(release["pid"]) if current_pid_running else None
        )
        ...
        releases[role] = {
            **release,
            ...
            "current_pid_occupant_created_utc": (
                current_created.isoformat() if current_created is not None else None
            ),
    ...
    result["authorization_scope"] = {
        ...
        "authorization_file_present": AUTHORIZATION.is_file(),
    }
    ...
    output = HERE / (
        "EXTERNAL-NATIVE-GAP-V12-FINAL-VERIFICATION-"
