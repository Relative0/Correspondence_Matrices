"""Read-only V20 preflight preserving and reconciling V19."""
...
import http_native_gap_preflight_v19 as previous
...
V19_OUTPUT = HERE / "native-gap-v19-001"
...
def check():
    ...
    try:
        ...
        client.close()
    if current != {"v1": [], "v2": []}:
        raise RuntimeError("nonempty RunPod inventory before V20")
    result["v19_native_result_reconciled"] = True
    result["v19_pod_id"] = saved["pod_id"]
    result["v19_estimated_compute_cost_usd"] = saved.get("estimated_compute_cost_usd")
