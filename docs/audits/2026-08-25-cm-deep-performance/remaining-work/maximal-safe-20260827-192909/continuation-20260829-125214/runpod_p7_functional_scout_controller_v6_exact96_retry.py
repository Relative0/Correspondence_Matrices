"""One-create exact-96 P7 Linux functional-scout transport-fix retry."""
import argparse
import base64
...
REMOTE_CODE_PATH = HERE / "runpod_p7_functional_scout_remote_v2.py"
...
PROPOSAL_PATH = HERE / "RUNPOD-P7-FUNCTIONAL-SCOUT-V6-EXACT96-RETRY-PROPOSAL-20260830.md"
AUTHORIZATION_PATH = HERE / "HTTP-P7-FUNCTIONAL-SCOUT-V6-EXACT96-RETRY-AUTHORIZED-20260830.json"
...
def require_authorization():
    ...
    expected = {
        "schema": "cm-runpod-p7-functional-scout-v6-exact96-retry-authorization/v1",
        "authorized": True,
        "one_create": True,
        ...
        "prior_attempt_pod_ids": ["1xh6csc4oxy067", "2fzt8mu6ji6nmw", "r044pqp2vgp7cy"],
...
def watchdog():
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(r"cm-p7-functional-scout-v6e96-[a-f0-9]{12}", state["name"])
...
def run():
    ...
    try:
        ...
        created = time.time()
        state = {"name": "cm-p7-functional-scout-v6e96-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        print(json.dumps({"action": "create_one_cpu_pod_for_p7_functional_scout_v6_exact96", "name": state["name"], "cpu": offer["id"], "rate": offer["rate_usd_per_hour"]}), flush=True)
