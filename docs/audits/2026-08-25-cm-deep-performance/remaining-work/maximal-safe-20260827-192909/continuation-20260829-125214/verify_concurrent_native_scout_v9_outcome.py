"""Read-only safety/evidence reconciliation for the concurrently launched V9 pod."""
...
RUN_DIR = HERE / "native-procfs-v9-001"
POD_ID = "jwyi342sjmjkcj"
PREVIOUS_FINAL = HERE / "HTTP-NATIVE-SCOUT-HOST-PREFLIGHT-AMENDMENT-FINAL-VERIFICATION-20260829-153952-549038.json"
AUTHORIZATION = HERE / "HTTP-NATIVE-SCOUT-V9-WATCHDOG-BINDING-AUTHORIZED-20260829.invalid-no-replacement"
PROPOSAL = HERE / "RUNPOD-NATIVE-SCOUT-V8-WATCHDOG-BINDING-AMENDMENT-20260829.md"
CONTROLLER = HERE / "runpod_native_scout_controller_v9.py"
PREFLIGHT = HERE / "http_native_scout_preflight_v9.py"
MANIFEST = HERE / "RUNPOD-NATIVE-SCOUT-UPLOAD-MANIFEST-V6-20260829.json"
...
def evidence_check():
    ...
    if len(starts) != 1 or digest != starts[0]["sha256"] or archive.stat().st_size != starts[0]["bytes"]:
        raise ValueError("retrieved V9 evidence differs from remote markers")
    ...
    with zipfile.ZipFile(archive) as bundle:
        ...
        for info in infos:
            ...
            if hashlib.sha256(target.read_bytes()).digest() != hashlib.sha256(bundle.read(info)).digest():
                raise ValueError("extracted evidence differs from V9 archive")
    ...
    return {
        ...
        "failure_confirmed": (
            ...
            and metadata == {"tests": 183, "failures": 0, "errors": 0, "skipped": 0}
...
def main():
    ...
    correct_prior_bound = float(previous_final["attributable_campaign_cost_bound_usd"])
    ...
    result["authorization_scope"] = {
        "recorded_aggregate_campaign_cap_usd": authorization.get("aggregate_campaign_cap_usd"),
        ...
        "invalidated_for_replay": not (HERE / "HTTP-NATIVE-SCOUT-V9-WATCHDOG-BINDING-AUTHORIZED-20260829.json").exists(),
    }
    ...
    output = HERE / (
        "CONCURRENT-NATIVE-SCOUT-V9-FINAL-VERIFICATION-"
