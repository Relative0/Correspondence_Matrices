"""Read-only V14 preflight preserving and reconciling V13."""
...
import http_native_gap_preflight_v13 as previous
...
V13_OUTPUT = HERE / "native-gap-v13-001"
...
def check():
    ...
    try:
        ...
        client.close()
    if current != {"v1": [], "v2": []}:
        raise RuntimeError("nonempty RunPod inventory before V14")
    result["v13_native_result_reconciled"] = True
    result["v13_pod_id"] = saved["pod_id"]
    result["v13_estimated_compute_cost_usd"] = saved.get("estimated_compute_cost_usd")
