"""V6 one-create controller for one final exact-96 P7 W3 development-tail partition."""
import argparse
import base64
...
SHARDS = {
    "ir-development-b-light": {"policy": "p7-ir", "role": "development", "cases": 15, "cells": 60,
                               "offset": 17, "limit": 15, "code": "irdl", "out": "p7-w3-tail-ir-development-b-light-v6-001",
                               "remote": "runpod_p7_w3_tail_remote_v6_ir_development_b_light.py"},
    "ir-development-sqrt": {"policy": "p7-ir", "role": "development", "cases": 1, "cells": 4,
                            "offset": 32, "limit": 1, "code": "irsq", "out": "p7-w3-tail-ir-development-sqrt-v6-001",
                            "remote": "runpod_p7_w3_tail_remote_v6_ir_development_sqrt.py"},
    "ir-development-square": {"policy": "p7-ir", "role": "development", "cases": 1, "cells": 4,
                              "offset": 33, "limit": 1, "code": "irsqr", "out": "p7-w3-tail-ir-development-square-v6-001",
                              "remote": "runpod_p7_w3_tail_remote_v6_ir_development_square.py"},
    "relation-development-a": {"policy": "p7-relation", "role": "development", "cases": 17, "cells": 85,
                               "offset": 0, "limit": 17, "code": "rda", "out": "p7-w3-tail-relation-development-a-v6-001",
                               "remote": "runpod_p7_w3_tail_remote_v6_relation_development_a.py"},
    "relation-development-b-light": {"policy": "p7-relation", "role": "development", "cases": 15, "cells": 75,
                                     "offset": 17, "limit": 15, "code": "rdl", "out": "p7-w3-tail-relation-development-b-light-v6-001",
                                     "remote": "runpod_p7_w3_tail_remote_v6_relation_development_b_light.py"},
    "relation-development-sqrt": {"policy": "p7-relation", "role": "development", "cases": 1, "cells": 5,
                                  "offset": 32, "limit": 1, "code": "rdsq", "out": "p7-w3-tail-relation-development-sqrt-v6-001",
                                  "remote": "runpod_p7_w3_tail_remote_v6_relation_development_sqrt.py"},
    "relation-development-square": {"policy": "p7-relation", "role": "development", "cases": 1, "cells": 5,
                                    "offset": 33, "limit": 1, "code": "rdsqr", "out": "p7-w3-tail-relation-development-square-v6-001",
                                    "remote": "runpod_p7_w3_tail_remote_v6_relation_development_square.py"},
}
SHARD_ID = os.environ.get("CM_W3_SPLIT_ID")
if SHARD_ID not in SHARDS:
    raise RuntimeError("CM_W3_SPLIT_ID must name one final W3 development-tail partition")
SHARD = SHARDS[SHARD_ID]
SHARD["name_prefix"] = "cm-p7-w3s6-" + SHARD["code"]
...
REMOTE_CODE_PATH = HERE / SHARD["remote"]
...
PROPOSAL_PATH = HERE / "RUNPOD-P7-W3-DEVELOPMENT-TAIL-V6-PROPOSAL-20260830.md"
AUTHORIZATION_PATH = HERE / ("HTTP-P7-W3-TAIL-" + SHARD_ID.upper() + "-V6-AUTHORIZED-20260830.json")
PARTITION_VALIDATION_PATH = HERE / "P7-W3-TAIL-V6-OFFLINE-VALIDATION.json"
...
def require_authorization():
    if not AUTHORIZATION_PATH.is_file():
        raise RuntimeError("exact P7 W3 shard authorization record is absent")
    ...
    expected = {
        ...
        "shard_role": SHARD["role"],
        "shard_cases": SHARD["cases"],
        "shard_cells": SHARD["cells"],
        "fresh_cell_processes": SHARD["cells"],
        "blocks": 1,
        "campaign_shards": 7,
        "case_offset": SHARD["offset"],
        "case_limit": SHARD["limit"],
        ...
        "prior_attempt_pod_ids": ["1xh6csc4oxy067", "2fzt8mu6ji6nmw", "r044pqp2vgp7cy", "6mlqn19hnco1b0", "d9z39u7pzvbju8", "pnpc0c0t6gu358", "9xjz22tif2ctrv", "alu08d0mlf02ba", "1uqsgeb7vpzihb", "dop3aggj7vsefp"],
        "prior_no_create_attempts": ["p7-functional-scout-v4-freeze-closed-001", "p7-w3-split-ir-development-b-v4-001"],
        "chunk_bytes": CHUNK_BYTES,
    }
    if any(authorization.get(key) != value for key, value in expected.items()):
        raise RuntimeError("P7 W3 shard authorization scope mismatch")
    hashes = {
        ...
        "partition_validation_sha256": hashlib.sha256(PARTITION_VALIDATION_PATH.read_bytes()).hexdigest(),
    }
    if any(authorization.get(key) != value for key, value in hashes.items()):
        raise RuntimeError("P7 W3 shard authorization hash mismatch")
    partition_validation = load(PARTITION_VALIDATION_PATH)
    if (
        partition_validation.get("ready") is not True
        or partition_validation.get("parent_freeze_sha256")
        != "54ea61a38135426975a0d1fead9b24c020dc565eb3d952356640fa38062598dd"
        or partition_validation.get("source_bundle_sha256")
        != "83fdde6cde6d02628ba24fd932671bc40ae2067a7fbb49187a38b580c71cc668"
    ):
        raise RuntimeError("P7 W3 partition validation is not ready")
...
def watchdog():
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(re.escape(SHARD["name_prefix"]) + r"-[a-f0-9]{12}", state["name"])
...
def expected_partition_case_ids():
    ...
    ordered = []
    for row in policies[SHARD["policy"]]["order_ledger"]:
        case = cases[row["case_id"]]
        if case["role"] == "development" and row["case_id"] not in ordered:
            ordered.append(row["case_id"])
    if len(ordered) != 34:
        raise RuntimeError("parent W3 development case count changed")
    selected = ordered[SHARD["offset"]:SHARD["offset"] + SHARD["limit"]]
    if len(selected) != SHARD["cases"]:
        raise RuntimeError("parent W3 partition cardinality changed")
    return selected
...
def save_evidence(log):
    ...
    try:
        ...
        metadata = load(evidence_root / "DERIVED-FREEZE-METADATA.json")
        shard_plan = load(evidence_root / SHARD["policy"] / "plan.json")
        expected_case_ids = expected_partition_case_ids()
        shard_ok, _shard_summary, _shard_verify, shard_pids = _p7_terminal_check(
            evidence_root / SHARD["policy"], SHARD["cells"], SHARD["policy"]
        )
    ...
    if (
        validation.get("junit_testcases") != {"tests": 42, "failures": 0, "errors": 0, "skipped": 0}
        or type(junit_metadata.get("tests")) is not int or junit_metadata.get("tests", 0) < 42
        ...
        or metadata.get("schema") != "cm-comparative-p7-functional-partition/v1"
        or metadata.get("partition_id") != SHARD_ID
        or metadata.get("policy_id") != SHARD["policy"]
        or metadata.get("role") != "development"
        or metadata.get("case_offset") != SHARD["offset"]
        or metadata.get("case_limit") != SHARD["limit"]
        or metadata.get("parent_development_cases") != 34
        or metadata.get("selected_case_ids_in_parent_order") != expected_case_ids
        or metadata.get("performance_measurement") is not False
        or set(shard_plan.get("case_ids") or []) != set(expected_case_ids)
        or len(shard_plan.get("case_ids") or []) != SHARD["cases"]
        or shard_plan.get("freeze_sha256") != metadata.get("derived_freeze_sha256")
        or not shard_ok
        or len(set(shard_pids)) != SHARD["cells"]
    ):
        result["partial_evidence"] = {
            "remote_status": validation.get("status"),
            "validation_failure": "retrieved evidence did not satisfy frozen P7 W3 shard gates",
        }
        return result
    result["verified"] = True
    result["p7"] = {
        "focused_tests": 42, "shard_id": SHARD_ID, "policy": SHARD["policy"],
        "role": SHARD["role"], "cases": SHARD["cases"], "blocks": 1,
        "cells": SHARD["cells"], "fresh_cell_processes": SHARD["cells"], "source_unchanged": True,
        "case_offset": SHARD["offset"], "case_limit": SHARD["limit"],
        "parent_freeze_sha256": metadata["parent_freeze_sha256"],
        "derived_freeze_sha256": metadata["derived_freeze_sha256"],
        "selected_case_ids_in_parent_order": expected_case_ids,
...
def run():
    ...
    try:
        ...
        if (len(manifest["files"]) != 96 or manifest.get("bytes") != 19484163
            ...
            raise RuntimeError("frozen P7 W3 shard package mismatch")
        ...
        created = time.time()
        state = {"name": SHARD["name_prefix"] + "-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        print(json.dumps({"action": "create_one_cpu_pod_for_p7_w3_shard", "shard_id": SHARD_ID,
                          "name": state["name"], "cpu": offer["id"], "rate": offer["rate_usd_per_hour"]}), flush=True)
