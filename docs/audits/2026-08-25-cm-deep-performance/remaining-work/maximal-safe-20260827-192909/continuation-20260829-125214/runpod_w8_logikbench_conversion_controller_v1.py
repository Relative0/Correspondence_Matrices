"""One-create W8 LogikBench conversion-only scout controller."""
import argparse
import base64
...
OUT = HERE / "w8-logikbench-conversion-v1-001"
...
AUTHORIZATION_PATH = HERE / "HTTP-W8-LOGIKBENCH-CONVERSION-AUTHORIZED-20260830.json"
...
def require_authorization():
    ...
    expected = {
        "schema": "cm-runpod-w8-logikbench-conversion-authorization/v1",
        "authorized": True,
        "one_create": True,
...
def watchdog():
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(r"cm-w8-logikbench-convert-v1-[a-f0-9]{12}", state["name"])
...
def run():
    ...
    try:
        ...
        created = time.time()
        state = {"name": "cm-w8-logikbench-convert-v1-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        print(json.dumps({"action": "create_one_cpu_pod_for_w8_logikbench_conversion_v1", "name": state["name"], "cpu": offer["id"], "rate": offer["rate_usd_per_hour"]}), flush=True)
