"""One-create W8 conversion scout after an unrelated-pod local refusal."""
import argparse
import base64
...
OUT = HERE / "w8-logikbench-conversion-v4-001"
...
REMOTE_CODE_PATH = HERE / "runpod_w8_logikbench_conversion_remote_v3.py"
base.REMOTE_CODE = REMOTE_CODE_PATH.read_text(encoding="utf-8")
base.OUT = OUT
BOOTSTRAP_PATH = HERE / "http_native_scout_bootstrap_v2.py"
MANIFEST_PATH = HERE / "RUNPOD-W8-LOGIKBENCH-CONVERSION-UPLOAD-MANIFEST-V3-20260830.json"
BUNDLE_PATH = HERE / "RUNPOD-W8-LOGIKBENCH-CONVERSION-UPLOAD-BUNDLE-V3-20260830.zip"
PROPOSAL_PATH = HERE / "RUNPOD-W8-LOGIKBENCH-CONVERSION-V3-RETRY-PROPOSAL-20260830.md"
AUTHORIZATION_PATH = HERE / "HTTP-W8-LOGIKBENCH-CONVERSION-V4-AUTHORIZED-20260830.json"
...
def require_authorization():
    if not AUTHORIZATION_PATH.is_file():
        raise RuntimeError("exact W8 conversion-scout authorization record is absent")
    authorization = load(AUTHORIZATION_PATH)
    expected = {
        "schema": "cm-runpod-w8-logikbench-conversion-authorization/v4",
        "authorized": True,
        "one_create": True,
        ...
        "carried_forward_unconsumed_authorization": True,
        "prior_local_no_create": "w8-logikbench-conversion-v1-001",
        "prior_created_failure": "w8-logikbench-conversion-v2-001",
        "prior_created_pod_id": "71gv8a3dttwnma",
        "prior_created_failure_cleaned": True,
        "prior_unrelated_pod_no_create": "w8-logikbench-conversion-v3-001",
        "chunk_bytes": CHUNK_BYTES,
    }
    if any(authorization.get(key) != value for key, value in expected.items()):
        raise RuntimeError("W8 conversion-scout authorization scope mismatch")
    hashes = {
    ...
    if any(authorization.get(key) != value for key, value in hashes.items()):
        raise RuntimeError("W8 conversion-scout authorization hash mismatch")
    return authorization


def frozen_bundle(manifest):
    payload = BUNDLE_PATH.read_bytes()
    if len(payload) != 204587 or hashlib.sha256(payload).hexdigest() != "be42ee3517167e83168b138e8607eeebf5c6cc66d6660167daa2b38b425277b3":
...
def watchdog():
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(r"cm-w8-logikbench-convert-v4-[a-f0-9]{12}", state["name"])
...
def execute_remote(pod_id, token, raw, created, record):
    ...
    with requests.Session() as proxy:
        ...
        record["uploaded_source_files"] = 159
        ...
        while time.time() < created + CLEANUP_AT - 30:
            ...
            if time.time() >= created + 390 and progress.get("stage") not in ("conversion",):
                raise RuntimeError("boot/Yosys install exceeded bounded setup deadline")
            time.sleep(2)
...
def save_evidence(log):
    ...
    if validation.get("status") != "complete":
        ...
        return result
    try:
        conversion_root = evidence_root / "w8-conversion"
        conversions = load(conversion_root / "conversions.json")
        fixtures = load(conversion_root / "fixture-summary.json")
        environment = load(conversion_root / "environment.json")
        checksums = load(conversion_root / "checksums.json")
    ...
    for row in checksums.get("files") or []:
        path = conversion_root / row.get("path", "")
        if (
            not path.is_file()
            ...
            checksum_ok = False
    converted_files_ok = all(
        (conversion_root / "converted" / (row["cluster_id"] + ".blif")).is_file()
        and (conversion_root / "converted" / (row["cluster_id"] + ".blif")).stat().st_size == row.get("bytes")
        and hashlib.sha256(
            (conversion_root / "converted" / (row["cluster_id"] + ".blif")).read_bytes()
    ...
    if (
        ...
        or conversions.get("schema") != "cm-comparative-w8-yosys-conversions/v1"
        or conversions.get("performance_measurement") is not False
        or conversions.get("performance_claim_permitted") is not False
        or conversions.get("attempted") != 70
        or conversions.get("converted") != len(converted)
        or conversions.get("rejected") != len(rejected)
        or len(rows) != 70
        or len({row.get("cluster_id") for row in rows}) != 70
        or len(converted) < 30
        or any(row.get("status") not in ("converted", "rejected") for row in rows)
        or not converted_files_ok
        or fixtures.get("fixture_count") != 5
        or fixtures.get("semantic_equivalence") is not True
        or any(row.get("semantic_equivalence") is not True for row in fixtures.get("fixtures") or [])
        or environment.get("static_admission_sha256") != "54ba7e252acc79138099d3f380713fbd230cc71a47338c8050ef5a152e457207"
        or environment.get("performance_measurement") is not False
        or not environment.get("yosys_version")
        or not environment.get("yosys_package")
        or checksums.get("schema") != "cm-comparative-w8-conversion-checksums/v1"
        or not checksum_ok
    ):
        result["partial_evidence"] = {
            ...
            "validation_failure": "retrieved evidence did not satisfy frozen W8 conversion gates",
        }
        return result
    result["verified"] = True
    result["w8_conversion"] = {
        "attempted": 70,
        "converted": len(converted),
        "rejected": len(rejected),
        "fixtures": 5,
        "fixture_semantic_equivalence": True,
...
def run():
    ...
    try:
        ...
        if (len(manifest["files"]) != 159 or manifest.get("bytes") != 617277
                or manifest.get("schema") != "cm-runpod-w8-logikbench-conversion-upload-manifest/v3"
            ...
            raise RuntimeError("frozen W8 conversion-scout package mismatch")
        ...
        created = time.time()
        prior_local = load(HERE / "w8-logikbench-conversion-v1-001/LOCAL-PREFLIGHT-FAILURE.json")
        if (prior_local.get("cloud_create_attempted") is not False
                or prior_local.get("external_upload_performed") is not False
                or prior_local.get("authorization_consumed") is not False):
            raise RuntimeError("prior local AC refusal is not safely reconciled")
        prior_no_create = load(HERE / "w8-logikbench-conversion-v3-001/RUN.json")
        if (prior_no_create.get("creation_attempted") is not False
                or prior_no_create.get("pod_created") is not False
                or prior_no_create.get("uploaded_source_files") != 0):
            raise RuntimeError("prior unrelated-pod refusal is not safely reconciled")
        state = {"name": "cm-w8-logikbench-convert-v4-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        write(OUT / "TRANSPORT-FREEZE.json", {
            ...
            "source_files": 159, "source_bytes": sum(row["bytes"] for row in manifest["files"]),
        ...
        print(json.dumps({"action": "create_one_cpu_pod_for_w8_logikbench_conversion_v4", "name": state["name"], "cpu": offer["id"], "rate": offer["rate_usd_per_hour"]}), flush=True)
