"""One-create W8 semantic scout retry with smaller resumable transport chunks."""
import argparse
import base64
...
import requests
import http_w8_logikbench_semantic_preflight_v1 as preflight

HERE = Path(__file__).resolve().parent
TRANSPORT = HERE.parent / "runpod-authorized-20260827-213104"
OUT = HERE / "w8-logikbench-semantic-v2-001"
...
AUTHORIZATION_PATH = HERE / "HTTP-W8-LOGIKBENCH-SEMANTIC-V2-AUTHORIZED-20260830.json"
...
def watchdog():
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(r"cm-w8-logikbench-semantic-v2-[a-f0-9]{12}", state["name"])
...
def run():
    ...
    try:
        ...
        state = {"name": "cm-w8-logikbench-semantic-v2-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        print(json.dumps({"action": "create_one_cpu_pod_for_w8_logikbench_semantic_v2_retry", "name": state["name"], "cpu": offer["id"], "rate": offer["rate_usd_per_hour"]}), flush=True)
