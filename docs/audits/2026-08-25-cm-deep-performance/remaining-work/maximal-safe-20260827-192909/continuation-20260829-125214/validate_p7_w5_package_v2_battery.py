"""Validate the battery-authorized immutable W5 package without authentication."""
...
ROOT = HERE.parents[5]
OUTPUT = HERE / "P7-W5-PACKAGE-V2-BATTERY-LOCAL-VALIDATION.json"
...
CONTROLLER = HERE / "runpod_p7_w5_controller_v2_battery.py"
PREFLIGHT = HERE / "http_p7_w5_development_preflight_v2_battery.py"
BOOTSTRAP = HERE / "http_native_scout_bootstrap_v3_w4_deadlines.py"
PROPOSAL = HERE / "RUNPOD-P7-W5-BATTERY-AMENDMENT-20260901.md"
...
def main() -> int:
    ...
    if (
        "battery_awake_guard" not in controller_source
        or "base.host_awake_guard" in controller_source
        or '"battery_launch_authorized": True' not in controller_source
        or "MINIMUM_BATTERY_PERCENT = 50" not in preflight_source
        or '"battery_override_authorized": True' not in preflight_source
    ):
        failures.append("battery amendment contract")
    ...
    result = {
        "schema": "cm-runpod-p7-w5-package-local-validation/v2-battery",
        "ready": not failures,
