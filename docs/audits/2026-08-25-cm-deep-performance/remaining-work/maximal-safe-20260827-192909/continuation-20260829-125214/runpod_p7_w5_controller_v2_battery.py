...
import requests
import http_p7_w5_development_preflight_v2_battery as preflight
...
PROPOSAL_PATH = HERE / "RUNPOD-P7-W5-BATTERY-AMENDMENT-20260901.md"
AUTHORIZATION_PATH = HERE / "HTTP-P7-W5-BATTERY-AUTHORIZED-20260901.json"
PACKAGE_VALIDATION_PATH = HERE / "P7-W5-PACKAGE-V2-BATTERY-LOCAL-VALIDATION.json"
...
def configure(shard_id):
    ...
    OUT = HERE / ("p7-w5-" + shard_id + "-v2-battery-001")
    STATE = OUT / "controller-state.json"
    IDENTITY = OUT / "POD-IDENTITY.json"
    READY = OUT / "watchdog-ready.json"
    STATE_ACK = OUT / "watchdog-state-ack.json"
...
@contextmanager
def battery_awake_guard(role):
    """Prevent idle sleep and record the user-authorized battery launch state."""
    if os.name != "nt":
        raise RuntimeError("battery-authorized W5 controller is validated for Windows only")
    import ctypes
    from ctypes import wintypes

    power = preflight.host_power_status()
    if power.get("battery_gate_passed") is not True:
        raise RuntimeError("battery is below the authorized W5 safety floor")
    kernel = ctypes.WinDLL("kernel32", use_last_error=True)
    set_state = kernel.SetThreadExecutionState
    set_state.argtypes = [wintypes.DWORD]
    set_state.restype = wintypes.DWORD
    requested = 0x80000001  # ES_CONTINUOUS | ES_SYSTEM_REQUIRED; no display/away mode.
    if not set_state(requested):
        raise RuntimeError("failed to establish temporary idle-sleep prevention")
    write(
        OUT / ("HOST-AWAKE-" + role + ".json"),
        {
            ...
            "limitation": "does not prevent lid-close, explicit sleep, depleted battery, or network loss",
        },
    )
    try:
        yield
        ...
        write(
            OUT / ("HOST-AWAKE-RELEASED-" + role + ".json"),
...
def require_authorization():
    ...
    expected = {
        "schema": "cm-runpod-p7-w5-development-authorization/v2-battery",
        "authorized": True,
        ...
        "battery_launch_authorized": True,
        "minimum_battery_percent_when_not_on_ac": 50,
    }
...
def watchdog():
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(r"cm-p7-w5-" + re.escape(SHARD_ID) + r"-v2-battery-[a-f0-9]{12}", state["name"])
...
def run():
    ...
    try:
        ...
        created = time.time()
        state = {"name": "cm-p7-w5-" + SHARD_ID + "-v2-battery-" + uuid.uuid4().hex[:12], "created_epoch": created,
...
def main():
    ...
    with battery_awake_guard("http-watchdog" if args.watchdog else "http-controller"):
