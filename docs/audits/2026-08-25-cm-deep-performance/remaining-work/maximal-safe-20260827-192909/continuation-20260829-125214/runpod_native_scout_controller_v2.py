"""One separately authorized retry of the comparative Linux/native scout."""
import argparse
import base64
...
OUT = HERE / "http-native-scout-retry-execute-001"
...
REMOTE_CODE_PATH = HERE / "runpod_native_scout_remote.py"
base.REMOTE_CODE = REMOTE_CODE_PATH.read_text(encoding="utf-8")
base.OUT = OUT
BOOTSTRAP_PATH = HERE / "http_native_scout_bootstrap_v1.py"
MANIFEST_PATH = HERE / "RUNPOD-NATIVE-SCOUT-UPLOAD-MANIFEST-V2-20260829.json"
PROPOSAL_PATH = HERE / "RUNPOD-NATIVE-SCOUT-RETRY-AMENDMENT-PROPOSAL-20260829.md"
AUTHORIZATION_PATH = HERE / "HTTP-NATIVE-SCOUT-RETRY-AUTHORIZED-20260829.json"
...
def execute_remote(pod_id, token, raw, created, record):
    ...
    with requests.Session() as proxy:
        ...
        record["uploaded_source_files"] = 30
...
def save_evidence(log):
    ...
    if (
        ...
        or validation.get("junit") != {"tests": 60, "failures": 0, "errors": 0, "skipped": 0}
        or validation.get("source_unchanged") is not True
        or validation.get("source_files") != 30
        ...
        or runtime.get("source_files") != 30
        or runtime.get("runpod_pod_id") != load(IDENTITY)["pod_id"]
        or len(runtime.get("affinity", [])) != 2
    ):
        raise RuntimeError("retrieved evidence did not satisfy frozen native-scout gates")
...
def run():
    ...
    try:
        ...
        if (len(manifest["files"]) != 30 or manifest.get("bytes") != 5461757
        ...
        write(OUT / "TRANSPORT-FREEZE.json", {
            ...
            "source_files": 30, "source_bytes": sum(row["bytes"] for row in manifest["files"]),
