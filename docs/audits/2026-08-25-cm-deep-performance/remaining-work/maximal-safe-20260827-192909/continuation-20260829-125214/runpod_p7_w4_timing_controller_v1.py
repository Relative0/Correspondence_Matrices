"""One-create controller for the frozen P7 W4 timing/RSS scout."""
import argparse
import base64
...
PROPOSAL_PATH = HERE / "RUNPOD-P7-W4-TIMING-SCOUT-PROPOSAL-20260831.md"
AUTHORIZATION_PATH = HERE / "HTTP-P7-W4-TIMING-V1-AUTHORIZED-20260831.json"
...
def require_authorization():
    ...
    expected = {
        "schema": "cm-runpod-p7-w4-timing-authorization/v1",
        "authorized": True,
        "one_create": True,
...
def watchdog():
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(r"cm-p7-w4-timing-v1-[a-f0-9]{12}", state["name"])
...
def run():
    ...
    try:
        ...
        created = time.time()
        state = {"name": "cm-p7-w4-timing-v1-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        print(json.dumps({"action": "create_one_cpu_pod_for_p7_w4_timing_scout", "name": state["name"], "cpu": offer["id"], "rate": offer["rate_usd_per_hour"]}), flush=True)
