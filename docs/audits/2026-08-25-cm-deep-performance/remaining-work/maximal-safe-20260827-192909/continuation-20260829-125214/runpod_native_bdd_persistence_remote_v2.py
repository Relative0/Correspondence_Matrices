...
ROOT = Path("/workspace/cm-native-bdd-persistence")
...
def publish_evidence(status, error, manifest, before):
    ...
    try:
        validation["native_dependencies"] = json.loads((OUT / "NATIVE-DEPENDENCIES.json").read_text(encoding="utf-8"))
    except Exception as exc:
        _validation_error(errors, "native-dependencies", exc)
    try:
