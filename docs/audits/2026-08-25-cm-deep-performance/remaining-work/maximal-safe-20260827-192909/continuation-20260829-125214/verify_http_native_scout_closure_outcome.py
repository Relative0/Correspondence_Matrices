"""Read-only postflight and partial-evidence audit for the closure scout attempt."""
...
import runpod_native_scout_controller_v4 as controller


HERE = Path(__file__).resolve().parent
RUN_DIR = HERE / "http-native-scout-closure-retry-execute-001"
CURRENT_POD_ID = "pes90ta8wgi2g6"
...
def evidence_check():
    ...
    p5_stderr = (output / "p5-smoke.stderr.txt").read_text(encoding="utf-8")
    ...
    validation_sections = {row.get("section") for row in validation.get("validation_errors", [])}
    expected_stderr = (
        "usage: cm_comparative_smoke.py run [-h] --output OUTPUT\n"
        "cm_comparative_smoke.py run: error: the following arguments are required: --output\n"
    )
    return {
        ...
        "p5_stderr_sha256": hashlib.sha256(p5_stderr.encode()).hexdigest(),
        ...
        "primary_failure_confirmed": (
            ...
            and validation.get("error") == "RuntimeError: p5-smoke failed with exit code 2"
            and focused.get("returncode") == 0
            and p5.get("returncode") == 2
            and p5_stderr == expected_stderr
            and len(cases) == 60
            ...
            and validation_sections == {"p5-smoke", "native-scout"}
...
def main():
    ...
    result["failure_evidence_preserved"] = (
        ...
        and not result["evidence"]["p5_summary_created"]
        and not result["evidence"]["native_scout_started"]
    )
    ...
    output = HERE / ("HTTP-NATIVE-SCOUT-CLOSURE-FINAL-VERIFICATION-" + datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S-%f") + ".json")
