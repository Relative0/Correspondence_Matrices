"""One-create V20 native-gap scout with fenced external d4 children."""
import argparse
import base64
...
import requests
import http_native_gap_preflight_v20 as preflight

HERE = Path(__file__).resolve().parent
TRANSPORT = HERE.parent / "runpod-authorized-20260827-213104"
OUT = HERE / "native-gap-v20-001"
...
REMOTE_CODE_PATH = HERE / "runpod_native_scout_remote_v4.py"
base.REMOTE_CODE = REMOTE_CODE_PATH.read_text(encoding="utf-8")
base.OUT = OUT
BOOTSTRAP_PATH = HERE / "http_native_scout_bootstrap_v2.py"
MANIFEST_PATH = HERE / "RUNPOD-NATIVE-SCOUT-UPLOAD-MANIFEST-V14-20260829.json"
PROPOSAL_PATH = HERE / "RUNPOD-NATIVE-GAP-CAMPAIGN-V20-D4V2-BARE-COMMENT-PROPOSAL-20260829.md"
AUTHORIZATION_PATH = HERE / "HTTP-NATIVE-GAP-CAMPAIGN-V20-AUTHORIZED-20260829.json"
...
def require_authorization():
    if not AUTHORIZATION_PATH.is_file():
        raise RuntimeError("exact native-scout authorization record is absent")
    authorization = load(AUTHORIZATION_PATH)
    expected = {
        "schema": "cm-runpod-native-gap-campaign-v20-authorization/v1",
        "authorized": True,
        "one_create": True,
        "no_replacement": True,
        "source_files": 37,
        ...
        "prior_failed_pod_ids": [
            ...
            "pow0qre2q39m4t", "3o7r0za7cm72yn", "jwyi342sjmjkcj", "kvpu2s8ozs7j27", "omsz7w8fmlhqn0", "4ovdjr0todatgl", "od0zndjtowjwon", "wctkcbmzs8ymze", "gj1k7p3gjywald", "hzn77xb71bq281"
        ],
        "chunk_bytes": CHUNK_BYTES,
        "prior_authorization_sha256": hashlib.sha256(preflight.V6_AUTHORIZATION.read_bytes()).hexdigest(),
        "local_preflight_failure_sha256": hashlib.sha256(preflight.LOCAL_FAILURE.read_bytes()).hexdigest(),
    }
    if any(authorization.get(key) != value for key, value in expected.items()):
        raise RuntimeError("native-scout authorization scope mismatch")
...
def watchdog():
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(r"cm-native-scout-[a-f0-9]{12}", state["name"])
...
def prepare_payload(bundle, manifest, created):
    payload = {"bundle": base64.b64encode(bundle).decode("ascii"),
               ...
               "environment": {"CM_BUNDLE_SHA256": hashlib.sha256(bundle).hexdigest(),
                               ...
                               "CM_SETUP_DEADLINE": str(created + 480)}}
...
def execute_remote(pod_id, token, raw, created, record):
    ...
    with requests.Session() as proxy:
        ...
        record["uploaded_source_files"] = 37
        ...
        while time.time() < created + CLEANUP_AT - 30:
            ...
            if time.time() >= created + 480 and progress.get("stage") not in ("focused-tests", "p5-smoke", "native-scout"):
                raise RuntimeError("boot/install exceeded eight-minute setup deadline")
            time.sleep(2)
...
def save_evidence(log):
    ...
    if validation.get("status") != "complete":
        ...
        return result
    try:
        smoke = load(evidence_root / "p5-smoke/summary.json")
        smoke_verify = load(evidence_root / "p5-smoke-verify.json")
        native_root = evidence_root / "native-scout"
        native = load(native_root / "summary.json")
        dependencies = load(native_root / "dependencies.json")
        controls = load(native_root / "linux-controls.json")
        cadical = load(native_root / "cadical.json")
        cudd = load(native_root / "cudd.json")
        d4 = load(native_root / "d4.json")
    ...
    if (
        validation.get("junit_testcases") != {"tests": 68, "failures": 0, "errors": 0, "skipped": 0}
        or type(junit_metadata.get("tests")) is not int
        or junit_metadata.get("tests", 0) < 68
        ...
        or validation.get("source_files") != 37
        or validation.get("smoke_summary") != {
            "status": "passed", "planned_cells": 144, "observed_cells": 144, "statuses": {"ok": 144}
        }
        or validation.get("native_summary", {}).get("status") != "passed"
        or smoke.get("status") != "passed"
        or smoke.get("planned_cells") != 144
        or smoke.get("performance_measurement") is not False
        or smoke.get("performance_claim_permitted") is not False
        or smoke.get("source_unchanged") is not True
        or smoke.get("reconciliation", {}).get("complete") is not True
        or smoke.get("reconciliation", {}).get("planned_cells") != 144
        or smoke.get("reconciliation", {}).get("observed_cells") != 144
        or smoke.get("reconciliation", {}).get("statuses") != {"ok": 144}
        or smoke.get("reconciliation", {}).get("partial_ledger_tail") is not False
        or smoke.get("reconciliation", {}).get("missing_cells") != []
        or smoke.get("reconciliation", {}).get("unexpected_cells") != []
        or smoke.get("reconciliation", {}).get("unfinished_cells") != []
        or smoke_verify.get("returncode") != 0
        or native.get("status") != "passed"
        or any(native.get(name) != "passed" for name in ("dependencies", "linux_controls", "cadical", "cudd", "d4"))
        or native.get("native_failures") != 0
        or native.get("semantic_mismatches") != 0
        or native.get("performance_measurement") is not False
        or native.get("performance_ranking_permitted") is not False
        or native.get("perf") not in {"available", "refused", "unavailable"}
        or dependencies.get("versions") != expected_versions
        or dependencies.get("temporary_artifacts_retained") is not False
        or {name: row.get("status") for name, row in controls.items()} != {
            "echo": "ok", "flood": "output_limit", "tree": "timeout", "memory": "memory_limit"
        }
        or cadical.get("worker", {}).get("status") != "passed"
        or cadical.get("worker", {}).get("native_execution") is not True
        or cudd.get("worker", {}).get("status") != "passed"
        or cudd.get("worker", {}).get("native_execution") is not True
        or cudd.get("worker", {}).get("autoref_substituted") is not False
        or d4.get("status") != "passed"
        or d4.get("native_execution") is not True
        or d4.get("elf", {}).get("linkage", {}).get("linkage") != "static"
        or d4.get("elf", {}).get("linkage", {}).get("has_pt_dynamic") is not False
        or d4.get("elf", {}).get("linkage", {}).get("has_pt_interp") is not False
        or d4.get("elf", {}).get("dependency_check") != {
            "status": "not_applicable_static", "ldd_executed": False
        }
        or len(d4.get("cases", [])) != 5
        or [
            (row.get("case"), row.get("expected"), row.get("parsed", {}).get("count"))
            for row in d4.get("cases", [])
        ] != [
            ("true-k1", 2, 2), ("false-k1", 0, 0), ("all-k3", 8, 8),
            ("unused-k3", 4, 4), ("conflict-k2", 0, 0),
        ]
        or runtime.get("source_files") != 37
        or runtime.get("runpod_pod_id") != load(IDENTITY)["pod_id"]
        or len(runtime.get("affinity", [])) != 2
    ):
        result["partial_evidence"] = {
            "remote_status": validation.get("status"),
            "validation_failure": "retrieved evidence did not satisfy frozen native-scout gates",
        }
        return result
    ...
    result["scout"] = {
        "focused_tests": 68,
        "p5_smoke_cells": 144,
        "native_components": ["CaDiCaL", "CUDD", "d4"],
        "linux_controls": {name: row["status"] for name, row in controls.items()},
        "perf": native["perf"],
...
def run():
    ...
    try:
        ...
        if (len(manifest["files"]) != 37 or manifest.get("bytes") != 5518220
                or manifest.get("authorization_status") != "pending"
                or len(lock["packages"]) != 13 or lock["source_builds_allowed"]):
            raise RuntimeError("frozen native-scout package mismatch")
        ...
        created = time.time()
        state = {"name": "cm-native-scout-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        write(OUT / "TRANSPORT-FREEZE.json", {
            ...
            "source_files": 37, "source_bytes": sum(row["bytes"] for row in manifest["files"]),
