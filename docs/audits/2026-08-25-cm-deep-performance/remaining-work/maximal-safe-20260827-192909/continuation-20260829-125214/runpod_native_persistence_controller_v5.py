"""One-create V5 retry with native-aware test selection and case-safe evidence names."""
import argparse
import base64
...
import requests
import http_native_persistence_preflight_v5 as preflight

HERE = Path(__file__).resolve().parent
TRANSPORT = HERE.parent / "runpod-authorized-20260827-213104"
OUT = HERE / "native-persistence-v5-001"
...
REMOTE_CODE_PATH = HERE / "runpod_native_persistence_remote_v5.py"
base.REMOTE_CODE = REMOTE_CODE_PATH.read_text(encoding="utf-8")
base.OUT = OUT
BOOTSTRAP_PATH = HERE / "http_native_scout_bootstrap_v2.py"
MANIFEST_PATH = HERE / "RUNPOD-NATIVE-PERSISTENCE-UPLOAD-MANIFEST-V1-20260830.json"
PROPOSAL_PATH = HERE / "RUNPOD-NATIVE-PERSISTENCE-CAMPAIGN-V5-RETRY-PROPOSAL-20260830.md"
AUTHORIZATION_PATH = HERE / "HTTP-NATIVE-PERSISTENCE-CAMPAIGN-V5-AUTHORIZED-20260830.json"
...
def require_authorization():
    if not AUTHORIZATION_PATH.is_file():
        raise RuntimeError("exact native-persistence authorization record is absent")
    authorization = load(AUTHORIZATION_PATH)
    expected = {
        "schema": "cm-runpod-native-persistence-campaign-v5-authorization/v1",
        "authorized": True,
        "one_create": True,
        "no_replacement_within_this_controller": True,
        "retry_number": 4,
        "external_source_upload_approved": True,
        "prior_native_persistence_pod_id": "vlfvhjewad21xf",
        "source_files": 136,
        "source_bytes": 5494221,
        "focused_tests": 22,
        "native_persistence_cells": 16,
        "fresh_build_processes": 16,
        "fresh_reload_processes": 16,
        "exact_relation_rows": 32,
        "d4_compiler_invocations": 16,
        "performance_ranking": False,
        "source_builds_allowed": ["astutils==0.0.6", "ply==3.10", "crillab/d4@333370cc1e843dd0749c1efe88516e72b5239174"],
        "system_packages_allowed": ["build-essential", "libboost-dev", "libgmp-dev", "zlib1g-dev"],
        "container_disk_gb": 12,
        "pod_volume_gb": 0,
        "network_volume": False,
        "lifetime_seconds": 1200,
        "phase_cap_usd": 0.10,
        "campaign_cap_usd": 10.00,
        "prior_native_readiness_pod_id": "rg3zlg5gbdbp5p",
        "chunk_bytes": CHUNK_BYTES,
    }
    if any(authorization.get(key) != value for key, value in expected.items()):
        raise RuntimeError("native-persistence authorization scope mismatch")
    hashes = {
    ...
    if any(authorization.get(key) != value for key, value in hashes.items()):
        raise RuntimeError("native-persistence authorization hash mismatch")
...
def watchdog():
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(r"cm-native-persistence-[a-f0-9]{12}", state["name"])
...
def execute_remote(pod_id, token, raw, created, record):
    ...
    with requests.Session() as proxy:
        ...
        record["uploaded_source_files"] = 136
        ...
        while time.time() < created + CLEANUP_AT - 30:
            ...
            if time.time() >= created + 780 and progress.get("stage") not in ("focused-tests", "native-persistence"):
...
def save_evidence(log):
    ...
    if validation.get("status") != "complete":
        result["partial_evidence"] = {
            ...
            "junit_metadata": validation.get("junit_metadata"),
            "junit_testcases": validation.get("junit_testcases"),
            "source_unchanged": validation.get("source_unchanged"),
        }
        return result
    try:
        pilot_root = evidence_root / "native-persistence"
        summary = load(pilot_root / "summary.json")
        plan = load(pilot_root / "plan.json")
        d4_build = load(evidence_root / "D4-COMPILER-IDENTITY.json")
        patoh_header = load(evidence_root / "PATOH-HEADER.json")
        native_dependencies = load(evidence_root / "NATIVE-DEPENDENCIES.json")
        pilot_verify = load(evidence_root / "native-persistence-verify.json")
        ledger = [
            json.loads(line)
            for line in (pilot_root / "ledger.jsonl").read_text(encoding="utf-8").splitlines()
            if line.strip()
        ]
        artifacts = sorted((pilot_root / "artifacts").glob("cell-*.json"))
    ...
    terminals = [row for row in ledger if row.get("status") == "ok" and isinstance(row.get("result"), dict)]
    terminal_ids = {row.get("cell_id") for row in terminals}
    schedule = plan.get("schedule") or {}
    cells = schedule.get("cells") or []
    capabilities = plan.get("capabilities") or {}
    zdd = capabilities.get("cudd_zdd") or {}
    d4 = capabilities.get("d4_ddnnf") or {}
    d4_identity = d4.get("identity") or {}
    zdd_identity = zdd.get("identity") or {}
    junit_metadata = validation.get("junit_metadata") or {}
    expected_reconciliation = {
        "complete": True,
        "missing_cells": [],
        "observed_cells": 16,
        "partial_ledger_tail": False,
        "planned_cells": 16,
        "statuses": {"ok": 16},
        "unexpected_cells": [],
        "unfinished_cells": [],
    }
    result_rows_valid = len(terminals) == 16 and len(terminal_ids) == 16
    for row in terminals:
        value = row["result"]
        identity = value.get("identity") or {}
        resources = value.get("resources") or {}
        build = identity.get("build_worker") or {}
        reload = identity.get("reload_worker") or {}
        arm = value.get("arm")
        if (
            arm not in {"cudd_zdd", "d4_ddnnf"}
            or value.get("status") != "ok"
            or identity.get("native_execution") is not True
            or identity.get("substitution_used") is not False
            or identity.get("answer_cache_included") is not False
            or identity.get("performance_claim_permitted") is not False
            or type(build.get("pid")) is not int
            or type(reload.get("pid")) is not int
            or build.get("pid") == reload.get("pid")
            or build.get("mode") != "build"
            or reload.get("mode") != "reload_query"
            or build.get("arm") != arm
            or reload.get("arm") != arm
            or len(reload.get("rows") or []) != 2
            or resources.get("fresh_processes_verified") is not True
            or resources.get("whole_tree_rss_measured") is not True
            or resources.get("memory_ranking_permitted") is not False
        ):
            result_rows_valid = False
            break

    if (
        validation.get("junit_testcases") != {"tests": 22, "failures": 0, "errors": 0, "skipped": 0}
        or type(junit_metadata.get("tests")) is not int
        or junit_metadata.get("tests", 0) < 22
        ...
        or validation.get("source_files") != 136
        or runtime.get("source_files") != 136
        or runtime.get("runpod_pod_id") != load(IDENTITY)["pod_id"]
        or len(runtime.get("affinity", [])) != 2
        or summary.get("status") != "passed"
        or summary.get("case_count") != 2
        or summary.get("executed_arm_count") != 2
        or summary.get("refused_arm_count") != 0
        or summary.get("refused_arms") != []
        or summary.get("blocks") != 4
        or summary.get("planned_cells") != 16
        or summary.get("cell_statuses") != {"ok": 16}
        or summary.get("arm_statuses") != {"cudd_zdd": {"ok": 8}, "d4_ddnnf": {"ok": 8}}
        or summary.get("executed_build_processes") != 16
        or summary.get("executed_reload_processes") != 16
        or summary.get("exact_relation_rows") != 32
        or summary.get("serialized_artifact_deterministic_across_blocks") is not True
        or summary.get("reconciliation") != expected_reconciliation
        or summary.get("concurrent_source_changes") != []
        or summary.get("cloud_run") is not True
        or summary.get("network_used") is not False
        or summary.get("performance_measurement") is not False
        or summary.get("performance_claim_permitted") is not False
        or plan.get("execution_arms") != ["cudd_zdd", "d4_ddnnf"]
        or set(plan.get("excluded_arms") or []) != {"cm", "cse", "cnf", "autoref_bdd_control", "cudd_bdd"}
        or schedule.get("blocks") != 4
        or len(cells) != 16
        or {cell.get("cell_id") for cell in cells} != terminal_ids
        or sum(cell.get("arm") == "cudd_zdd" for cell in cells) != 8
        or sum(cell.get("arm") == "d4_ddnnf" for cell in cells) != 8
        or zdd.get("status") != "available"
        or zdd.get("native_execution") is not True
        or zdd_identity.get("status") != "identified"
        or zdd_identity.get("native_extension") is not True
        or zdd_identity.get("module") != "dd.cudd_zdd"
        or d4.get("status") != "available"
        or d4.get("native_execution") is not True
        or d4_identity.get("status") != "identified"
        or d4_identity.get("source_commit") != "333370cc1e843dd0749c1efe88516e72b5239174"
        or d4_identity.get("sha256") != d4_build.get("binary_sha256")
        or patoh_header != {
            "bytes": 11944,
            "public_dependency_download": True,
            "sha256": "a403cadf44785c773cc77895294f6674b7e2c7006a29bd41b401a81e8aff443c",
            "source_commit": "333370cc1e843dd0749c1efe88516e72b5239174",
            "url": "https://raw.githubusercontent.com/crillab/d4/333370cc1e843dd0749c1efe88516e72b5239174/patoh/patoh.h",
        }
        or d4_build.get("status") != "passed"
        or d4_build.get("source_commit") != "333370cc1e843dd0749c1efe88516e72b5239174"
        or d4_build.get("performance_ranking_permitted") is not False
        or native_dependencies.get("temporary_artifacts_retained") is not False
        or (native_dependencies.get("versions") or {}).get("dd") != "0.6.0"
        or pilot_verify.get("returncode") != 0
        or len(artifacts) != 16
        or not result_rows_valid
    ):
        result["partial_evidence"] = {
            "remote_status": validation.get("status"),
            "validation_failure": "retrieved evidence did not satisfy frozen native-persistence gates",
        }
        return result
    ...
    result["persistence"] = {
        ...
        "fresh_reload_processes": 16,
        "exact_relation_rows": 32,
        "native_arms": ["CUDD ZDD", "d4 d-DNNF"],
        "d4_binary_sha256": d4_build["binary_sha256"],
...
def run():
    ...
    try:
        ...
        if (len(manifest["files"]) != 136 or manifest.get("bytes") != 5494221
                or manifest.get("authorization_status") != "pending"
                or len(lock["packages"]) != 13 or lock["source_builds_allowed"]):
            raise RuntimeError("frozen native-persistence package mismatch")
        ...
        created = time.time()
        state = {"name": "cm-native-persistence-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        write(OUT / "TRANSPORT-FREEZE.json", {
            ...
            "source_files": 136, "source_bytes": sum(row["bytes"] for row in manifest["files"]),
        ...
        print(json.dumps({"action": "create_one_cpu_pod_for_native_persistence", "name": state["name"], "cpu": offer["id"], "rate": offer["rate_usd_per_hour"]}), flush=True)
