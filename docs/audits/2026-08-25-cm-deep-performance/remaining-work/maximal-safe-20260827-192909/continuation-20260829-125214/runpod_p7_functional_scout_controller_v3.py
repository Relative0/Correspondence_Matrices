"""One-create dependency-closed P7 Linux functional-scout retry."""
import argparse
import base64
...
import requests
import http_p7_functional_scout_preflight_v3 as preflight

HERE = Path(__file__).resolve().parent
TRANSPORT = HERE.parent / "runpod-authorized-20260827-213104"
OUT = HERE / "p7-functional-scout-v3-001"
...
MANIFEST_PATH = HERE / "RUNPOD-P7-FUNCTIONAL-SCOUT-UPLOAD-MANIFEST-V4-20260830.json"
BUNDLE_PATH = HERE / "RUNPOD-P7-FUNCTIONAL-SCOUT-UPLOAD-BUNDLE-V4-20260830.zip"
PROPOSAL_PATH = HERE / "RUNPOD-P7-FUNCTIONAL-SCOUT-V3-RETRY-PROPOSAL-20260830.md"
AUTHORIZATION_PATH = HERE / "HTTP-P7-FUNCTIONAL-SCOUT-V3-RETRY-AUTHORIZED-20260830.json"
...
def require_authorization():
    ...
    expected = {
        "schema": "cm-runpod-p7-functional-scout-v3-retry-authorization/v1",
        "authorized": True,
        "one_create": True,
        "no_replacement_within_this_controller": True,
        "external_source_upload_approved": True,
        "source_files": 156,
        "source_bytes": 11591021,
        "source_bundle_bytes": 1731976,
        "focused_tests": 32,
        "p7_ir_cells": 16,
        ...
        "prior_p7_functional_scout_pod_id": "1xh6csc4oxy067",
...
def frozen_bundle(manifest):
    ...
    if len(payload) != 1731976 or hashlib.sha256(payload).hexdigest() != "3f4ae4ad709b029ebedf6b8cab8d4f359f40bf741ae9e3280f4d97a7a0660b17":
...
def execute_remote(pod_id, token, raw, created, record):
    ...
    with requests.Session() as proxy:
        ...
        record["uploaded_source_files"] = 156
...
def save_evidence(log):
    ...
    if (
        ...
        or validation.get("source_files") != 156
        or runtime.get("source_files") != 156
...
def run():
    ...
    try:
        ...
        if (len(manifest["files"]) != 156 or manifest.get("bytes") != 11591021
        ...
        write(OUT / "TRANSPORT-FREEZE.json", {
            ...
            "source_files": 156, "source_bytes": sum(row["bytes"] for row in manifest["files"]),
