"""One-create W8 conversion scout carrying forward an AC-refused authorization."""
import argparse
import base64
...
import requests
import http_w8_logikbench_conversion_preflight_v1 as preflight

HERE = Path(__file__).resolve().parent
TRANSPORT = HERE.parent / "runpod-authorized-20260827-213104"
OUT = HERE / "w8-logikbench-conversion-v2-001"
...
REMOTE_CODE_PATH = HERE / "runpod_w8_logikbench_conversion_remote_v2.py"
base.REMOTE_CODE = REMOTE_CODE_PATH.read_text(encoding="utf-8")
base.OUT = OUT
BOOTSTRAP_PATH = HERE / "http_native_scout_bootstrap_v2.py"
MANIFEST_PATH = HERE / "RUNPOD-W8-LOGIKBENCH-CONVERSION-UPLOAD-MANIFEST-V2-20260830.json"
BUNDLE_PATH = HERE / "RUNPOD-W8-LOGIKBENCH-CONVERSION-UPLOAD-BUNDLE-V2-20260830.zip"
PROPOSAL_PATH = HERE / "RUNPOD-W8-LOGIKBENCH-CONVERSION-PROPOSAL-20260830.md"
AUTHORIZATION_PATH = HERE / "HTTP-W8-LOGIKBENCH-CONVERSION-V2-AUTHORIZED-20260830.json"
...
def require_authorization():
    ...
    expected = {
        "schema": "cm-runpod-w8-logikbench-conversion-authorization/v2",
        "authorized": True,
        "one_create": True,
...
def frozen_bundle(manifest):
    ...
    if len(payload) != 204586 or hashlib.sha256(payload).hexdigest() != "1b3796d6ded0f6d1b0d6266c5e783f1b0687aae9c7ecfdac901ad625c6e6ff95":
...
def watchdog():
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(r"cm-w8-logikbench-convert-v2-[a-f0-9]{12}", state["name"])
...
def run():
    ...
    try:
        ...
        if (len(manifest["files"]) != 159 or manifest.get("bytes") != 617274
        ...
        state = {"name": "cm-w8-logikbench-convert-v2-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        print(json.dumps({"action": "create_one_cpu_pod_for_w8_logikbench_conversion_v2", "name": state["name"], "cpu": offer["id"], "rate": offer["rate_usd_per_hour"]}), flush=True)
