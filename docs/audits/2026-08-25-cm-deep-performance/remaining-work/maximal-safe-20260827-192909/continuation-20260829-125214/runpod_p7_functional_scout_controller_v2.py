...
OUT = HERE / "p7-functional-scout-v2-001"
...
PROPOSAL_PATH = HERE / "RUNPOD-P7-FUNCTIONAL-SCOUT-V2-RETRY-PROPOSAL-20260830.md"
AUTHORIZATION_PATH = HERE / "HTTP-P7-FUNCTIONAL-SCOUT-V2-AUTHORIZED-20260830.json"
...
def require_authorization():
    ...
    expected = {
        "schema": "cm-runpod-p7-functional-scout-v2-authorization/v1",
        "authorized": True,
        "one_create": True,
        ...
        "prior_failed_pod_id": "1xh6csc4oxy067",
...
def watchdog():
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(r"cm-p7-functional-scout-v2-[a-f0-9]{12}", state["name"])
...
def run():
    ...
    try:
        ...
        created = time.time()
        state = {"name": "cm-p7-functional-scout-v2-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        print(json.dumps({"action": "create_one_cpu_pod_for_p7_functional_scout_v2", "name": state["name"], "cpu": offer["id"], "rate": offer["rate_usd_per_hour"]}), flush=True)
