"""One-create exact-96 P7 W3 full correctness/oracle scout."""
import argparse
import base64
...
OUT = HERE / "p7-w3-correctness-v1-001"
...
REMOTE_CODE_PATH = HERE / "runpod_p7_w3_correctness_remote_v1.py"
...
PROPOSAL_PATH = HERE / "RUNPOD-P7-W3-CORRECTNESS-PROPOSAL-20260830.md"
AUTHORIZATION_PATH = HERE / "HTTP-P7-W3-CORRECTNESS-AUTHORIZED-20260830.json"
...
def require_authorization():
    if not AUTHORIZATION_PATH.is_file():
        raise RuntimeError("exact P7 W3 correctness authorization record is absent")
    authorization = load(AUTHORIZATION_PATH)
    expected = {
        "schema": "cm-runpod-p7-w3-correctness-authorization/v1",
        "authorized": True,
        "one_create": True,
        ...
        "focused_tests": 42,
        "p7_cases_per_policy": 58,
        "p7_ir_cells": 232,
        "p7_relation_cells": 290,
        "fresh_cell_processes": 522,
        "blocks": 1,
        ...
        "prior_attempt_pod_ids": ["1xh6csc4oxy067", "2fzt8mu6ji6nmw", "r044pqp2vgp7cy", "6mlqn19hnco1b0"],
        "prior_no_create_attempts": ["p7-functional-scout-v4-freeze-closed-001"],
        "chunk_bytes": CHUNK_BYTES,
    }
    if any(authorization.get(key) != value for key, value in expected.items()):
        raise RuntimeError("P7 W3 correctness authorization scope mismatch")
    hashes = {
    ...
    if any(authorization.get(key) != value for key, value in hashes.items()):
        raise RuntimeError("P7 W3 correctness authorization hash mismatch")
...
def watchdog():
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(r"cm-p7-w3-correctness-v1-[a-f0-9]{12}", state["name"])
...
def save_evidence(log):
    ...
    try:
        ...
        ir_ok, _ir_summary, _ir_verify, ir_pids = _p7_terminal_check(evidence_root / "p7-ir", 232, "p7-ir")
        relation_ok, _relation_summary, _relation_verify, relation_pids = _p7_terminal_check(
            evidence_root / "p7-relation", 290, "p7-relation"
        )
    ...
    if (
        ...
        or len(set(ir_pids + relation_pids)) != 522
    ):
        result["partial_evidence"] = {
            "remote_status": validation.get("status"),
            "validation_failure": "retrieved evidence did not satisfy frozen P7 W3 correctness gates",
        }
        return result
    result["verified"] = True
    result["p7"] = {
        "focused_tests": 42, "policies": ["p7-ir", "p7-relation"],
        "cases_per_policy": 58, "blocks": 1, "cells": 522,
        "roles": ["regression", "development"],
        "fresh_cell_processes": 522, "source_unchanged": True,
...
def run():
    ...
    try:
        ...
        if (len(manifest["files"]) != 96 or manifest.get("bytes") != 19484163
            ...
            raise RuntimeError("frozen P7 W3 package mismatch")
        ...
        created = time.time()
        state = {"name": "cm-p7-w3-correctness-v1-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        print(json.dumps({"action": "create_one_cpu_pod_for_p7_w3_correctness", "name": state["name"], "cpu": offer["id"], "rate": offer["rate_usd_per_hour"]}), flush=True)
