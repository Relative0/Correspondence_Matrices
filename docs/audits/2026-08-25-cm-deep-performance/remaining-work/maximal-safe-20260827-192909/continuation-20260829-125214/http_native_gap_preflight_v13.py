"""Read-only V13 preflight preserving and reconciling V12."""
...
import http_native_scout_preflight_v12 as previous
...
V12_OUTPUT = HERE / "native-gap-v12-001"
...
def check():
    ...
    try:
        ...
        client.close()
    if current != {"v1": [], "v2": []}:
        raise RuntimeError("nonempty RunPod inventory before V13")
    result["v12_native_result_reconciled"] = True
    result["v12_pod_id"] = saved["pod_id"]
    result["v12_estimated_compute_cost_usd"] = saved.get("estimated_compute_cost_usd")
