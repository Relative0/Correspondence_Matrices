"""Read-only V16 preflight preserving and reconciling V15."""
...
import http_native_gap_preflight_v15 as previous
...
V15_OUTPUT = HERE / "native-gap-v15-001"
...
def check():
    ...
    try:
        ...
        client.close()
    if current != {"v1": [], "v2": []}:
        raise RuntimeError("nonempty RunPod inventory before V16")
    result["v15_native_result_reconciled"] = True
    result["v15_pod_id"] = saved["pod_id"]
    result["v15_estimated_compute_cost_usd"] = saved.get("estimated_compute_cost_usd")
