"""One-create native CUDD BDD fresh-process persistence closure."""
import argparse
import base64
...
import requests
import http_native_bdd_persistence_preflight_v2 as preflight

HERE = Path(__file__).resolve().parent
TRANSPORT = HERE.parent / "runpod-authorized-20260827-213104"
OUT = HERE / "native-bdd-persistence-v2-001"
...
REMOTE_CODE_PATH = HERE / "runpod_native_bdd_persistence_remote_v2.py"
base.REMOTE_CODE = REMOTE_CODE_PATH.read_text(encoding="utf-8")
base.OUT = OUT
BOOTSTRAP_PATH = HERE / "http_native_scout_bootstrap_v2.py"
MANIFEST_PATH = HERE / "RUNPOD-NATIVE-BDD-PERSISTENCE-UPLOAD-MANIFEST-V2-20260830.json"
BUNDLE_PATH = HERE / "RUNPOD-NATIVE-BDD-PERSISTENCE-UPLOAD-BUNDLE-V2-20260830.zip"
PROPOSAL_PATH = HERE / "RUNPOD-NATIVE-BDD-PERSISTENCE-V2-RETRY-PROPOSAL-20260830.md"
AUTHORIZATION_PATH = HERE / "HTTP-NATIVE-BDD-PERSISTENCE-V2-AUTHORIZED-20260830.json"
...
def require_authorization():
    if not AUTHORIZATION_PATH.is_file():
        raise RuntimeError("exact native-BDD-persistence authorization record is absent")
    authorization = load(AUTHORIZATION_PATH)
    expected = {
        "schema": "cm-runpod-native-bdd-persistence-v2-authorization/v1",
        "authorized": True,
        "one_create": True,
        "no_replacement_within_this_controller": True,
        "external_source_upload_approved": True,
        "source_files": 136,
        "source_bytes": 5497827,
        "source_bundle_bytes": 1078139,
        "focused_tests": 25,
        "native_persistence_cells": 4,
        "fresh_build_processes": 4,
        "fresh_reload_processes": 4,
        "exact_relation_rows": 8,
        "d4_compiler_invocations": 0,
        ...
        "prior_native_bdd_persistence_pod_id": "8fh5st71uqe2pe",
        "chunk_bytes": CHUNK_BYTES,
    }
    if any(authorization.get(key) != value for key, value in expected.items()):
        raise RuntimeError("native-BDD-persistence authorization scope mismatch")
    hashes = {
    ...
    if any(authorization.get(key) != value for key, value in hashes.items()):
        raise RuntimeError("native-BDD-persistence authorization hash mismatch")
    return authorization


def frozen_bundle(manifest):
    payload = BUNDLE_PATH.read_bytes()
    if len(payload) != 1078139 or hashlib.sha256(payload).hexdigest() != "053ede82b0f69a1053778b74778ab7ea1b3f5af5a898924289e38d73447f92b3":
...
def watchdog():
    ...
    if (set(state) != {"name", "created_epoch", "cleanup_epoch", "horizon_epoch"}
        or not re.fullmatch(r"cm-native-bdd-persistence-[a-f0-9]{12}", state["name"])
...
def save_evidence(log):
    ...
    bdd = (plan.get("capabilities") or {}).get("cudd_bdd") or {}
    bdd_identity = bdd.get("identity") or {}
    ...
    result_rows_valid = len(terminals) == 4 and len(terminal_ids) == 4
    for row in terminals:
        ...
        if (
            ...
            or build.get("arm") != "cudd_bdd"
            or reload.get("arm") != "cudd_bdd"
    ...
    if (
        validation.get("junit_testcases") != {"tests": 25, "failures": 0, "errors": 0, "skipped": 0}
        or type(junit_metadata.get("tests")) is not int
        or junit_metadata.get("tests", 0) < 25
        ...
        or summary.get("executed_arm_count") != 1
        or summary.get("refused_arm_count") != 0
        or summary.get("refused_arms") != []
        or summary.get("blocks") != 2
        or summary.get("planned_cells") != 4
        or summary.get("cell_statuses") != {"ok": 4}
        or summary.get("arm_statuses") != {"cudd_bdd": {"ok": 4}}
        or summary.get("executed_build_processes") != 4
        or summary.get("executed_reload_processes") != 4
        or summary.get("exact_relation_rows") != 8
        ...
        or plan.get("execution_arms") != ["cudd_bdd"]
        or set(plan.get("excluded_arms") or []) != {
            "cm", "cse", "cnf", "autoref_bdd_control", "cudd_zdd", "d4_ddnnf"
        }
        or schedule.get("blocks") != 2
        or len(cells) != 4
        or {cell.get("cell_id") for cell in cells} != terminal_ids
        or any(cell.get("arm") != "cudd_bdd" for cell in cells)
        or bdd.get("status") != "available"
        or bdd.get("native_execution") is not True
        or bdd_identity.get("status") != "identified"
        or bdd_identity.get("native_extension") is not True
        or bdd_identity.get("module") != "dd.cudd"
        ...
        result["partial_evidence"] = {
            "remote_status": validation.get("status"),
            "validation_failure": "retrieved evidence did not satisfy frozen native-CUDD-BDD persistence gates",
        }
        return result
...
def run():
    ...
    try:
        ...
        if (len(manifest["files"]) != 136 or manifest.get("bytes") != 5497827
        ...
        created = time.time()
        state = {"name": "cm-native-bdd-persistence-" + uuid.uuid4().hex[:12], "created_epoch": created,
        ...
        print(json.dumps({"action": "create_one_cpu_pod_for_native_bdd_persistence", "name": state["name"], "cpu": offer["id"], "rate": offer["rate_usd_per_hour"]}), flush=True)
