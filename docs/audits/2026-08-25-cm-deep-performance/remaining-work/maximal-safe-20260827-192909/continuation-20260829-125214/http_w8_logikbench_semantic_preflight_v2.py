"""Read-only W8 semantic preflight with all compatible Secure CPU flavors."""
...
from verify_w8_logikbench_semantic_upload_v1 import verify as verify_upload
...
def check() -> dict:
    ...
    conversion_audit = json.loads(
        (HERE / "W8-LOGIKBENCH-CONVERSION-FINAL-AUDIT.json").read_text(encoding="utf-8")
    )
    conversion_postflight = json.loads(
        (HERE / "W8-LOGIKBENCH-CONVERSION-FINAL-POSTFLIGHT.json").read_text(encoding="utf-8")
    )
    if (
        conversion_audit.get("verified") is not True
        or conversion_audit.get("conversion", {}).get("converted") != 64
        or conversion_audit.get("performance_measurement") is not False
        or conversion_audit.get("performance_claim_permitted") is not False
        or conversion_postflight.get("all_created_pods_absent") is not True
        or conversion_postflight.get("all_inventories_empty") is not True
        or conversion_postflight.get("inventories") != {"v1": [], "v2": []}
    ):
        raise RuntimeError("audited W8 conversion evidence is not ready for semantic selection")
    ...
    result = {
        ...
        "w8_conversion_audit_verified": True,
        "w8_conversion_cleanup_verified": True,
        "w8_converted_clusters": 64,
        "w8_required_primary_cases": 30,
...
if __name__ == "__main__":
    try:
        ...
        value = {
            "checked_utc": utc_now(), "ready": False,
            "error_type": type(exc).__name__, "resource_writes": 0,
        }
