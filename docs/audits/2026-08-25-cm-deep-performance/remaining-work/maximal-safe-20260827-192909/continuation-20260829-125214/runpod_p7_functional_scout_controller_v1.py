"""One-create P7 Linux isolated-cell functional scout."""
import argparse
import base64
...
import requests
import http_p7_functional_scout_preflight_v1 as preflight

HERE = Path(__file__).resolve().parent
TRANSPORT = HERE.parent / "runpod-authorized-20260827-213104"
OUT = HERE / "p7-functional-scout-v1-001"
...
MANIFEST_PATH = HERE / "RUNPOD-P7-FUNCTIONAL-SCOUT-UPLOAD-MANIFEST-V1-20260830.json"
BUNDLE_PATH = HERE / "RUNPOD-P7-FUNCTIONAL-SCOUT-UPLOAD-BUNDLE-V1-20260830.zip"
PROPOSAL_PATH = HERE / "RUNPOD-P7-FUNCTIONAL-SCOUT-V1-PROPOSAL-20260830.md"
AUTHORIZATION_PATH = HERE / "HTTP-P7-FUNCTIONAL-SCOUT-V1-AUTHORIZED-20260830.json"
...
def require_authorization():
    ...
    expected = {
        "schema": "cm-runpod-p7-functional-scout-v1-authorization/v1",
        "authorized": True,
        "one_create": True,
        "no_replacement_within_this_controller": True,
        "external_source_upload_approved": True,
        "source_files": 152,
        "source_bytes": 11224621,
        "source_bundle_bytes": 1745617,
        "focused_tests": 32,
        "p7_ir_cells": 16,
...
def frozen_bundle(manifest):
    ...
    if len(payload) != 1745617 or hashlib.sha256(payload).hexdigest() != "b57838452a590b89018c1e9dee77fb31b30d2e0931b739da97797ab1a7ca7076":
...
def execute_remote(pod_id, token, raw, created, record):
    ...
    with requests.Session() as proxy:
        ...
        record["uploaded_source_files"] = 152
...
def save_evidence(log):
    ...
    if (
        ...
        or validation.get("source_files") != 152
        or runtime.get("source_files") != 152
...
def run():
    ...
    try:
        ...
        if (len(manifest["files"]) != 152 or manifest.get("bytes") != 11224621
        ...
        write(OUT / "TRANSPORT-FREEZE.json", {
            ...
            "source_files": 152, "source_bytes": sum(row["bytes"] for row in manifest["files"]),
