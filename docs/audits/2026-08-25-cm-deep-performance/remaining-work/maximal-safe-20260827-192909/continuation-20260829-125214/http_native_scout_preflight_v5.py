"""Read-only preflight for a P5-CLI-corrected scout after four reconciled failures."""
...
CLOSURE_POD_ID = "pes90ta8wgi2g6"
CLOSURE_RUN = HERE / "http-native-scout-closure-retry-execute-001/RUN.json"
CLOSURE_FINAL = HERE / "HTTP-NATIVE-SCOUT-CLOSURE-FINAL-VERIFICATION-20260829-112440-495599.json"
CLOSURE_CONTROLLER = HERE / "runpod_native_scout_controller_v4.py"
CLOSURE_FREEZE = HERE / "http-native-scout-closure-retry-execute-001/TRANSPORT-FREEZE.json"
...
def prior_attempts():
    ...
    closure_run = load(CLOSURE_RUN)
    closure_final = load(CLOSURE_FINAL)
    closure_freeze = load(CLOSURE_FREEZE)
    if (
        closure_run.get("status") != "failed"
        or closure_run.get("error_type") != "RuntimeError"
        or closure_run.get("error") != "remote workload reported failure"
        or closure_run.get("pod_id") != CLOSURE_POD_ID
        or closure_run.get("creation_http_status") != 201
        or closure_run.get("creation_uncertain") is not False
        or closure_run.get("uploaded_source_files") != 37
        or closure_run.get("remote_progress", {}).get("remote_status") != "failed"
        or closure_run.get("remote_progress", {}).get("stage") != "p5-smoke"
        or closure_run.get("evidence", {}).get("validation", {}).get("source_unchanged") is not True
        or closure_run.get("cleanup", {}).get("owned_pod_absent") is not True
        or closure_final.get("attempt_safely_reconciled") is not True
        or closure_final.get("authorization_consumed") is not True
        or closure_final.get("workload_completed") is not False
        or closure_final.get("owned_pods_absent_verified") is not True
        or closure_final.get("create_requests_this_authorization") != 1
        or closure_final.get("automatic_replacement_queued") is not False
        or closure_final.get("evidence", {}).get("primary_failure_confirmed") is not True
        or closure_final.get("evidence", {}).get("source_unchanged") is not True
        or closure_final.get("failure_evidence_preserved") is not True
        or not isinstance(closure_final.get("estimated_attempt_cost_bound_usd"), (int, float))
        or closure_final["estimated_attempt_cost_bound_usd"] < 0
        or hashlib.sha256(CLOSURE_CONTROLLER.read_bytes()).hexdigest() != closure_freeze.get("controller_sha256")
    ):
        raise RuntimeError("failed dependency-closed native-scout attempt is not completely reconciled")
    for version in ("v1", "v2"):
        check = closure_final.get("checks", {}).get(version, {})
        if check.get("inventory"):
            raise RuntimeError("dependency-closed retry final inventory was not empty")
        if set(check.get("details_http_status", {}).values()) != {404}:
            raise RuntimeError("dependency-closed retry detail reconciliation changed")
    closure_cost = max(
        float(closure_final["estimated_attempt_cost_bound_usd"]),
        float(closure_final.get("billing", {}).get("current_pod_observed_cost_usd", 0.0)),
    )
    prior_cost += closure_cost
    return {
        ...
        "failed_dependency_closed_scout_retry_reconciled": True,
    }
...
def billing_check(client):
    ...
    return {"metadata": {key: metadata.get(key) for key in ("query", "recordCount", "uniquePodCount", "totals")},
            ...
            "new_comparative_campaign_observed_cost_usd": sum(
                ...
                if row["podId"] in {SCOUT_POD_ID, RETRY_POD_ID, CHUNK_POD_ID, CLOSURE_POD_ID}
...
if __name__ == "__main__":
    ...
    path = HERE / ("HTTP-NATIVE-SCOUT-P5-CLI-RETRY-PREFLIGHT-" + datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S-%f") + ".json")
