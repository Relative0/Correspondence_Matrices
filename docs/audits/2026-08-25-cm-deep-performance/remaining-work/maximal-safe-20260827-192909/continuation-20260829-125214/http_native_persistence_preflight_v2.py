"""Read-only native-persistence V2 preflight preserving and reconciling V1."""
...
import http_native_persistence_preflight_v1 as previous
...
V1_OUTPUT = HERE / "native-persistence-v1-001"
...
def check():
    ...
    if (
        ...
        or saved.get("pod_id") != "jof02fgwixhwj8"
        or validation.get("error") != "ModuleNotFoundError: No module named 'scripts'"
        ...
        raise RuntimeError("native persistence V1 result is not reconciled")
    ...
    try:
        ...
        client.close()
    if current != {"v1": [], "v2": []}:
        raise RuntimeError("nonempty RunPod inventory before native persistence V2")
    result["native_persistence_v1_reconciled"] = True
    result["native_persistence_v1_pod_id"] = saved["pod_id"]
    result["native_persistence_v1_estimated_compute_cost_usd"] = saved.get("estimated_compute_cost_usd")
