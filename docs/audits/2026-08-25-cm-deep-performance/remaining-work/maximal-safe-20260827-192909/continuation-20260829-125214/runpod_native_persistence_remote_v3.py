...
def publish_evidence(status, error, manifest, before):
    ...
    try:
        validation["d4_build"] = json.loads((OUT / "D4-BUILD.json").read_text(encoding="utf-8"))
    except Exception as exc:
...
try:
    ...
    with urllib.request.urlopen(request, timeout=30) as response:
        ...
        if content_length is not None and int(content_length) != 12250:
            raise RuntimeError("PaToH header content length")
        patoh_payload = response.read(12251)
    if (
        len(patoh_payload) != 12250
        or hashlib.sha256(patoh_payload).hexdigest()
        != "60e00eabe484f67f6efefa6747e4d4e218322e5a454ab3b6eac70a041afbd3bc"
    ...
    with (OUT / "D4-BUILD.json").open("x", encoding="utf-8") as stream:
        json.dump(
            {
                "status": "passed",
