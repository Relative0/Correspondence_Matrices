...
NEW_MANIFEST = HERE / "RUNPOD-P7-FUNCTIONAL-SCOUT-UPLOAD-MANIFEST-V6-20260830.json"
NEW_BUNDLE = HERE / "RUNPOD-P7-FUNCTIONAL-SCOUT-UPLOAD-BUNDLE-V6-20260830.zip"
...
def main() -> None:
    ...
    for case in freeze["cases"]:
        ...
        if target in frozen_sources:
            prior = frozen_sources[target]
            if (prior["bytes"], prior["sha256"]) != identity:
                raise RuntimeError(f"inconsistent frozen source identity: {target}")
        else:
            frozen_sources[target] = source
    existing_targets = {row["target"] for row in rows}
    added_static_targets = {
        FEATURES_TARGET, CORPUS_TARGET, BACKENDS_INIT_TARGET, BITSET_ENGINE_TARGET,
    }
    missing_frozen_sources = {
        target: source
        for target, source in frozen_sources.items()
        if target not in existing_targets and target not in added_static_targets
    }
    if not missing_frozen_sources:
        raise RuntimeError("no missing frozen sources detected")
    ...
    for target, frozen in sorted(missing_frozen_sources.items()):
    ...
    manifest["correction"] = (
        ...
        "and every remaining frozen regression, development, and confirmation "
        "source required by the full offline-gate identity verification. "
