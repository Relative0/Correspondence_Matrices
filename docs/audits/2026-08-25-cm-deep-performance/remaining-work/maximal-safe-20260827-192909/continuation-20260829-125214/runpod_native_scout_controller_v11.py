"""One-create native scout with a nonperformance native-worker measurement fence."""
import argparse
import base64
...
import requests
import http_native_scout_preflight_v11 as preflight

HERE = Path(__file__).resolve().parent
TRANSPORT = HERE.parent / "runpod-authorized-20260827-213104"
OUT = HERE / "native-procfs-v11-001"
...
MANIFEST_PATH = HERE / "RUNPOD-NATIVE-SCOUT-UPLOAD-MANIFEST-V8-20260829.json"
PROPOSAL_PATH = HERE / "RUNPOD-NATIVE-SCOUT-MEASUREMENT-FENCE-RETRY-PROPOSAL-20260829.md"
AUTHORIZATION_PATH = HERE / "HTTP-NATIVE-SCOUT-V11-MEASUREMENT-FENCE-AUTHORIZED-20260829.json"
...
def require_authorization():
    ...
    expected = {
        "schema": "cm-runpod-native-scout-v11-measurement-fence-authorization/v1",
        "authorized": True,
        "one_create": True,
        "no_replacement": True,
        "source_files": 37,
        ...
        "prior_failed_pod_ids": [
            ...
            "pow0qre2q39m4t", "3o7r0za7cm72yn", "jwyi342sjmjkcj", "kvpu2s8ozs7j27"
        ],
...
def save_evidence(log):
    ...
    if (
        validation.get("junit_testcases") != {"tests": 65, "failures": 0, "errors": 0, "skipped": 0}
        or type(junit_metadata.get("tests")) is not int
        or junit_metadata.get("tests", 0) < 65
...
def run():
    ...
    try:
        ...
        if (len(manifest["files"]) != 37 or manifest.get("bytes") != 5507655
