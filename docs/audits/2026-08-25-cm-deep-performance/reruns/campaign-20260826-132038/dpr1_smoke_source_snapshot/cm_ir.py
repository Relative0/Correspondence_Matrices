...
class CMIRBuilder:
    def __init__(
        ...
        *,
        share_aware_flatten: bool = True,
        build_memo: bool = True,
        compact_canonical_order: bool = False,
    ):
        ...
        # DP-R1 experimental path: exact immutable labels ordered identically
        # to public CMNode.key, but compared without revisiting deep tuples.
        # Disabled by default until the paired acceptance study passes.
        self.compact_canonical_order = bool(compact_canonical_order)
        self._order_descriptors: List[Tuple[object, ...]] = []
        self._order_labels: List[Fraction] = []
        self._order_label_by_uid: List[Fraction] = []
    ...
    def _register_order_label(self, node: CMNode, uid: int) -> None:
        if node.kind == "var":
            descriptor: Tuple[object, ...] = ("VAR", node.var_name)
        elif node.kind == "const":
            descriptor = ("CONST", node.const_value)
        else:
            descriptor = (node.op,) + tuple(
                self._order_label_by_uid[self._node_uid(arg)] for arg in node.args
            )
        position = bisect_left(self._order_descriptors, descriptor)
        if position < len(self._order_descriptors) and self._order_descriptors[position] == descriptor:
            raise AssertionError("new intern node duplicated an ordering descriptor")
        if not self._order_labels:
            label = Fraction(0)
        elif position == 0:
            label = self._order_labels[0] - 1
        elif position == len(self._order_labels):
            label = self._order_labels[-1] + 1
        else:
            label = (self._order_labels[position - 1] + self._order_labels[position]) / 2
        self._order_descriptors.insert(position, descriptor)
        self._order_labels.insert(position, label)
        if uid != len(self._order_label_by_uid):
            raise AssertionError("non-contiguous builder intern uid")
        self._order_label_by_uid.append(label)

    def _canonical_order_key(self, node: CMNode) -> object:
        if not self.compact_canonical_order:
            return node.key
        return self._order_label_by_uid[self._node_uid(node)]
    ...
    def _intern(
        ...
        *,
        ...
        uid = len(self._uid_of_node)
        self._uid_of_node[id(node)] = uid
        if self.compact_canonical_order:
            self._register_order_label(node, uid)
        if t0 is not None:
            _add_float(self.diagnostics, "ir_intern_time_s", time.perf_counter() - t0)
        return node
        ...
        try:
            ...
            for node in args:
                if node.kind == "binary" and node.op == op and op in ASSOCIATIVE_OPS:
                    ...
                    out.append(node)
            sorted_out = sorted(out, key=self._canonical_order_key)
    ...
    def make_xor(self, args: Sequence[CMNode]) -> CMNode:
        ...
        out = [node_by_uid[uid] for uid in
               sorted(counts, key=lambda u: self._canonical_order_key(node_by_uid[u]))
               if (counts[uid] % 2) == 1]
    ...
    def make_eqv(self, left: CMNode, right: CMNode) -> CMNode:
        ...
        ordered = tuple(sorted((left, right), key=self._canonical_order_key))
