...
def _summaries(rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for name, group in groups:
        ...
        memory = [float(row["peak_bytes_ratio"]) for row in group]
        ...
        output.append(
            {
                ...
                "peak_bytes_ratio_geomean": _geomean(memory),
...
def main() -> int:
    ...
    for corpus in corpora:
        ...
        for index, record in enumerate(records, 1):
            row = _measure_record(corpus, record, args.repetitions)
            rows.append(row)
            print(
    ...
    environment = {
        ...
        "allocation_window": "one cold compile per arm under tracemalloc",
