...
uses ``memo_by_uid`` while it builds the CM IR.  Before the 2026-08-25 change,
the production path also maintained an id-keyed memo.  This harness constructs
that historical two-memo baseline and the one-memo candidate explicitly, so the
ablation remains reproducible after the candidate becomes production.  It
changes no production state and refuses overwrite.
"""
...
from cm_ir import CMIRBuilder, _BuildState  # noqa: E402
...
def _baseline_compile(expr: Any) -> Any:
    """Compile with both the structural UID and historical identity memos."""
    builder = CMIRBuilder(share_aware_flatten=True, build_memo=True)
    uid_by_id, shared_uids = builder._shared_assoc_uids(expr)
...
def _dag_signature(node: Any) -> tuple[tuple[Any, ...], ...]:
    """Exact canonical CM-DAG signature without unfolding ``CMNode.key``.

    Direct equality of independently built deep keys follows every unfolded
    occurrence and is impractical for high-sharing circuit cones.  Ordered
    child indices preserve the exact canonical DAG shape in O(s) space/time.
    """
    ...
    while stack:
        ...
        if not processed:
            ...
            continue
        child_indices = tuple(index_by_id[id(child)] for child in current.args)
...
def _measure_record(
    ...
    baseline = lambda: _baseline_compile(expr)
    ...
    if _dag_signature(before_node) != _dag_signature(after_node):
        raise AssertionError(f"{record.get('id')}: canonical DAG mismatch")
