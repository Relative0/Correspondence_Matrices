...
class ExactDispatcherTests(unittest.TestCase):
    ...
    def test_c14_task_policy_abstention_and_global_disable(self):
        ...
        foreign_identity = {
            **identity,
            "system": "Windows" if identity["system"] == "Linux" else "Linux",
        }
        foreign = select_task_arm(
            policy, ExactTaskContract("latency_sensitive"), n_vars=8,
            identity=foreign_identity)
        self.assertTrue(foreign.abstained)
        self.assertEqual(foreign.selected_arm, "set_no_sentinel")
        self.assertEqual(foreign.reason, "platform_calibration_mismatch")
        self.assertEqual(select_task_arm(
