...
class FreshPersistenceTests(unittest.TestCase):
    def test_inventory_never_substitutes_autoref_for_native_arms(self):
        ...
        for arm in ("cudd_bdd", "cudd_zdd", "d4_ddnnf"):
            with self.subTest(arm=arm):
                available = inventory[arm]["status"] == "available"
                self.assertEqual(inventory[arm]["native_execution"], available)
                self.assertFalse(inventory[arm]["portability_control"])
                if available:
                    self.assertEqual(inventory[arm]["identity"]["status"], "identified")
                    if arm != "d4_ddnnf":
                        self.assertTrue(inventory[arm]["identity"]["native_extension"])
    ...
    def test_bdd_artifact_tamper_and_invalid_graph_are_refused(self):
        ...
        with tempfile.TemporaryDirectory(prefix="cm-fresh-bdd-") as directory:
            ...
            self.assertEqual(build["identity"]["adapter"], "canonical_bdd_reachable_graph/v1")

    def test_bdd_artifacts_are_canonical_across_fresh_builds(self):
        inventory = fresh.capability_inventory()
        with tempfile.TemporaryDirectory(prefix="cm-fresh-bdd-canonical-") as directory:
            root = Path(directory)
            for arm in fresh.BDD_ARMS:
                if inventory[arm]["status"] != "available":
                    continue
                with self.subTest(arm=arm):
                    first_path = (root / f"cell-{arm}-first.json").resolve()
                    second_path = (root / f"cell-{arm}-second.json").resolve()
                    first = fresh.build_artifact(scenario(), arm, first_path)
                    second = fresh.build_artifact(scenario(), arm, second_path)
