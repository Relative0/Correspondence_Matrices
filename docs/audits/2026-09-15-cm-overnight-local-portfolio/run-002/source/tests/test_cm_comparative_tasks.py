...
class ComparativeTaskTests(unittest.TestCase):
    def run_task(self, task, backend, lifecycle):
        ...
        tasks.validate_task_result(result, contract, expected, expected_backend=backend, expected_case_id="matched-k6")
    ...
    def test_mutated_semantic_output_counts_or_claims_are_refused(self):
        ...
        mutations.append(changed)
        changed = copy.deepcopy(result)
        changed["resources"]["rss_measured"] = True
        mutations.append(changed)
        for changed in mutations:
            with self.assertRaises(ValueError):
                tasks.validate_task_result(
                    changed, contract, expected, expected_backend="cnf", expected_case_id="matched-k6"
                )
