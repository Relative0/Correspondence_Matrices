...
class MeasurementVerificationTests(unittest.TestCase):
    ...
    def fake_cell(self, behavior, expected=1, cell_id="test"):
        return verify.run_cell(self.root, self.ledger, cell_id, self.request(), expected, invoke=behavior)

    def request(self):
        return {"mode": "measure", "arm": "cnf", "case": {"k": 0, "clauses": []}}

    def completed(self, value=1):
        request = self.request()
        result = verify.measure(request["case"], request["arm"])
        result.update(packed_hex=hex(value), pid=1, interpreter=sys.executable, source_root=str(self.root),
                      request_sha256=verify.digest(request))
        return subprocess.CompletedProcess([], 0, verify.encoded(result), b"")
