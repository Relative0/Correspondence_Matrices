...
def configure(attempt, profile):
    ...
    transport.REMOTE_CODE = (ROOT / 'scripts/cm_next_research_remote.py').read_text()
...
def check_inventory(snapshot):
    ...
    for rows in snapshot.values():
        for row in rows:
            ...
            if directory is None or not (directory/'RESERVATION.json').is_file():
                raise RuntimeError('foreign pod present; leaving it untouched')
...
def run(attempt, out):
    ...
    try:
        ...
        assert digest(ROOT / 'scripts/cm_next_research_remote.py') == frozen['remote_sha256']
