...
AXIS = np.array([False, True], dtype=bool)  # [0,1] index 0=False, 1=True

def _node_from_var_node(v: Var):
    name = getattr(v, 'name', None)
    if isinstance(name, str):
        return AXIS.copy(), [name]
    # fall back to index-based name from Var.i
    return AXIS.copy(), [f"x{int(v.i)}"]
...
def _compile_lazy(e):
    if isinstance(e, Var): return _node_from_var_node(e)
    if isinstance(e, Not):
