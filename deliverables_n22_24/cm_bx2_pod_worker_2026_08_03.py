"""BX2 pod worker: install dd with CUDD, run the order-sensitivity leg, serve results.
...
errors out â€” no autoref fallback is ever measured.
"""
...
def campaign():
    try:
        ...
        if "cudd-ok" not in check.stdout:
            raise RuntimeError("dd.cudd unavailable after install â€” fail closed, "
        ...
        result = subprocess.run(
            ["python", "deliverables_n22_24/cm_bx2_cudd_orders_2026_08_03.py"],
    ...
    def do_GET(self):
        if self.path == "/health":
            self.send_json({"ok": True, "service": "cm-remote-worker",
                            "role": "bx2-cudd-orders", "stage": STATE["stage"]})
        elif self.path == "/progress":
            self.send_json(STATE)
        elif self.path == "/results":
            files = {}
            out_dir = REPO / "deliverables_n22_24" / "pod_out"
            for name in ("cm_bx2_cudd_orders_results_2026_08_03.json",
                         "CM_bx2_cudd_orders_summary_2026_08_03.csv"):
                p = out_dir / name
                if p.exists():
