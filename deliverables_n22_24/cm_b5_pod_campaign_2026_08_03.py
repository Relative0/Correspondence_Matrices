"""B5 orchestrator — one Linux pod with dd.cudd for the matched comparison.
...
OUT_DIR = BASE / "b5_cudd_2026_08_03_run5"
...
def main():
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with zipfile.ZipFile(archive_path, "w", zipfile.ZIP_DEFLATED) as ar:
                ...
                for path in (REPO / "cmbench").rglob("*.py"):
                    ar.write(path, path.relative_to(REPO))
                for name in ("cm_b5_cudd_matched_2026_08_03.py",
            ...
            common = {
                "name": f"cm-b5-cudd-{int(time.time())}",
            ...
            for source, remote in ((archive_path, "repo.zip"),
                                   (BASE / "cm_b5_pod_worker_2026_08_03.py",
                                    "cm_remote_worker.py")):
                post_retry(
                    f"{boot}/put",
                    json={"name": remote,
        ...
        (OUT_DIR / "b5_pod_audit_2026_08_03.json").write_text(
